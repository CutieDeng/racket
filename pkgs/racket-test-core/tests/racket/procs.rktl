...
(define-syntax-rule (make-make-procs make-procs body arg ...)
  (define (make-procs arg ...) body))

(apply-to-all-procs
 make-make-procs make-procs
 `((,f0 0 () ())
   (,(wrap f0) 0 () ())
...
(module all-procs-in-a-module racket/base
  (require (for-syntax racket/base)
           racket/include)
  (include "proc-defs.rktl")
  (apply-to-all-procs provide))

(define realmed-module
  `(module all-procs-in-a-realmed-module racket/base
     (#%declare #:realm other-world)
     (require (for-syntax racket/base)
              racket/include)
     (include "proc-defs.rktl")
     (apply-to-all-procs provide)))
...
(define (compile-with-env-config key val name)
  ...
  (close-output-port o)
  (define compiled-mod (parameterize ([read-accept-compiled #t])
                         (read i)))
  (close-input-port i)
  (close-input-port e)
...
(cond
  [(eq? 'chez-scheme (system-type 'vm))
   (compile-with-env-config #"PLT_CS_COMPILE_LIMIT" #"10" 'all-procs-in-a-realmed-module/limited)
   (compile-with-env-config #"PLT_CS_JIT" #"y" 'all-procs-in-a-realmed-module/jit)]
  [else
   (eval '(module all-procs-in-a-realmed-module/limited racket/base
            (require 'all-procs-in-a-realmed-module)
            (provide (all-from-out 'all-procs-in-a-realmed-module))))
   (eval '(module all-procs-in-a-realmed-module/jit racket/base
            (require 'all-procs-in-a-realmed-module)
            (provide (all-from-out 'all-procs-in-a-realmed-module))))])

(define procs
  (append (apply-to-all-procs make-procs)
          (let ()
            (local-require 'all-procs-in-a-module)
            (apply-to-all-procs make-procs))
          (let ()
            (local-require 'all-procs-in-a-realmed-module)
            (apply-to-all-procs make-procs))
          (if (eq? 'chez-scheme (system-type 'vm))
              (append
               (let ()
                 (local-require 'all-procs-in-a-realmed-module/limited)
                 (apply-to-all-procs make-procs))
               (let ()
                 (local-require 'all-procs-in-a-realmed-module/jit)
                 (apply-to-all-procs make-procs)))
              null)))
