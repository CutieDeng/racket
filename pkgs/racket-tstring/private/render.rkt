...
;; write-template — 把模板各段直接流写到端口, 不物化整串。
;;
;; 目的: 与 printf 性能对齐。(display t"x={v}") 经此路径 = 写字面段 +
;; 逐插值直接写端口, 与 (printf "x=~a" v) 等量工作; 相比 f"" 的
;; (string-append ...) 省掉整条输出串的分配 (用户指出的"多一次拷贝")。
;;
;; 快路径: 插值无 format-spec 且无 conversion (常见的 {v}) → 直接
;; (display value port), 语义等同 ~a 但省掉中间字符串; 否则走
;; format-fstring-value 格式化后 write-string (仅带格式的插值才分配)。
(define (write-template tpl port)
  (define-values (strings interpolations)
    (checked-template-parts 'write-template tpl))
  (let loop ((strings strings) (interpolations interpolations))
    (write-string (car strings) port)
    (cond
      ((null? interpolations) (void))
      (else
       (define i (car interpolations))
       (define spec (interpolation-format-spec i))
       (define conv (interpolation-conversion i))
       (if (and (not spec) (or (not conv) (eq? conv 'none)))
           ;; 快路径: {v} 无格式 → 直接写端口 (≡ printf ~a, 零中间串)
           (display (interpolation-value i) port)
           ;; 慢路径: 带 format-spec/conversion, 仅此时分配格式化串
           (write-string
            (format-fstring-value (interpolation-value i) spec conv) port))
       (loop (cdr strings) (cdr interpolations))))))

;; 装填 template.rkt 的流式渲染钩子 → (display a-template) 走 write-template。
(set-box! template-write-hook write-template)
