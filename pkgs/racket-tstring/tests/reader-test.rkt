...
(check-equal? formatted-rendered "002")
(check-equal? converted-rendered "\"hi\"")
(check-true (template? formatted-template))
(check-equal? (interpolation-value (car (template-interpolations formatted-template))) "Alice")
(check-equal? (interpolation-format-spec (car (template-interpolations formatted-template))) ">8s")
