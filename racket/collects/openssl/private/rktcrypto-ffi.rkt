...
;; PBKDF2 (RFC 8018) and raw AES-CBC — used to load encrypted PKCS#8 (PBES2).
(define-rkt rktcrypto_pbkdf2
  (_fun _int _bytes _intptr _bytes _intptr _uint32 _bytes _intptr -> _int))
(define-rkt rktcrypto_aes_cbc_decrypt
  (_fun _bytes _intptr _bytes _bytes _bytes _intptr -> _void))
;; 3DES-CBC / RC2-CBC (whole 8-byte blocks; encrypt flag) — legacy PKCS#12 PBE.
(define-rkt rktcrypto_des3_cbc
  (_fun _bytes _bytes _bytes _bytes _intptr _int -> _void))
(define-rkt rktcrypto_rc2_cbc
  (_fun _bytes _intptr _intptr _bytes _bytes _bytes _intptr _int -> _void))
;; CMS SignedData verification (single signer, embedded content+cert).
(define-rkt rktcrypto_cms_verify
  (_fun _bytes _intptr -> _int))
;; RSAES-OAEP-SHA256 public-key encrypt (n,e) — CMS EnvelopedData key transport.
(define-rkt rktcrypto_rsa_oaep_encrypt_pub
  (_fun _bytes _intptr _bytes _intptr _bytes _intptr _bytes -> _int))
;; AES-CBC encrypt (whole 16-byte blocks; caller pads).
(define-rkt rktcrypto_aes_cbc_encrypt
  (_fun _bytes _intptr _bytes _bytes _bytes _intptr -> _void))
