...
;; 变参构造器: (intbits 1 3 5) → 含这些位置的位集
(define (intbits . positions
        ) ; end intbits
  (list->intbits positions
  ) ; end list->intbits
) ; end define

;; 推导式: 每轮 body 的值作为一个位置收进位集
(define-syntax (for/intbits stx
               ) ; end for/intbits
  (syntax-case stx (
                   ) ; end literals
    [(_ clauses body ...)
     (quasisyntax/loc stx
       (for/fold/derived #,stx ([bs intbits-empty]) clauses
         (intbits-set bs (let (
                              ) ; end bindings
                           body ...
                         ) ; end let
