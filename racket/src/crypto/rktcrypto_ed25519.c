...
int rktcrypto_ed25519_verify(const unsigned char sig[64],
  ...
  /* RFC 8032 §5.1.7: reject non-canonical S (must have 0 <= S < L). The
     sig[63]&224 test only bounds S < 2^253; enforce the full order L here so
     S = s0 + L (and other S in [L, 2^253)) are rejected. */
  { static const unsigned char L[32]={
      0xed,0xd3,0xf5,0x5c,0x1a,0x63,0x12,0x58,0xd6,0x9c,0xf7,0xa2,0xde,0xf9,0xde,0x14,
      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x10};
    int j,lt=0; for(j=31;j>=0;j--){ if(sig[32+j]!=L[j]){ lt=sig[32+j]<L[j]; break; } }
    if(!lt) return 0; }               /* S >= L (or S == L) */
