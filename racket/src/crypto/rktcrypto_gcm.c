...
/* Portable GMAC (GCM auth over msg-as-AAD, empty ciphertext) — same FIPS
   construction as the NEON path, over the portable bit-by-bit GHASH. */
void rktcrypto_aes_gmac(const unsigned char *key, intptr_t keylen, const unsigned char iv[12],
                        const unsigned char *msg, intptr_t len, unsigned char tag[16])
{
  unsigned char rk[240], zero[16], hblock[16], lenblock[16], s[16], j0[16], ej0[16];
  uint64_t h[2], acc[2] = {0, 0};
  int Nr, i;
  Nr = rktcrypto_aes_expand_key(key, keylen, rk);
  memset(zero, 0, 16);
  ...
  ghash_bytes(acc, h, msg, len);                            /* msg authenticated as AAD */
  { uint64_t abits = (uint64_t)len << 3;                    /* len(aad) || len(ct)=0 */
    for (i = 0; i < 8; i++) lenblock[i]     = (unsigned char)(abits >> (56 - 8 * i));
    for (i = 8; i < 16; i++) lenblock[i] = 0; }
