/* Legacy ciphers: Triple-DES (FIPS 46-3, EDE), RC4 (Rivest's cipher 4), and
   Camellia (RFC 3713).

   3DES (64-bit block, Sweet32) and RC4 (keystream bias) are cryptographically
   weak; Camellia is sound but legacy relative to AES. All exist so code paths
   that must interoperate with old protocols and file formats (TLS/CMS/etc.)
   need no external library. From scratch, no external code; not for new
   designs. */

#include "rktcrypto_cipher.h"
#include <string.h>
#include <stdint.h>

/* ============================ DES / 3DES ============================ */
/* FIPS bit numbering: bit 1 is the most significant bit of the block. */

static const unsigned char DES_IP[64]={58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7};
static const unsigned char DES_FP[64]={40,8,48,16,56,24,64,32,39,7,47,15,55,23,63,31,38,6,46,14,54,22,62,30,37,5,45,13,53,21,61,29,36,4,44,12,52,20,60,28,35,3,43,11,51,19,59,27,34,2,42,10,50,18,58,26,33,1,41,9,49,17,57,25};
static const unsigned char DES_E[48]={32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1};
static const unsigned char DES_P[32]={16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25};
static const unsigned char DES_PC1[56]={57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36,63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4};
static const unsigned char DES_PC2[48]={14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32};
static const unsigned char DES_SH[16]={1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
static const unsigned char DES_S[8][64]={
{14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7,0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8,4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0,15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13},
{15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10,3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5,0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15,13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9},
{10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8,13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1,13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7,1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12},
{7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15,13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9,10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4,3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14},
{2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9,14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6,4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14,11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3},
{12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11,10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8,9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6,4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13},
{4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1,13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6,1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2,6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12},
{13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7,1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2,7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8,2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11}};

static uint64_t des_perm(uint64_t in, const unsigned char *tab, int n, int inbits){
  uint64_t o=0; int i;
  for(i=0;i<n;i++){ o=(o<<1)|((in>>(inbits-tab[i]))&1); }
  return o;
}

/* SP tables fuse each S-box with the P permutation: SP[b][6bit] already holds
   the P-permuted 32-bit contribution, so the feistel is 8 lookups XORed --
   no per-bit P loop. Built once from the S/P tables. */
static uint32_t DES_SP[8][64];
/* Byte-indexed IP/FP: IP_T[i][v] = IP applied to value v placed at input byte i.
   IP(x) = OR_i IP_T[i][byte_i(x)] -- no 64-bit permutation loop per block. */
static uint64_t DES_IP_T[8][256], DES_FP_T[8][256];
static int des_sp_inited=0;
static void des_sp_init(void){
  int b,j,v;
  for(b=0;b<8;b++) for(j=0;j<64;j++){
    int row=((j>>5)<<1)|(j&1), col=(j>>1)&0xF;
    uint32_t sval=DES_S[b][row*16+col];
    DES_SP[b][j]=(uint32_t)des_perm((uint32_t)sval<<(28-4*b),DES_P,32,32);
  }
  for(b=0;b<8;b++) for(v=0;v<256;v++){
    uint64_t in=(uint64_t)v<<(56-8*b);
    DES_IP_T[b][v]=des_perm(in,DES_IP,64,64);
    DES_FP_T[b][v]=des_perm(in,DES_FP,64,64);
  }
  des_sp_inited=1;
}
/* Subkey stored PRE-SPLIT into its eight 6-bit groups (des_key[r][b]) so the
   feistel hot loop does no per-lookup subkey shift and no post-XOR mask: each
   R-window (<=0x3f) XOR a 6-bit key group (<=0x3f) indexes SP directly. */
typedef struct { unsigned char g[16][8]; } des_ks;
static void des_schedule(const unsigned char k[8], des_ks *ks){
  uint64_t key=0, cd, sk; uint32_t C,D; int r,b;
  if(!des_sp_inited) des_sp_init();
  { int i; for(i=0;i<8;i++) key=(key<<8)|k[i]; }
  cd=des_perm(key,DES_PC1,56,64); C=(cd>>28)&0xFFFFFFF; D=cd&0xFFFFFFF;
  for(r=0;r<16;r++){ int s=DES_SH[r];
    C=((C<<s)|(C>>(28-s)))&0xFFFFFFF; D=((D<<s)|(D>>(28-s)))&0xFFFFFFF;
    sk=des_perm(((uint64_t)C<<28)|D,DES_PC2,48,56);
    for(b=0;b<8;b++) ks->g[r][b]=(unsigned char)((sk>>(42-6*b))&0x3F); }
}

/* E-expansion via shifts (no 48-bit permutation loop): the eight overlapping
   6-bit groups of E(R) are contiguous windows of R (with wraparound at the ends).
   Each group is XORed with the pre-split 6-bit subkey group and indexes SP. */
static uint32_t des_feistel(uint32_t R, const unsigned char kk[8]){
  /* The two wrapping E-groups are exactly single rotates: g0 = ror(R,27)&0x3f,
     g7 = ror(R,31)&0x3f -- one ROR each instead of shift|shift|mask|or, so all
     eight index computations are ~equal length (shortens the serial Feistel path
     that bounds CBC-encrypt). */
  uint32_t r0=(R>>27)|(R<<5), r7=(R>>31)|(R<<1);
  return DES_SP[0][(r0&0x3F) ^ kk[0]]
        ^DES_SP[1][((R>>23)&0x3F) ^ kk[1]]
        ^DES_SP[2][((R>>19)&0x3F) ^ kk[2]]
        ^DES_SP[3][((R>>15)&0x3F) ^ kk[3]]
        ^DES_SP[4][((R>>11)&0x3F) ^ kk[4]]
        ^DES_SP[5][((R>>7)&0x3F) ^ kk[5]]
        ^DES_SP[6][((R>>3)&0x3F) ^ kk[6]]
        ^DES_SP[7][(r7&0x3F) ^ kk[7]];
}

/* Fused 3DES block: the intermediate FP (end of stage k) and IP (start of stage
   k+1) are inverses and cancel, so one IP + 48 rounds (swap between stages) +
   one FP instead of 3 IP + 3 FP. */
/* IP (byte->state) and FP (state->bytes) via the byte-indexed tables. */
static inline uint64_t des_ip(const unsigned char in[8]){
  return DES_IP_T[0][in[0]]|DES_IP_T[1][in[1]]|DES_IP_T[2][in[2]]|DES_IP_T[3][in[3]]
        |DES_IP_T[4][in[4]]|DES_IP_T[5][in[5]]|DES_IP_T[6][in[6]]|DES_IP_T[7][in[7]];
}
static inline void des_fp_store(uint64_t pre, unsigned char out[8]){
  uint64_t o=DES_FP_T[0][(pre>>56)&0xFF]|DES_FP_T[1][(pre>>48)&0xFF]|DES_FP_T[2][(pre>>40)&0xFF]|DES_FP_T[3][(pre>>32)&0xFF]
           |DES_FP_T[4][(pre>>24)&0xFF]|DES_FP_T[5][(pre>>16)&0xFF]|DES_FP_T[6][(pre>>8)&0xFF]|DES_FP_T[7][pre&0xFF];
  out[0]=(o>>56)&0xFF;out[1]=(o>>48)&0xFF;out[2]=(o>>40)&0xFF;out[3]=(o>>32)&0xFF;
  out[4]=(o>>24)&0xFF;out[5]=(o>>16)&0xFF;out[6]=(o>>8)&0xFF;out[7]=o&0xFF;
}
static inline void des3_dirs(const des_ks *s1,const des_ks *s2,const des_ks *s3,
                             const des_ks *S[3],int fwd[3],int encrypt){
  if(encrypt){ S[0]=s1;fwd[0]=1; S[1]=s2;fwd[1]=0; S[2]=s3;fwd[2]=1; }
  else       { S[0]=s3;fwd[0]=0; S[1]=s2;fwd[1]=1; S[2]=s1;fwd[2]=0; }
}
/* Core on the IP'd 64-bit state -> pre-output state (before FP). Fused: one IP +
   48 rounds (swap between stages) + one FP. */
static uint64_t des3_core(const des_ks *S[3],const int fwd[3],uint64_t b){
  int st,r; uint32_t L=(uint32_t)(b>>32),R=(uint32_t)b,t;
  for(st=0;st<3;st++){
    for(r=0;r<16;r++){ int rr=fwd[st]?r:15-r; uint32_t nR=L^des_feistel(R,S[st]->g[rr]); L=R; R=nR; }
    t=L; L=R; R=t;
  }
  return ((uint64_t)L<<32)|R;
}
/* EDE-encrypt core, fully unrolled with the subkey order baked in (s1 forward,
   s2 backward, s3 forward): no fwd[] select or S[st] indirection, so the compiler
   sees one clean 48-round serial chain -- the direction of the CBC-encrypt hot
   loop, whose critical path is this chain. */
static uint64_t des3_enc_core(const des_ks *s1,const des_ks *s2,const des_ks *s3,uint64_t b){
  uint32_t L=(uint32_t)(b>>32),R=(uint32_t)b,t,nR;
  #define FR(kk) do{ nR=L^des_feistel(R,(kk)); L=R; R=nR; }while(0)
  FR(s1->g[0]);FR(s1->g[1]);FR(s1->g[2]);FR(s1->g[3]);FR(s1->g[4]);FR(s1->g[5]);FR(s1->g[6]);FR(s1->g[7]);
  FR(s1->g[8]);FR(s1->g[9]);FR(s1->g[10]);FR(s1->g[11]);FR(s1->g[12]);FR(s1->g[13]);FR(s1->g[14]);FR(s1->g[15]);
  t=L;L=R;R=t;
  FR(s2->g[15]);FR(s2->g[14]);FR(s2->g[13]);FR(s2->g[12]);FR(s2->g[11]);FR(s2->g[10]);FR(s2->g[9]);FR(s2->g[8]);
  FR(s2->g[7]);FR(s2->g[6]);FR(s2->g[5]);FR(s2->g[4]);FR(s2->g[3]);FR(s2->g[2]);FR(s2->g[1]);FR(s2->g[0]);
  t=L;L=R;R=t;
  FR(s3->g[0]);FR(s3->g[1]);FR(s3->g[2]);FR(s3->g[3]);FR(s3->g[4]);FR(s3->g[5]);FR(s3->g[6]);FR(s3->g[7]);
  FR(s3->g[8]);FR(s3->g[9]);FR(s3->g[10]);FR(s3->g[11]);FR(s3->g[12]);FR(s3->g[13]);FR(s3->g[14]);FR(s3->g[15]);
  t=L;L=R;R=t;
  #undef FR
  return ((uint64_t)L<<32)|R;
}
/* 4-way interleaved core: four independent blocks share each round's subkey, so
   the four des_feistel SP-table load chains overlap -- hides the ~48-round Feistel
   latency (throughput- not latency-bound), the win OpenSSL's single-block C can't
   get. For ECB and CBC-decrypt, where blocks are independent. */
static void des3_core4(const des_ks *S[3],const int fwd[3],const uint64_t b[4],uint64_t o[4]){
  int st,r; uint32_t L0=(uint32_t)(b[0]>>32),R0=(uint32_t)b[0],L1=(uint32_t)(b[1]>>32),R1=(uint32_t)b[1],
    L2=(uint32_t)(b[2]>>32),R2=(uint32_t)b[2],L3=(uint32_t)(b[3]>>32),R3=(uint32_t)b[3],t;
  for(st=0;st<3;st++){
    for(r=0;r<16;r++){ int rr=fwd[st]?r:15-r; const unsigned char *k=S[st]->g[rr];
      uint32_t n0=L0^des_feistel(R0,k); L0=R0; R0=n0;
      uint32_t n1=L1^des_feistel(R1,k); L1=R1; R1=n1;
      uint32_t n2=L2^des_feistel(R2,k); L2=R2; R2=n2;
      uint32_t n3=L3^des_feistel(R3,k); L3=R3; R3=n3; }
    t=L0;L0=R0;R0=t; t=L1;L1=R1;R1=t; t=L2;L2=R2;R2=t; t=L3;L3=R3;R3=t;
  }
  o[0]=((uint64_t)L0<<32)|R0; o[1]=((uint64_t)L1<<32)|R1;
  o[2]=((uint64_t)L2<<32)|R2; o[3]=((uint64_t)L3<<32)|R3;
}

/* 3DES-EDE, key = k1||k2||k3 (24 bytes). encrypt!=0 -> EDE, else DED. */
void rktcrypto_des3_ecb(const unsigned char key[24], const unsigned char *in,
                        unsigned char *out, intptr_t nblk, int encrypt){
  des_ks s1,s2,s3; const des_ks *S[3]; int fwd[3]; intptr_t i=0;
  des_schedule(key,&s1); des_schedule(key+8,&s2); des_schedule(key+16,&s3);
  des3_dirs(&s1,&s2,&s3,S,fwd,encrypt);
  for(; i+4<=nblk; i+=4){ uint64_t b[4],o[4];
    b[0]=des_ip(in+8*i); b[1]=des_ip(in+8*i+8); b[2]=des_ip(in+8*i+16); b[3]=des_ip(in+8*i+24);
    des3_core4(S,fwd,b,o);
    des_fp_store(o[0],out+8*i); des_fp_store(o[1],out+8*i+8); des_fp_store(o[2],out+8*i+16); des_fp_store(o[3],out+8*i+24); }
  for(; i<nblk; i++) des_fp_store(des3_core(S,fwd,des_ip(in+8*i)),out+8*i);
}

void rktcrypto_des3_cbc(const unsigned char key[24], const unsigned char iv[8],
                        const unsigned char *in, unsigned char *out, intptr_t nblk, int encrypt){
  des_ks s1,s2,s3; const des_ks *S[3]; int fwd[3]; intptr_t i;
  des_schedule(key,&s1); des_schedule(key+8,&s2); des_schedule(key+16,&s3);
  des3_dirs(&s1,&s2,&s3,S,fwd,encrypt);
  if(encrypt){
    /* IP and FP are inverse linear permutations, so IP(P_i ^ C_{i-1}) = IP(P_i) ^
       pre_{i-1} (the previous pre-output state): the chaining stays a single word
       XOR -- no byte fold, no memcpy, no re-IP. */
    uint64_t prevpre=des_ip(iv);
    for(i=0;i<nblk;i++){ uint64_t pre=des3_enc_core(&s1,&s2,&s3,des_ip(in+8*i)^prevpre);
      des_fp_store(pre,out+8*i); prevpre=pre; }
  } else {
    /* decrypt: blocks are independent given the ciphertext -> 4-way interleave,
       then XOR each plaintext with the previous ciphertext word. */
    unsigned char prev[8]; memcpy(prev,iv,8); intptr_t j=0;
    for(; j+4<=nblk; j+=4){ uint64_t b[4],o[4]; unsigned char *op=out+8*j; const unsigned char *ip=in+8*j;
      b[0]=des_ip(ip); b[1]=des_ip(ip+8); b[2]=des_ip(ip+16); b[3]=des_ip(ip+24);
      des3_core4(S,fwd,b,o);
      unsigned char pt[32]; des_fp_store(o[0],pt); des_fp_store(o[1],pt+8); des_fp_store(o[2],pt+16); des_fp_store(o[3],pt+24);
      for(int q=0;q<8;q++) op[q]=pt[q]^prev[q];
      for(int q=0;q<8;q++) op[8+q]=pt[8+q]^ip[q];
      for(int q=0;q<8;q++) op[16+q]=pt[16+q]^ip[8+q];
      for(int q=0;q<8;q++) op[24+q]=pt[24+q]^ip[16+q];
      memcpy(prev,ip+24,8); }
    for(; j<nblk; j++){ unsigned char c[8]; memcpy(c,in+8*j,8);
      unsigned char pt[8]; des_fp_store(des3_core(S,fwd,des_ip(c)),pt);
      for(int q=0;q<8;q++) out[8*j+q]=pt[q]^prev[q];
      memcpy(prev,c,8); }
  }
}

/* ============================ single DES ============================ */
/* Single DES (FIPS 46-3), 8-byte key. Cryptographically weak (56-bit key,
   64-bit block); for legacy interop / old container formats only. Reuses the
   shared DES machinery (schedule / IP / feistel / FP) -- one 16-round stage
   plus the pre-output swap, exactly one des3 stage. */
#if defined(__aarch64__) && defined(__APPLE__)
/* rktasm 16-round Feistel core (rktcrypto_des_asm.S); DES_SP is initialized by
   the des_schedule call that precedes every des1_core use. */
extern uint64_t des1_core_asm(const uint32_t *sp, const unsigned char *sk,
                              intptr_t stride, uint64_t state);
#endif
static uint64_t des1_core(const des_ks *ks, int encrypt, uint64_t b){
#if defined(__aarch64__) && defined(__APPLE__)
  return des1_core_asm(&DES_SP[0][0],
                       encrypt ? &ks->g[0][0] : &ks->g[15][0],
                       encrypt ? 8 : -8, b);
#else
  uint32_t L=(uint32_t)(b>>32), R=(uint32_t)b, nR; int r;
  for(r=0;r<16;r++){ int rr = encrypt ? r : 15-r; nR = L ^ des_feistel(R, ks->g[rr]); L=R; R=nR; }
  return ((uint64_t)R<<32)|L;   /* pre-output swap folded into the return */
#endif
}
void rktcrypto_des_ecb(const unsigned char key[8], const unsigned char *in,
                       unsigned char *out, intptr_t nblk, int encrypt){
  des_ks ks; intptr_t i;
  des_schedule(key,&ks);
  for(i=0;i<nblk;i++) des_fp_store(des1_core(&ks,encrypt,des_ip(in+8*i)),out+8*i);
}
void rktcrypto_des_cbc(const unsigned char key[8], const unsigned char iv[8],
                       const unsigned char *in, unsigned char *out, intptr_t nblk, int encrypt){
  des_ks ks; intptr_t i;
  des_schedule(key,&ks);
  if(encrypt){
    uint64_t prevpre=des_ip(iv);
    for(i=0;i<nblk;i++){ uint64_t pre=des1_core(&ks,1,des_ip(in+8*i)^prevpre);
      des_fp_store(pre,out+8*i); prevpre=pre; }
  } else {
    unsigned char prev[8]; memcpy(prev,iv,8);
    for(i=0;i<nblk;i++){ unsigned char c[8]; memcpy(c,in+8*i,8);
      unsigned char pt[8]; des_fp_store(des1_core(&ks,0,des_ip(c)),pt);
      { int q; for(q=0;q<8;q++) out[8*i+q]=(unsigned char)(pt[q]^prev[q]); }
      memcpy(prev,c,8); }
  }
}

/* =============================== DES-X =============================== */
/* DES-X: y = K3 ^ DES_K2(x ^ K1), key = K1(8) || K2(8) || K3(8) (24 bytes).
   Whitening lifts the effective key strength against brute force but the 64-bit
   block (Sweet32) remains; interop only. Reuses the single-DES core (which on
   aarch64/Apple is the rktasm kernel), so no new asm. */
void rktcrypto_desx_ecb(const unsigned char key[24], const unsigned char *in,
                        unsigned char *out, intptr_t nblk, int encrypt){
  des_ks ks; intptr_t i;
  const unsigned char *K1=key, *K2=key+8, *K3=key+16;
  const unsigned char *pre  = encrypt ? K1 : K3;   /* whitening XORed before DES */
  const unsigned char *post = encrypt ? K3 : K1;   /* whitening XORed after DES  */
  des_schedule(K2,&ks);
  for(i=0;i<nblk;i++){ unsigned char t[8]; int q;
    for(q=0;q<8;q++) t[q]=(unsigned char)(in[8*i+q]^pre[q]);
    des_fp_store(des1_core(&ks,encrypt,des_ip(t)),out+8*i);
    for(q=0;q<8;q++) out[8*i+q]=(unsigned char)(out[8*i+q]^post[q]); }
}

/* =============================== RC2 =============================== */
/* RC2 (RFC 2268), 8-byte blocks, key 1..128 bytes, effective key bits
   T1 in 1..1024. Weak (64-bit block, small key options); interop only
   (PKCS#12 / old S/MIME). MIX+MASH core is the rktasm kernel on aarch64/Apple. */
static const unsigned char RC2_PI[256]={
};

static void rc2_expand(const unsigned char *key, int T, int T1, uint16_t K[64]){
  unsigned char L[128]; int i;
  int T8=(T1+7)/8; unsigned char TM=(unsigned char)(0xff >> (8*T8 - T1));
  for(i=0;i<T;i++) L[i]=key[i];
  for(i=T;i<128;i++) L[i]=RC2_PI[(L[i-1]+L[i-T]) & 0xff];
  L[128-T8]=RC2_PI[L[128-T8] & TM];
  for(i=127-T8;i>=0;i--) L[i]=RC2_PI[L[i+1] ^ L[i+T8]];
  for(i=0;i<64;i++) K[i]=(uint16_t)(L[2*i] | (L[2*i+1]<<8));
}

#if defined(__aarch64__) && defined(__APPLE__)
extern void rc2_enc_core_asm(const uint16_t *K, const unsigned char in[8], unsigned char out[8]);
extern void rc2_dec_core_asm(const uint16_t *K, const unsigned char in[8], unsigned char out[8]);
#endif
#define RC2_ROL(x,n) ((uint16_t)(((x)<<(n))|((x)>>(16-(n)))))
#define RC2_ROR(x,n) ((uint16_t)(((x)>>(n))|((x)<<(16-(n)))))

static void rc2_enc_core(const uint16_t K[64], const unsigned char in[8], unsigned char out[8]){
#if defined(__aarch64__) && defined(__APPLE__)
  rc2_enc_core_asm(K,in,out);
#else
  uint16_t R[4]; int i,j=0; static const int s[4]={1,2,3,5};
  for(i=0;i<4;i++) R[i]=(uint16_t)(in[2*i] | (in[2*i+1]<<8));
  for(int rnd=0;rnd<16;rnd++){
    for(i=0;i<4;i++){ R[i]=(uint16_t)(R[i]+K[j]+(R[(i+3)&3]&R[(i+2)&3])+((~R[(i+3)&3])&R[(i+1)&3]));
      j++; R[i]=RC2_ROL(R[i],s[i]); }
    if(rnd==4||rnd==10){ for(i=0;i<4;i++) R[i]=(uint16_t)(R[i]+K[R[(i+3)&3]&63]); }
  }
  for(i=0;i<4;i++){ out[2*i]=(unsigned char)R[i]; out[2*i+1]=(unsigned char)(R[i]>>8); }
#endif
}
static void rc2_dec_core(const uint16_t K[64], const unsigned char in[8], unsigned char out[8]){
#if defined(__aarch64__) && defined(__APPLE__)
  rc2_dec_core_asm(K,in,out);
#else
  uint16_t R[4]; int i,j=63; static const int s[4]={1,2,3,5};
  for(i=0;i<4;i++) R[i]=(uint16_t)(in[2*i] | (in[2*i+1]<<8));
  for(int rnd=15;rnd>=0;rnd--){
    if(rnd==4||rnd==10){ for(i=3;i>=0;i--) R[i]=(uint16_t)(R[i]-K[R[(i+3)&3]&63]); }
    for(i=3;i>=0;i--){ R[i]=RC2_ROR(R[i],s[i]);
      R[i]=(uint16_t)(R[i]-K[j]-(R[(i+3)&3]&R[(i+2)&3])-((~R[(i+3)&3])&R[(i+1)&3])); j--; }
  }
  for(i=0;i<4;i++){ out[2*i]=(unsigned char)R[i]; out[2*i+1]=(unsigned char)(R[i]>>8); }
#endif
}

void rktcrypto_rc2_ecb(const unsigned char *key, intptr_t keylen, intptr_t effbits,
                       const unsigned char *in, unsigned char *out, intptr_t nblk, int encrypt){
  uint16_t K[64]; intptr_t i;
  rc2_expand(key,(int)keylen,(int)effbits,K);
  for(i=0;i<nblk;i++){ if(encrypt) rc2_enc_core(K,in+8*i,out+8*i); else rc2_dec_core(K,in+8*i,out+8*i); }
}
void rktcrypto_rc2_cbc(const unsigned char *key, intptr_t keylen, intptr_t effbits,
                       const unsigned char iv[8], const unsigned char *in, unsigned char *out,
                       intptr_t nblk, int encrypt){
  uint16_t K[64]; intptr_t i; unsigned char prev[8]; int q;
  rc2_expand(key,(int)keylen,(int)effbits,K); memcpy(prev,iv,8);
  if(encrypt){
    for(i=0;i<nblk;i++){ unsigned char t[8];
      for(q=0;q<8;q++) t[q]=(unsigned char)(in[8*i+q]^prev[q]);
      rc2_enc_core(K,t,out+8*i); memcpy(prev,out+8*i,8); }
  } else {
    for(i=0;i<nblk;i++){ unsigned char c[8],p[8]; memcpy(c,in+8*i,8);
      rc2_dec_core(K,c,p);
      for(q=0;q<8;q++) out[8*i+q]=(unsigned char)(p[q]^prev[q]);
      memcpy(prev,c,8); }
  }
}

/* ============================= Blowfish ============================= */
/* Blowfish (Schneier), 64-bit block, key 1..56 bytes; P/S from the hex digits
   of pi. Weak legacy cipher (64-bit block / Sweet32); interop only. 16-round
   Feistel core is the rktasm kernel on aarch64/Apple. */
static const uint32_t BF_P_INIT[18]={
0x243f6a88,0x85a308d3,0x13198a2e,0x03707344,0xa4093822,0x299f31d0,
0x082efa98,0xec4e6c89,0x452821e6,0x38d01377,0xbe5466cf,0x34e90c6c,
0xc0ac29b7,0xc97c50dd,0x3f84d5b5,0xb5470917,0x9216d5d9,0x8979fb1b};

static const uint32_t BF_S_INIT[4][256]={{
0xd1310ba6,0x98dfb5ac,0x2ffd72db,0xd01adfb7,0xb8e1afed,0x6a267e96,0xba7c9045,0xf12c7f99,
0x24a19947,0xb3916cf7,0x0801f2e2,0x858efc16,0x636920d8,0x71574e69,0xa458fea3,0xf4933d7e,
0x0d95748f,0x728eb658,0x718bcd58,0x82154aee,0x7b54a41d,0xc25a59b5,0x9c30d539,0x2af26013,
0xc5d1b023,0x286085f0,0xca417918,0xb8db38ef,0x8e79dcb0,0x603a180e,0x6c9e0e8b,0xb01e8a3e,
0xd71577c1,0xbd314b27,0x78af2fda,0x55605c60,0xe65525f3,0xaa55ab94,0x57489862,0x63e81440,
0x55ca396a,0x2aab10b6,0xb4cc5c34,0x1141e8ce,0xa15486af,0x7c72e993,0xb3ee1411,0x636fbc2a,
0x2ba9c55d,0x741831f6,0xce5c3e16,0x9b87931e,0xafd6ba33,0x6c24cf5c,0x7a325381,0x28958677,
0x3b8f4898,0x6b4bb9af,0xc4bfe81b,0x66282193,0x61d809cc,0xfb21a991,0x487cac60,0x5dec8032,
0xef845d5d,0xe98575b1,0xdc262302,0xeb651b88,0x23893e81,0xd396acc5,0x0f6d6ff3,0x83f44239,
0x2e0b4482,0xa4842004,0x69c8f04a,0x9e1f9b5e,0x21c66842,0xf6e96c9a,0x670c9c61,0xabd388f0,
0x6a51a0d2,0xd8542f68,0x960fa728,0xab5133a3,0x6eef0b6c,0x137a3be4,0xba3bf050,0x7efb2a98,
0xa1f1651d,0x39af0176,0x66ca593e,0x82430e88,0x8cee8619,0x456f9fb4,0x7d84a5c3,0x3b8b5ebe,
0xe06f75d8,0x85c12073,0x401a449f,0x56c16aa6,0x4ed3aa62,0x363f7706,0x1bfedf72,0x429b023d,
0x37d0d724,0xd00a1248,0xdb0fead3,0x49f1c09b,0x075372c9,0x80991b7b,0x25d479d8,0xf6e8def7,
0xe3fe501a,0xb6794c3b,0x976ce0bd,0x04c006ba,0xc1a94fb6,0x409f60c4,0x5e5c9ec2,0x196a2463,
0x68fb6faf,0x3e6c53b5,0x1339b2eb,0x3b52ec6f,0x6dfc511f,0x9b30952c,0xcc814544,0xaf5ebd09,
0xbee3d004,0xde334afd,0x660f2807,0x192e4bb3,0xc0cba857,0x45c8740f,0xd20b5f39,0xb9d3fbdb,
0x5579c0bd,0x1a60320a,0xd6a100c6,0x402c7279,0x679f25fe,0xfb1fa3cc,0x8ea5e9f8,0xdb3222f8,
0x3c7516df,0xfd616b15,0x2f501ec8,0xad0552ab,0x323db5fa,0xfd238760,0x53317b48,0x3e00df82,
0x9e5c57bb,0xca6f8ca0,0x1a87562e,0xdf1769db,0xd542a8f6,0x287effc3,0xac6732c6,0x8c4f5573,
0x695b27b0,0xbbca58c8,0xe1ffa35d,0xb8f011a0,0x10fa3d98,0xfd2183b8,0x4afcb56c,0x2dd1d35b,
0x9a53e479,0xb6f84565,0xd28e49bc,0x4bfb9790,0xe1ddf2da,0xa4cb7e33,0x62fb1341,0xcee4c6e8,
0xef20cada,0x36774c01,0xd07e9efe,0x2bf11fb4,0x95dbda4d,0xae909198,0xeaad8e71,0x6b93d5a0,
0xd08ed1d0,0xafc725e0,0x8e3c5b2f,0x8e7594b7,0x8ff6e2fb,0xf2122b64,0x8888b812,0x900df01c,
0x4fad5ea0,0x688fc31c,0xd1cff191,0xb3a8c1ad,0x2f2f2218,0xbe0e1777,0xea752dfe,0x8b021fa1,
0xe5a0cc0f,0xb56f74e8,0x18acf3d6,0xce89e299,0xb4a84fe0,0xfd13e0b7,0x7cc43b81,0xd2ada8d9,
0x165fa266,0x80957705,0x93cc7314,0x211a1477,0xe6ad2065,0x77b5fa86,0xc75442f5,0xfb9d35cf,
0xebcdaf0c,0x7b3e89a0,0xd6411bd3,0xae1e7e49,0x00250e2d,0x2071b35e,0x226800bb,0x57b8e0af,
0x2464369b,0xf009b91e,0x5563911d,0x59dfa6aa,0x78c14389,0xd95a537f,0x207d5ba2,0x02e5b9c5,
0x83260376,0x6295cfa9,0x11c81968,0x4e734a41,0xb3472dca,0x7b14a94a,0x1b510052,0x9a532915,
0xd60f573f,0xbc9bc6e4,0x2b60a476,0x81e67400,0x08ba6fb5,0x571be91f,0xf296ec6b,0x2a0dd915,
0xb6636521,0xe7b9f9b6,0xff34052e,0xc5855664,0x53b02d5d,0xa99f8fa1,0x08ba4799,0x6e85076a},{
0x4b7a70e9,0xb5b32944,0xdb75092e,0xc4192623,0xad6ea6b0,0x49a7df7d,0x9cee60b8,0x8fedb266,
0xecaa8c71,0x699a17ff,0x5664526c,0xc2b19ee1,0x193602a5,0x75094c29,0xa0591340,0xe4183a3e,
0x3f54989a,0x5b429d65,0x6b8fe4d6,0x99f73fd6,0xa1d29c07,0xefe830f5,0x4d2d38e6,0xf0255dc1,
0x4cdd2086,0x8470eb26,0x6382e9c6,0x021ecc5e,0x09686b3f,0x3ebaefc9,0x3c971814,0x6b6a70a1,
0x687f3584,0x52a0e286,0xb79c5305,0xaa500737,0x3e07841c,0x7fdeae5c,0x8e7d44ec,0x5716f2b8,
0xb03ada37,0xf0500c0d,0xf01c1f04,0x0200b3ff,0xae0cf51a,0x3cb574b2,0x25837a58,0xdc0921bd,
0xd19113f9,0x7ca92ff6,0x94324773,0x22f54701,0x3ae5e581,0x37c2dadc,0xc8b57634,0x9af3dda7,
0xa9446146,0x0fd0030e,0xecc8c73e,0xa4751e41,0xe238cd99,0x3bea0e2f,0x3280bba1,0x183eb331,
0x4e548b38,0x4f6db908,0x6f420d03,0xf60a04bf,0x2cb81290,0x24977c79,0x5679b072,0xbcaf89af,
0xde9a771f,0xd9930810,0xb38bae12,0xdccf3f2e,0x5512721f,0x2e6b7124,0x501adde6,0x9f84cd87,
0x7a584718,0x7408da17,0xbc9f9abc,0xe94b7d8c,0xec7aec3a,0xdb851dfa,0x63094366,0xc464c3d2,
0xef1c1847,0x3215d908,0xdd433b37,0x24c2ba16,0x12a14d43,0x2a65c451,0x50940002,0x133ae4dd,
0x71dff89e,0x10314e55,0x81ac77d6,0x5f11199b,0x043556f1,0xd7a3c76b,0x3c11183b,0x5924a509,
0xf28fe6ed,0x97f1fbfa,0x9ebabf2c,0x1e153c6e,0x86e34570,0xeae96fb1,0x860e5e0a,0x5a3e2ab3,
0x771fe71c,0x4e3d06fa,0x2965dcb9,0x99e71d0f,0x803e89d6,0x5266c825,0x2e4cc978,0x9c10b36a,
0xc6150eba,0x94e2ea78,0xa5fc3c53,0x1e0a2df4,0xf2f74ea7,0x361d2b3d,0x1939260f,0x19c27960,
0x5223a708,0xf71312b6,0xebadfe6e,0xeac31f66,0xe3bc4595,0xa67bc883,0xb17f37d1,0x018cff28,
0xc332ddef,0xbe6c5aa5,0x65582185,0x68ab9802,0xeecea50f,0xdb2f953b,0x2aef7dad,0x5b6e2f84,
0x1521b628,0x29076170,0xecdd4775,0x619f1510,0x13cca830,0xeb61bd96,0x0334fe1e,0xaa0363cf,
0xb5735c90,0x4c70a239,0xd59e9e0b,0xcbaade14,0xeecc86bc,0x60622ca7,0x9cab5cab,0xb2f3846e,
0x648b1eaf,0x19bdf0ca,0xa02369b9,0x655abb50,0x40685a32,0x3c2ab4b3,0x319ee9d5,0xc021b8f7,
0x9b540b19,0x875fa099,0x95f7997e,0x623d7da8,0xf837889a,0x97e32d77,0x11ed935f,0x16681281,
0x0e358829,0xc7e61fd6,0x96dedfa1,0x7858ba99,0x57f584a5,0x1b227263,0x9b83c3ff,0x1ac24696,
0xcdb30aeb,0x532e3054,0x8fd948e4,0x6dbc3128,0x58ebf2ef,0x34c6ffea,0xfe28ed61,0xee7c3c73,
0x5d4a14d9,0xe864b7e3,0x42105d14,0x203e13e0,0x45eee2b6,0xa3aaabea,0xdb6c4f15,0xfacb4fd0,
0xc742f442,0xef6abbb5,0x654f3b1d,0x41cd2105,0xd81e799e,0x86854dc7,0xe44b476a,0x3d816250,
0xcf62a1f2,0x5b8d2646,0xfc8883a0,0xc1c7b6a3,0x7f1524c3,0x69cb7492,0x47848a0b,0x5692b285,
0x095bbf00,0xad19489d,0x1462b174,0x23820e00,0x58428d2a,0x0c55f5ea,0x1dadf43e,0x233f7061,
0x3372f092,0x8d937e41,0xd65fecf1,0x6c223bdb,0x7cde3759,0xcbee7460,0x4085f2a7,0xce77326e,
0xa6078084,0x19f8509e,0xe8efd855,0x61d99735,0xa969a7aa,0xc50c06c2,0x5a04abfc,0x800bcadc,
0x9e447a2e,0xc3453484,0xfdd56705,0x0e1e9ec9,0xdb73dbd3,0x105588cd,0x675fda79,0xe3674340,
0xc5c43465,0x713e38d8,0x3d28f89e,0xf16dff20,0x153e21e7,0x8fb03d4a,0xe6e39f2b,0xdb83adf7},{
0xe93d5a68,0x948140f7,0xf64c261c,0x94692934,0x411520f7,0x7602d4f7,0xbcf46b2e,0xd4a20068,
0xd4082471,0x3320f46a,0x43b7d4b7,0x500061af,0x1e39f62e,0x97244546,0x14214f74,0xbf8b8840,
0x4d95fc1d,0x96b591af,0x70f4ddd3,0x66a02f45,0xbfbc09ec,0x03bd9785,0x7fac6dd0,0x31cb8504,
0x96eb27b3,0x55fd3941,0xda2547e6,0xabca0a9a,0x28507825,0x530429f4,0x0a2c86da,0xe9b66dfb,
0x68dc1462,0xd7486900,0x680ec0a4,0x27a18dee,0x4f3ffea2,0xe887ad8c,0xb58ce006,0x7af4d6b6,
0xaace1e7c,0xd3375fec,0xce78a399,0x406b2a42,0x20fe9e35,0xd9f385b9,0xee39d7ab,0x3b124e8b,
0x1dc9faf7,0x4b6d1856,0x26a36631,0xeae397b2,0x3a6efa74,0xdd5b4332,0x6841e7f7,0xca7820fb,
0xfb0af54e,0xd8feb397,0x454056ac,0xba489527,0x55533a3a,0x20838d87,0xfe6ba9b7,0xd096954b,
0x55a867bc,0xa1159a58,0xcca92963,0x99e1db33,0xa62a4a56,0x3f3125f9,0x5ef47e1c,0x9029317c,
0xfdf8e802,0x04272f70,0x80bb155c,0x05282ce3,0x95c11548,0xe4c66d22,0x48c1133f,0xc70f86dc,
0x07f9c9ee,0x41041f0f,0x404779a4,0x5d886e17,0x325f51eb,0xd59bc0d1,0xf2bcc18f,0x41113564,
0x257b7834,0x602a9c60,0xdff8e8a3,0x1f636c1b,0x0e12b4c2,0x02e1329e,0xaf664fd1,0xcad18115,
0x6b2395e0,0x333e92e1,0x3b240b62,0xeebeb922,0x85b2a20e,0xe6ba0d99,0xde720c8c,0x2da2f728,
0xd0127845,0x95b794fd,0x647d0862,0xe7ccf5f0,0x5449a36f,0x877d48fa,0xc39dfd27,0xf33e8d1e,
0x0a476341,0x992eff74,0x3a6f6eab,0xf4f8fd37,0xa812dc60,0xa1ebddf8,0x991be14c,0xdb6e6b0d,
0xc67b5510,0x6d672c37,0x2765d43b,0xdcd0e804,0xf1290dc7,0xcc00ffa3,0xb5390f92,0x690fed0b,
0x667b9ffb,0xcedb7d9c,0xa091cf0b,0xd9155ea3,0xbb132f88,0x515bad24,0x7b9479bf,0x763bd6eb,
0x37392eb3,0xcc115979,0x8026e297,0xf42e312d,0x6842ada7,0xc66a2b3b,0x12754ccc,0x782ef11c,
0x6a124237,0xb79251e7,0x06a1bbe6,0x4bfb6350,0x1a6b1018,0x11caedfa,0x3d25bdd8,0xe2e1c3c9,
0x44421659,0x0a121386,0xd90cec6e,0xd5abea2a,0x64af674e,0xda86a85f,0xbebfe988,0x64e4c3fe,
0x9dbc8057,0xf0f7c086,0x60787bf8,0x6003604d,0xd1fd8346,0xf6381fb0,0x7745ae04,0xd736fccc,
0x83426b33,0xf01eab71,0xb0804187,0x3c005e5f,0x77a057be,0xbde8ae24,0x55464299,0xbf582e61,
0x4e58f48f,0xf2ddfda2,0xf474ef38,0x8789bdc2,0x5366f9c3,0xc8b38e74,0xb475f255,0x46fcd9b9,
0x7aeb2661,0x8b1ddf84,0x846a0e79,0x915f95e2,0x466e598e,0x20b45770,0x8cd55591,0xc902de4c,
0xb90bace1,0xbb8205d0,0x11a86248,0x7574a99e,0xb77f19b6,0xe0a9dc09,0x662d09a1,0xc4324633,
0xe85a1f02,0x09f0be8c,0x4a99a025,0x1d6efe10,0x1ab93d1d,0x0ba5a4df,0xa186f20f,0x2868f169,
0xdcb7da83,0x573906fe,0xa1e2ce9b,0x4fcd7f52,0x50115e01,0xa70683fa,0xa002b5c4,0x0de6d027,
0x9af88c27,0x773f8641,0xc3604c06,0x61a806b5,0xf0177a28,0xc0f586e0,0x006058aa,0x30dc7d62,
0x11e69ed7,0x2338ea63,0x53c2dd94,0xc2c21634,0xbbcbee56,0x90bcb6de,0xebfc7da1,0xce591d76,
0x6f05e409,0x4b7c0188,0x39720a3d,0x7c927c24,0x86e3725f,0x724d9db9,0x1ac15bb4,0xd39eb8fc,
0xed545578,0x08fca5b5,0xd83d7cd3,0x4dad0fc4,0x1e50ef5e,0xb161e6f8,0xa28514d9,0x6c51133c,
0x6fd5c7e7,0x56e14ec4,0x362abfce,0xddc6c837,0xd79a3234,0x92638212,0x670efa8e,0x406000e0},{
0x3a39ce37,0xd3faf5cf,0xabc27737,0x5ac52d1b,0x5cb0679e,0x4fa33742,0xd3822740,0x99bc9bbe,
0xd5118e9d,0xbf0f7315,0xd62d1c7e,0xc700c47b,0xb78c1b6b,0x21a19045,0xb26eb1be,0x6a366eb4,
0x5748ab2f,0xbc946e79,0xc6a376d2,0x6549c2c8,0x530ff8ee,0x468dde7d,0xd5730a1d,0x4cd04dc6,
0x2939bbdb,0xa9ba4650,0xac9526e8,0xbe5ee304,0xa1fad5f0,0x6a2d519a,0x63ef8ce2,0x9a86ee22,
0xc089c2b8,0x43242ef6,0xa51e03aa,0x9cf2d0a4,0x83c061ba,0x9be96a4d,0x8fe51550,0xba645bd6,
0x2826a2f9,0xa73a3ae1,0x4ba99586,0xef5562e9,0xc72fefd3,0xf752f7da,0x3f046f69,0x77fa0a59,
0x80e4a915,0x87b08601,0x9b09e6ad,0x3b3ee593,0xe990fd5a,0x9e34d797,0x2cf0b7d9,0x022b8b51,
0x96d5ac3a,0x017da67d,0xd1cf3ed6,0x7c7d2d28,0x1f9f25cf,0xadf2b89b,0x5ad6b472,0x5a88f54c,
0xe029ac71,0xe019a5e6,0x47b0acfd,0xed93fa9b,0xe8d3c48d,0x283b57cc,0xf8d56629,0x79132e28,
0x785f0191,0xed756055,0xf7960e44,0xe3d35e8c,0x15056dd4,0x88f46dba,0x03a16125,0x0564f0bd,
0xc3eb9e15,0x3c9057a2,0x97271aec,0xa93a072a,0x1b3f6d9b,0x1e6321f5,0xf59c66fb,0x26dcf319,
0x7533d928,0xb155fdf5,0x03563482,0x8aba3cbb,0x28517711,0xc20ad9f8,0xabcc5167,0xccad925f,
0x4de81751,0x3830dc8e,0x379d5862,0x9320f991,0xea7a90c2,0xfb3e7bce,0x5121ce64,0x774fbe32,
0xa8b6e37e,0xc3293d46,0x48de5369,0x6413e680,0xa2ae0810,0xdd6db224,0x69852dfd,0x09072166,
0xb39a460a,0x6445c0dd,0x586cdecf,0x1c20c8ae,0x5bbef7dd,0x1b588d40,0xccd2017f,0x6bb4e3bb,
0xdda26a7e,0x3a59ff45,0x3e350a44,0xbcb4cdd5,0x72eacea8,0xfa6484bb,0x8d6612ae,0xbf3c6f47,
0xd29be463,0x542f5d9e,0xaec2771b,0xf64e6370,0x740e0d8d,0xe75b1357,0xf8721671,0xaf537d5d,
0x4040cb08,0x4eb4e2cc,0x34d2466a,0x0115af84,0xe1b00428,0x95983a1d,0x06b89fb4,0xce6ea048,
0x6f3f3b82,0x3520ab82,0x011a1d4b,0x277227f8,0x611560b1,0xe7933fdc,0xbb3a792b,0x344525bd,
0xa08839e1,0x51ce794b,0x2f32c9b7,0xa01fbac9,0xe01cc87e,0xbcc7d1f6,0xcf0111c3,0xa1e8aac7,
0x1a908749,0xd44fbd9a,0xd0dadecb,0xd50ada38,0x0339c32a,0xc6913667,0x8df9317c,0xe0b12b4f,
0xf79e59b7,0x43f5bb3a,0xf2d519ff,0x27d9459c,0xbf97222c,0x15e6fc2a,0x0f91fc71,0x9b941525,
0xfae59361,0xceb69ceb,0xc2a86459,0x12baa8d1,0xb6c1075e,0xe3056a0c,0x10d25065,0xcb03a442,
0xe0ec6e0e,0x1698db3b,0x4c98a0be,0x3278e964,0x9f1f9532,0xe0d392df,0xd3a0342b,0x8971f21e,
0x1b0a7441,0x4ba3348c,0xc5be7120,0xc37632d8,0xdf359f8d,0x9b992f2e,0xe60b6f47,0x0fe3f11d,
0xe54cda54,0x1edad891,0xce6279cf,0xcd3e7e6f,0x1618b166,0xfd2c1d05,0x848fd2c5,0xf6fb2299,
0xf523f357,0xa6327623,0x93a83531,0x56cccd02,0xacf08162,0x5a75ebb5,0x6e163697,0x88d273cc,
0xde966292,0x81b949d0,0x4c50901b,0x71c65614,0xe6c6c7bd,0x327a140a,0x45e1d006,0xc3f27b9a,
0xc9aa53fd,0x62a80f00,0xbb25bfe2,0x35bdd2f6,0x71126905,0xb2040222,0xb6cbcf7c,0xcd769c2b,
0x53113ec0,0x1640e3d3,0x38abbd60,0x2547adf0,0xba38209c,0xf746ce76,0x77afa1c5,0x20756060,
0x85cbfe4e,0x8ae88dd8,0x7aaaf9b0,0x4cf9aa7e,0x1948c25c,0x02fb8a8c,0x01c36ae4,0xd6ebe1f9,
0x90d4f869,0xa65cdea0,0x3f09252d,0xc208e69f,0xb74e6132,0xce77e25b,0x578fdfe3,0x3ac372e6}};

typedef struct { uint32_t P[18], S[4][256]; } bf_ctx;
static uint32_t bf_F(const bf_ctx *b, uint32_t x) __attribute__((unused));
static uint32_t bf_F(const bf_ctx *b, uint32_t x){
  return ((b->S[0][x>>24] + b->S[1][(x>>16)&0xff]) ^ b->S[2][(x>>8)&0xff]) + b->S[3][x&0xff];
}
#if defined(__aarch64__) && defined(__APPLE__)
extern void bf_core_asm(const uint32_t *P, const uint32_t *S, uint32_t io[2], intptr_t stride);
#endif
static void bf_block(const bf_ctx *b, uint32_t *xl, uint32_t *xr, int encrypt){
#if defined(__aarch64__) && defined(__APPLE__)
  uint32_t io[2]={*xl,*xr};
  bf_core_asm(encrypt ? &b->P[0] : &b->P[17], &b->S[0][0], io, encrypt ? 4 : -4);
  *xl=io[0]; *xr=io[1];
#else
  uint32_t L=*xl,R=*xr,t; int i;
  if(encrypt){ for(i=0;i<16;i++){ L^=b->P[i]; R^=bf_F(b,L); t=L;L=R;R=t; } t=L;L=R;R=t; R^=b->P[16]; L^=b->P[17]; }
  else       { for(i=17;i>1;i--){ L^=b->P[i]; R^=bf_F(b,L); t=L;L=R;R=t; } t=L;L=R;R=t; R^=b->P[1];  L^=b->P[0];  }
  *xl=L; *xr=R;
#endif
}
static void bf_expand(bf_ctx *b, const unsigned char *key, int klen){
  int i,j=0,s; uint32_t L=0,R=0;
  memcpy(b->P,BF_P_INIT,sizeof BF_P_INIT); memcpy(b->S,BF_S_INIT,sizeof BF_S_INIT);
  for(i=0;i<18;i++){ uint32_t d=0; for(j=0;j<4;j++){ d=(d<<8)|key[(i*4+j)%klen]; } b->P[i]^=d; }
  for(i=0;i<18;i+=2){ bf_block(b,&L,&R,1); b->P[i]=L; b->P[i+1]=R; }
  for(s=0;s<4;s++) for(i=0;i<256;i+=2){ bf_block(b,&L,&R,1); b->S[s][i]=L; b->S[s][i+1]=R; }
}
static void bf_load(const unsigned char *p, uint32_t *L, uint32_t *R){
  *L=((uint32_t)p[0]<<24)|((uint32_t)p[1]<<16)|((uint32_t)p[2]<<8)|p[3];
  *R=((uint32_t)p[4]<<24)|((uint32_t)p[5]<<16)|((uint32_t)p[6]<<8)|p[7];
}
static void bf_store(unsigned char *o, uint32_t L, uint32_t R){
  o[0]=(unsigned char)(L>>24);o[1]=(unsigned char)(L>>16);o[2]=(unsigned char)(L>>8);o[3]=(unsigned char)L;
  o[4]=(unsigned char)(R>>24);o[5]=(unsigned char)(R>>16);o[6]=(unsigned char)(R>>8);o[7]=(unsigned char)R;
}
void rktcrypto_blowfish_ecb(const unsigned char *key, intptr_t klen, const unsigned char *in,
                            unsigned char *out, intptr_t nblk, int encrypt){
  bf_ctx b; intptr_t n; uint32_t L,R; bf_expand(&b,key,(int)klen);
  for(n=0;n<nblk;n++){ bf_load(in+8*n,&L,&R); bf_block(&b,&L,&R,encrypt); bf_store(out+8*n,L,R); }
}
void rktcrypto_blowfish_cbc(const unsigned char *key, intptr_t klen, const unsigned char iv[8],
                            const unsigned char *in, unsigned char *out, intptr_t nblk, int encrypt){
  bf_ctx b; intptr_t n; uint32_t L,R; unsigned char prev[8]; int q; bf_expand(&b,key,(int)klen); memcpy(prev,iv,8);
  if(encrypt){ for(n=0;n<nblk;n++){ unsigned char t[8]; for(q=0;q<8;q++)t[q]=(unsigned char)(in[8*n+q]^prev[q]);
      bf_load(t,&L,&R); bf_block(&b,&L,&R,1); bf_store(out+8*n,L,R); memcpy(prev,out+8*n,8); } }
  else { for(n=0;n<nblk;n++){ unsigned char c[8],p[8]; memcpy(c,in+8*n,8); bf_load(c,&L,&R); bf_block(&b,&L,&R,0);
      bf_store(p,L,R); for(q=0;q<8;q++)out[8*n+q]=(unsigned char)(p[q]^prev[q]); memcpy(prev,c,8); } }
}


/* =============================== IDEA =============================== */
/* IDEA (Lai-Massey), 64-bit block, 128-bit key; mul is mod (2^16+1) with 0
   representing 2^16. Patent expired; weak by modern standards (64-bit block).
   8-round core is the rktasm kernel on aarch64/Apple; no tables. */
static uint16_t idea_mul(uint16_t a, uint16_t b){
  uint32_t p=(uint32_t)a*b; if(p==0) return (uint16_t)(1-a-b);
  int32_t r=(int32_t)(p&0xffff)-(int32_t)(p>>16); if(r<=0) r+=0x10001; return (uint16_t)r;
}
static uint16_t idea_mulinv(uint16_t a){
  if(a<=1) return a; uint32_t b=1,base=a,e=0xffff;
  while(e){ if(e&1) b=idea_mul((uint16_t)b,(uint16_t)base); base=idea_mul((uint16_t)base,(uint16_t)base); e>>=1; }
  return (uint16_t)b;
}
static uint16_t idea_addinv(uint16_t a){ return (uint16_t)(0x10000-a); }
static void idea_enc_key(const unsigned char *key, uint16_t Z[52]){
  uint64_t hi=0,lo=0; int i,zi=0;
  for(i=0;i<8;i++) hi=(hi<<8)|key[i]; for(i=8;i<16;i++) lo=(lo<<8)|key[i];
  while(zi<52){ uint16_t w[8]={(uint16_t)(hi>>48),(uint16_t)(hi>>32),(uint16_t)(hi>>16),(uint16_t)hi,
      (uint16_t)(lo>>48),(uint16_t)(lo>>32),(uint16_t)(lo>>16),(uint16_t)lo};
    for(i=0;i<8&&zi<52;i++) Z[zi++]=w[i];
    uint64_t nh=(hi<<25)|(lo>>39),nl=(lo<<25)|(hi>>39); hi=nh; lo=nl; }
}
static void idea_dec_key(const uint16_t Z[52], uint16_t D[52]){
  int p=0,i; uint16_t t1,t2,t3;
  t1=idea_mulinv(Z[48]); t2=idea_addinv(Z[49]); t3=idea_addinv(Z[50]);
  D[p++]=t1; D[p++]=t2; D[p++]=t3; D[p++]=idea_mulinv(Z[51]); D[p++]=Z[46]; D[p++]=Z[47];
  for(i=1;i<8;i++){ int k=48-6*i;
    D[p++]=idea_mulinv(Z[k]); D[p++]=idea_addinv(Z[k+2]); D[p++]=idea_addinv(Z[k+1]);
    D[p++]=idea_mulinv(Z[k+3]); D[p++]=Z[k-2]; D[p++]=Z[k-1]; }
  D[p++]=idea_mulinv(Z[0]); D[p++]=idea_addinv(Z[1]); D[p++]=idea_addinv(Z[2]); D[p++]=idea_mulinv(Z[3]);
}
#if defined(__aarch64__) && defined(__APPLE__)
extern void idea_core_asm(const uint16_t *Z, const unsigned char in[8], unsigned char out[8]);
#endif
static void idea_core(const uint16_t Z[52], const unsigned char in[8], unsigned char out[8]){
#if defined(__aarch64__) && defined(__APPLE__)
  idea_core_asm(Z,in,out);
#else
  uint16_t X1=(in[0]<<8)|in[1],X2=(in[2]<<8)|in[3],X3=(in[4]<<8)|in[5],X4=(in[6]<<8)|in[7];
  const uint16_t *z=Z; int r;
  for(r=0;r<8;r++){ X1=idea_mul(X1,z[0]); X2=(uint16_t)(X2+z[1]); X3=(uint16_t)(X3+z[2]); X4=idea_mul(X4,z[3]);
    uint16_t t0=idea_mul((uint16_t)(X1^X3),z[4]);
    uint16_t t1=idea_mul((uint16_t)((uint16_t)(X2^X4)+t0),z[5]);
    t0=(uint16_t)(t0+t1); X1^=t1; X4^=t0;
    uint16_t x=(uint16_t)(X2^t0); X2=(uint16_t)(X3^t1); X3=x; z+=6; }
  uint16_t Y1=idea_mul(X1,z[0]),Y2=(uint16_t)(X3+z[1]),Y3=(uint16_t)(X2+z[2]),Y4=idea_mul(X4,z[3]);
  out[0]=(unsigned char)(Y1>>8);out[1]=(unsigned char)Y1;out[2]=(unsigned char)(Y2>>8);out[3]=(unsigned char)Y2;
  out[4]=(unsigned char)(Y3>>8);out[5]=(unsigned char)Y3;out[6]=(unsigned char)(Y4>>8);out[7]=(unsigned char)Y4;
#endif
}
void rktcrypto_idea_ecb(const unsigned char key[16], const unsigned char *in, unsigned char *out,
                        intptr_t nblk, int encrypt){
  uint16_t Z[52],D[52]; const uint16_t *sk; intptr_t n;
  idea_enc_key(key,Z); if(encrypt) sk=Z; else { idea_dec_key(Z,D); sk=D; }
  for(n=0;n<nblk;n++) idea_core(sk,in+8*n,out+8*n);
}
void rktcrypto_idea_cbc(const unsigned char key[16], const unsigned char iv[8], const unsigned char *in,
                        unsigned char *out, intptr_t nblk, int encrypt){
  uint16_t Z[52],D[52]; const uint16_t *sk; intptr_t n; unsigned char prev[8]; int q;
  idea_enc_key(key,Z); if(encrypt) sk=Z; else { idea_dec_key(Z,D); sk=D; } memcpy(prev,iv,8);
  if(encrypt){ for(n=0;n<nblk;n++){ unsigned char t[8]; for(q=0;q<8;q++)t[q]=(unsigned char)(in[8*n+q]^prev[q]);
      idea_core(sk,t,out+8*n); memcpy(prev,out+8*n,8); } }
  else { for(n=0;n<nblk;n++){ unsigned char c[8],p[8]; memcpy(c,in+8*n,8); idea_core(sk,c,p);
      for(q=0;q<8;q++)out[8*n+q]=(unsigned char)(p[q]^prev[q]); memcpy(prev,c,8); } }
}


/* ============================= CAST-128 ============================= */
/* CAST-128 / CAST5 (RFC 2144), 64-bit block, key up to 128 bits. S-boxes are
   the standard RFC 2144 tables. Weak legacy cipher (64-bit block); interop only.
   16-round Feistel core (3 round-function types) is the rktasm kernel. */
static const uint32_t CAST_S[8][256]={
{
0x30fb40d4,0x9fa0ff0b,0x6beccd2f,0x3f258c7a,0x1e213f2f,0x9c004dd3,0x6003e540,0xcf9fc949,
0xbfd4af27,0x88bbbdb5,0xe2034090,0x98d09675,0x6e63a0e0,0x15c361d2,0xc2e7661d,0x22d4ff8e,
0x28683b6f,0xc07fd059,0xff2379c8,0x775f50e2,0x43c340d3,0xdf2f8656,0x887ca41a,0xa2d2bd2d,
0xa1c9e0d6,0x346c4819,0x61b76d87,0x22540f2f,0x2abe32e1,0xaa54166b,0x22568e3a,0xa2d341d0,
0x66db40c8,0xa784392f,0x004dff2f,0x2db9d2de,0x97943fac,0x4a97c1d8,0x527644b7,0xb5f437a7,
0xb82cbaef,0xd751d159,0x6ff7f0ed,0x5a097a1f,0x827b68d0,0x90ecf52e,0x22b0c054,0xbc8e5935,
0x4b6d2f7f,0x50bb64a2,0xd2664910,0xbee5812d,0xb7332290,0xe93b159f,0xb48ee411,0x4bff345d,
0xfd45c240,0xad31973f,0xc4f6d02e,0x55fc8165,0xd5b1caad,0xa1ac2dae,0xa2d4b76d,0xc19b0c50,
0x882240f2,0x0c6e4f38,0xa4e4bfd7,0x4f5ba272,0x564c1d2f,0xc59c5319,0xb949e354,0xb04669fe,
0xb1b6ab8a,0xc71358dd,0x6385c545,0x110f935d,0x57538ad5,0x6a390493,0xe63d37e0,0x2a54f6b3,
0x3a787d5f,0x6276a0b5,0x19a6fcdf,0x7a42206a,0x29f9d4d5,0xf61b1891,0xbb72275e,0xaa508167,
0x38901091,0xc6b505eb,0x84c7cb8c,0x2ad75a0f,0x874a1427,0xa2d1936b,0x2ad286af,0xaa56d291,
0xd7894360,0x425c750d,0x93b39e26,0x187184c9,0x6c00b32d,0x73e2bb14,0xa0bebc3c,0x54623779,
0x64459eab,0x3f328b82,0x7718cf82,0x59a2cea6,0x04ee002e,0x89fe78e6,0x3fab0950,0x325ff6c2,
0x81383f05,0x6963c5c8,0x76cb5ad6,0xd49974c9,0xca180dcf,0x380782d5,0xc7fa5cf6,0x8ac31511,
0x35e79e13,0x47da91d0,0xf40f9086,0xa7e2419e,0x31366241,0x051ef495,0xaa573b04,0x4a805d8d,
0x548300d0,0x00322a3c,0xbf64cddf,0xba57a68e,0x75c6372b,0x50afd341,0xa7c13275,0x915a0bf5,
0x6b54bfab,0x2b0b1426,0xab4cc9d7,0x449ccd82,0xf7fbf265,0xab85c5f3,0x1b55db94,0xaad4e324,
0xcfa4bd3f,0x2deaa3e2,0x9e204d02,0xc8bd25ac,0xeadf55b3,0xd5bd9e98,0xe31231b2,0x2ad5ad6c,
0x954329de,0xadbe4528,0xd8710f69,0xaa51c90f,0xaa786bf6,0x22513f1e,0xaa51a79b,0x2ad344cc,
0x7b5a41f0,0xd37cfbad,0x1b069505,0x41ece491,0xb4c332e6,0x032268d4,0xc9600acc,0xce387e6d,
0xbf6bb16c,0x6a70fb78,0x0d03d9c9,0xd4df39de,0xe01063da,0x4736f464,0x5ad328d8,0xb347cc96,
0x75bb0fc3,0x98511bfb,0x4ffbcc35,0xb58bcf6a,0xe11f0abc,0xbfc5fe4a,0xa70aec10,0xac39570a,
0x3f04442f,0x6188b153,0xe0397a2e,0x5727cb79,0x9ceb418f,0x1cacd68d,0x2ad37c96,0x0175cb9d,
0xc69dff09,0xc75b65f0,0xd9db40d8,0xec0e7779,0x4744ead4,0xb11c3274,0xdd24cb9e,0x7e1c54bd,
0xf01144f9,0xd2240eb1,0x9675b3fd,0xa3ac3755,0xd47c27af,0x51c85f4d,0x56907596,0xa5bb15e6,
0x580304f0,0xca042cf1,0x011a37ea,0x8dbfaadb,0x35ba3e4a,0x3526ffa0,0xc37b4d09,0xbc306ed9,
0x98a52666,0x5648f725,0xff5e569d,0x0ced63d0,0x7c63b2cf,0x700b45e1,0xd5ea50f1,0x85a92872,
0xaf1fbda7,0xd4234870,0xa7870bf3,0x2d3b4d79,0x42e04198,0x0cd0ede7,0x26470db8,0xf881814c,
0x474d6ad7,0x7c0c5e5c,0xd1231959,0x381b7298,0xf5d2f4db,0xab838653,0x6e2f1e23,0x83719c9e,
0xbd91e046,0x9a56456e,0xdc39200c,0x20c8c571,0x962bda1c,0xe1e696ff,0xb141ab08,0x7cca89b9,
0x1a69e783,0x02cc4843,0xa2f7c579,0x429ef47d,0x427b169c,0x5ac9f049,0xdd8f0f00,0x5c8165bf
},
{
0x1f201094,0xef0ba75b,0x69e3cf7e,0x393f4380,0xfe61cf7a,0xeec5207a,0x55889c94,0x72fc0651,
0xada7ef79,0x4e1d7235,0xd55a63ce,0xde0436ba,0x99c430ef,0x5f0c0794,0x18dcdb7d,0xa1d6eff3,
0xa0b52f7b,0x59e83605,0xee15b094,0xe9ffd909,0xdc440086,0xef944459,0xba83ccb3,0xe0c3cdfb,
0xd1da4181,0x3b092ab1,0xf997f1c1,0xa5e6cf7b,0x01420ddb,0xe4e7ef5b,0x25a1ff41,0xe180f806,
0x1fc41080,0x179bee7a,0xd37ac6a9,0xfe5830a4,0x98de8b7f,0x77e83f4e,0x79929269,0x24fa9f7b,
0xe113c85b,0xacc40083,0xd7503525,0xf7ea615f,0x62143154,0x0d554b63,0x5d681121,0xc866c359,
0x3d63cf73,0xcee234c0,0xd4d87e87,0x5c672b21,0x071f6181,0x39f7627f,0x361e3084,0xe4eb573b,
0x602f64a4,0xd63acd9c,0x1bbc4635,0x9e81032d,0x2701f50c,0x99847ab4,0xa0e3df79,0xba6cf38c,
0x10843094,0x2537a95e,0xf46f6ffe,0xa1ff3b1f,0x208cfb6a,0x8f458c74,0xd9e0a227,0x4ec73a34,
0xfc884f69,0x3e4de8df,0xef0e0088,0x3559648d,0x8a45388c,0x1d804366,0x721d9bfd,0xa58684bb,
0xe8256333,0x844e8212,0x128d8098,0xfed33fb4,0xce280ae1,0x27e19ba5,0xd5a6c252,0xe49754bd,
0xc5d655dd,0xeb667064,0x77840b4d,0xa1b6a801,0x84db26a9,0xe0b56714,0x21f043b7,0xe5d05860,
0x54f03084,0x066ff472,0xa31aa153,0xdadc4755,0xb5625dbf,0x68561be6,0x83ca6b94,0x2d6ed23b,
0xeccf01db,0xa6d3d0ba,0xb6803d5c,0xaf77a709,0x33b4a34c,0x397bc8d6,0x5ee22b95,0x5f0e5304,
0x81ed6f61,0x20e74364,0xb45e1378,0xde18639b,0x881ca122,0xb96726d1,0x8049a7e8,0x22b7da7b,
0x5e552d25,0x5272d237,0x79d2951c,0xc60d894c,0x488cb402,0x1ba4fe5b,0xa4b09f6b,0x1ca815cf,
0xa20c3005,0x8871df63,0xb9de2fcb,0x0cc6c9e9,0x0beeff53,0xe3214517,0xb4542835,0x9f63293c,
0xee41e729,0x6e1d2d7c,0x50045286,0x1e6685f3,0xf33401c6,0x30a22c95,0x31a70850,0x60930f13,
0x73f98417,0xa1269859,0xec645c44,0x52c877a9,0xcdff33a6,0xa02b1741,0x7cbad9a2,0x2180036f,
0x50d99c08,0xcb3f4861,0xc26bd765,0x64a3f6ab,0x80342676,0x25a75e7b,0xe4e6d1fc,0x20c710e6,
0xcdf0b680,0x17844d3b,0x31eef84d,0x7e0824e4,0x2ccb49eb,0x846a3bae,0x8ff77888,0xee5d60f6,
0x7af75673,0x2fdd5cdb,0xa11631c1,0x30f66f43,0xb3faec54,0x157fd7fa,0xef8579cc,0xd152de58,
0xdb2ffd5e,0x8f32ce19,0x306af97a,0x02f03ef8,0x99319ad5,0xc242fa0f,0xa7e3ebb0,0xc68e4906,
0xb8da230c,0x80823028,0xdcdef3c8,0xd35fb171,0x088a1bc8,0xbec0c560,0x61a3c9e8,0xbca8f54d,
0xc72feffa,0x22822e99,0x82c570b4,0xd8d94e89,0x8b1c34bc,0x301e16e6,0x273be979,0xb0ffeaa6,
0x61d9b8c6,0x00b24869,0xb7ffce3f,0x08dc283b,0x43daf65a,0xf7e19798,0x7619b72f,0x8f1c9ba4,
0xdc8637a0,0x16a7d3b1,0x9fc393b7,0xa7136eeb,0xc6bcc63e,0x1a513742,0xef6828bc,0x520365d6,
0x2d6a77ab,0x3527ed4b,0x821fd216,0x095c6e2e,0xdb92f2fb,0x5eea29cb,0x145892f5,0x91584f7f,
0x5483697b,0x2667a8cc,0x85196048,0x8c4bacea,0x833860d4,0x0d23e0f9,0x6c387e8a,0x0ae6d249,
0xb284600c,0xd835731d,0xdcb1c647,0xac4c56ea,0x3ebd81b3,0x230eabb0,0x6438bc87,0xf0b5b1fa,
0x8f5ea2b3,0xfc184642,0x0a036b7a,0x4fb089bd,0x649da589,0xa345415e,0x5c038323,0x3e5d3bb9,
0x43d79572,0x7e6dd07c,0x06dfdf1e,0x6c6cc4ef,0x7160a539,0x73bfbe70,0x83877605,0x4523ecf1
},
{
0x8defc240,0x25fa5d9f,0xeb903dbf,0xe810c907,0x47607fff,0x369fe44b,0x8c1fc644,0xaececa90,
0xbeb1f9bf,0xeefbcaea,0xe8cf1950,0x51df07ae,0x920e8806,0xf0ad0548,0xe13c8d83,0x927010d5,
0x11107d9f,0x07647db9,0xb2e3e4d4,0x3d4f285e,0xb9afa820,0xfade82e0,0xa067268b,0x8272792e,
0x553fb2c0,0x489ae22b,0xd4ef9794,0x125e3fbc,0x21fffcee,0x825b1bfd,0x9255c5ed,0x1257a240,
0x4e1a8302,0xbae07fff,0x528246e7,0x8e57140e,0x3373f7bf,0x8c9f8188,0xa6fc4ee8,0xc982b5a5,
0xa8c01db7,0x579fc264,0x67094f31,0xf2bd3f5f,0x40fff7c1,0x1fb78dfc,0x8e6bd2c1,0x437be59b,
0x99b03dbf,0xb5dbc64b,0x638dc0e6,0x55819d99,0xa197c81c,0x4a012d6e,0xc5884a28,0xccc36f71,
0xb843c213,0x6c0743f1,0x8309893c,0x0feddd5f,0x2f7fe850,0xd7c07f7e,0x02507fbf,0x5afb9a04,
0xa747d2d0,0x1651192e,0xaf70bf3e,0x58c31380,0x5f98302e,0x727cc3c4,0x0a0fb402,0x0f7fef82,
0x8c96fdad,0x5d2c2aae,0x8ee99a49,0x50da88b8,0x8427f4a0,0x1eac5790,0x796fb449,0x8252dc15,
0xefbd7d9b,0xa672597d,0xada840d8,0x45f54504,0xfa5d7403,0xe83ec305,0x4f91751a,0x925669c2,
0x23efe941,0xa903f12e,0x60270df2,0x0276e4b6,0x94fd6574,0x927985b2,0x8276dbcb,0x02778176,
0xf8af918d,0x4e48f79e,0x8f616ddf,0xe29d840e,0x842f7d83,0x340ce5c8,0x96bbb682,0x93b4b148,
0xef303cab,0x984faf28,0x779faf9b,0x92dc560d,0x224d1e20,0x8437aa88,0x7d29dc96,0x2756d3dc,
0x8b907cee,0xb51fd240,0xe7c07ce3,0xe566b4a1,0xc3e9615e,0x3cf8209d,0x6094d1e3,0xcd9ca341,
0x5c76460e,0x00ea983b,0xd4d67881,0xfd47572c,0xf76cedd9,0xbda8229c,0x127dadaa,0x438a074e,
0x1f97c090,0x081bdb8a,0x93a07ebe,0xb938ca15,0x97b03cff,0x3dc2c0f8,0x8d1ab2ec,0x64380e51,
0x68cc7bfb,0xd90f2788,0x12490181,0x5de5ffd4,0xdd7ef86a,0x76a2e214,0xb9a40368,0x925d958f,
0x4b39fffa,0xba39aee9,0xa4ffd30b,0xfaf7933b,0x6d498623,0x193cbcfa,0x27627545,0x825cf47a,
0x61bd8ba0,0xd11e42d1,0xcead04f4,0x127ea392,0x10428db7,0x8272a972,0x9270c4a8,0x127de50b,
0x285ba1c8,0x3c62f44f,0x35c0eaa5,0xe805d231,0x428929fb,0xb4fcdf82,0x4fb66a53,0x0e7dc15b,
0x1f081fab,0x108618ae,0xfcfd086d,0xf9ff2889,0x694bcc11,0x236a5cae,0x12deca4d,0x2c3f8cc5,
0xd2d02dfe,0xf8ef5896,0xe4cf52da,0x95155b67,0x494a488c,0xb9b6a80c,0x5c8f82bc,0x89d36b45,
0x3a609437,0xec00c9a9,0x44715253,0x0a874b49,0xd773bc40,0x7c34671c,0x02717ef6,0x4feb5536,
0xa2d02fff,0xd2bf60c4,0xd43f03c0,0x50b4ef6d,0x07478cd1,0x006e1888,0xa2e53f55,0xb9e6d4bc,
0xa2048016,0x97573833,0xd7207d67,0xde0f8f3d,0x72f87b33,0xabcc4f33,0x7688c55d,0x7b00a6b0,
0x947b0001,0x570075d2,0xf9bb88f8,0x8942019e,0x4264a5ff,0x856302e0,0x72dbd92b,0xee971b69,
0x6ea22fde,0x5f08ae2b,0xaf7a616d,0xe5c98767,0xcf1febd2,0x61efc8c2,0xf1ac2571,0xcc8239c2,
0x67214cb8,0xb1e583d1,0xb7dc3e62,0x7f10bdce,0xf90a5c38,0x0ff0443d,0x606e6dc6,0x60543a49,
0x5727c148,0x2be98a1d,0x8ab41738,0x20e1be24,0xaf96da0f,0x68458425,0x99833be5,0x600d457d,
0x282f9350,0x8334b362,0xd91d1120,0x2b6d8da0,0x642b1e31,0x9c305a00,0x52bce688,0x1b03588a,
0xf7baefd5,0x4142ed9c,0xa4315c11,0x83323ec5,0xdfef4636,0xa133c501,0xe9d3531c,0xee353783
},
{
0x9db30420,0x1fb6e9de,0xa7be7bef,0xd273a298,0x4a4f7bdb,0x64ad8c57,0x85510443,0xfa020ed1,
0x7e287aff,0xe60fb663,0x095f35a1,0x79ebf120,0xfd059d43,0x6497b7b1,0xf3641f63,0x241e4adf,
0x28147f5f,0x4fa2b8cd,0xc9430040,0x0cc32220,0xfdd30b30,0xc0a5374f,0x1d2d00d9,0x24147b15,
0xee4d111a,0x0fca5167,0x71ff904c,0x2d195ffe,0x1a05645f,0x0c13fefe,0x081b08ca,0x05170121,
0x80530100,0xe83e5efe,0xac9af4f8,0x7fe72701,0xd2b8ee5f,0x06df4261,0xbb9e9b8a,0x7293ea25,
0xce84ffdf,0xf5718801,0x3dd64b04,0xa26f263b,0x7ed48400,0x547eebe6,0x446d4ca0,0x6cf3d6f5,
0x2649abdf,0xaea0c7f5,0x36338cc1,0x503f7e93,0xd3772061,0x11b638e1,0x72500e03,0xf80eb2bb,
0xabe0502e,0xec8d77de,0x57971e81,0xe14f6746,0xc9335400,0x6920318f,0x081dbb99,0xffc304a5,
0x4d351805,0x7f3d5ce3,0xa6c866c6,0x5d5bcca9,0xdaec6fea,0x9f926f91,0x9f46222f,0x3991467d,
0xa5bf6d8e,0x1143c44f,0x43958302,0xd0214eeb,0x022083b8,0x3fb6180c,0x18f8931e,0x281658e6,
0x26486e3e,0x8bd78a70,0x7477e4c1,0xb506e07c,0xf32d0a25,0x79098b02,0xe4eabb81,0x28123b23,
0x69dead38,0x1574ca16,0xdf871b62,0x211c40b7,0xa51a9ef9,0x0014377b,0x041e8ac8,0x09114003,
0xbd59e4d2,0xe3d156d5,0x4fe876d5,0x2f91a340,0x557be8de,0x00eae4a7,0x0ce5c2ec,0x4db4bba6,
0xe756bdff,0xdd3369ac,0xec17b035,0x06572327,0x99afc8b0,0x56c8c391,0x6b65811c,0x5e146119,
0x6e85cb75,0xbe07c002,0xc2325577,0x893ff4ec,0x5bbfc92d,0xd0ec3b25,0xb7801ab7,0x8d6d3b24,
0x20c763ef,0xc366a5fc,0x9c382880,0x0ace3205,0xaac9548a,0xeca1d7c7,0x041afa32,0x1d16625a,
0x6701902c,0x9b757a54,0x31d477f7,0x9126b031,0x36cc6fdb,0xc70b8b46,0xd9e66a48,0x56e55a79,
0x026a4ceb,0x52437eff,0x2f8f76b4,0x0df980a5,0x8674cde3,0xedda04eb,0x17a9be04,0x2c18f4df,
0xb7747f9d,0xab2af7b4,0xefc34d20,0x2e096b7c,0x1741a254,0xe5b6a035,0x213d42f6,0x2c1c7c26,
0x61c2f50f,0x6552daf9,0xd2c231f8,0x25130f69,0xd8167fa2,0x0418f2c8,0x001a96a6,0x0d1526ab,
0x63315c21,0x5e0a72ec,0x49bafefd,0x187908d9,0x8d0dbd86,0x311170a7,0x3e9b640c,0xcc3e10d7,
0xd5cad3b6,0x0caec388,0xf73001e1,0x6c728aff,0x71eae2a1,0x1f9af36e,0xcfcbd12f,0xc1de8417,
0xac07be6b,0xcb44a1d8,0x8b9b0f56,0x013988c3,0xb1c52fca,0xb4be31cd,0xd8782806,0x12a3a4e2,
0x6f7de532,0x58fd7eb6,0xd01ee900,0x24adffc2,0xf4990fc5,0x9711aac5,0x001d7b95,0x82e5e7d2,
0x109873f6,0x00613096,0xc32d9521,0xada121ff,0x29908415,0x7fbb977f,0xaf9eb3db,0x29c9ed2a,
0x5ce2a465,0xa730f32c,0xd0aa3fe8,0x8a5cc091,0xd49e2ce7,0x0ce454a9,0xd60acd86,0x015f1919,
0x77079103,0xdea03af6,0x78a8565e,0xdee356df,0x21f05cbe,0x8b75e387,0xb3c50651,0xb8a5c3ef,
0xd8eeb6d2,0xe523be77,0xc2154529,0x2f69efdf,0xafe67afb,0xf470c4b2,0xf3e0eb5b,0xd6cc9876,
0x39e4460c,0x1fda8538,0x1987832f,0xca007367,0xa99144f8,0x296b299e,0x492fc295,0x9266beab,
0xb5676e69,0x9bd3ddda,0xdf7e052f,0xdb25701c,0x1b5e51ee,0xf65324e6,0x6afce36c,0x0316cc04,
0x8644213e,0xb7dc59d0,0x7965291f,0xccd6fd43,0x41823979,0x932bcdf6,0xb657c34d,0x4edfd282,
0x7ae5290c,0x3cb9536b,0x851e20fe,0x9833557e,0x13ecf0b0,0xd3ffb372,0x3f85c5c1,0x0aef7ed2
},
{
0x7ec90c04,0x2c6e74b9,0x9b0e66df,0xa6337911,0xb86a7fff,0x1dd358f5,0x44dd9d44,0x1731167f,
0x08fbf1fa,0xe7f511cc,0xd2051b00,0x735aba00,0x2ab722d8,0x386381cb,0xacf6243a,0x69befd7a,
0xe6a2e77f,0xf0c720cd,0xc4494816,0xccf5c180,0x38851640,0x15b0a848,0xe68b18cb,0x4caadeff,
0x5f480a01,0x0412b2aa,0x259814fc,0x41d0efe2,0x4e40b48d,0x248eb6fb,0x8dba1cfe,0x41a99b02,
0x1a550a04,0xba8f65cb,0x7251f4e7,0x95a51725,0xc106ecd7,0x97a5980a,0xc539b9aa,0x4d79fe6a,
0xf2f3f763,0x68af8040,0xed0c9e56,0x11b4958b,0xe1eb5a88,0x8709e6b0,0xd7e07156,0x4e29fea7,
0x6366e52d,0x02d1c000,0xc4ac8e05,0x9377f571,0x0c05372a,0x578535f2,0x2261be02,0xd642a0c9,
0xdf13a280,0x74b55bd2,0x682199c0,0xd421e5ec,0x53fb3ce8,0xc8adedb3,0x28a87fc9,0x3d959981,
0x5c1ff900,0xfe38d399,0x0c4eff0b,0x062407ea,0xaa2f4fb1,0x4fb96976,0x90c79505,0xb0a8a774,
0xef55a1ff,0xe59ca2c2,0xa6b62d27,0xe66a4263,0xdf65001f,0x0ec50966,0xdfdd55bc,0x29de0655,
0x911e739a,0x17af8975,0x32c7911c,0x89f89468,0x0d01e980,0x524755f4,0x03b63cc9,0x0cc844b2,
0xbcf3f0aa,0x87ac36e9,0xe53a7426,0x01b3d82b,0x1a9e7449,0x64ee2d7e,0xcddbb1da,0x01c94910,
0xb868bf80,0x0d26f3fd,0x9342ede7,0x04a5c284,0x636737b6,0x50f5b616,0xf24766e3,0x8eca36c1,
0x136e05db,0xfef18391,0xfb887a37,0xd6e7f7d4,0xc7fb7dc9,0x3063fcdf,0xb6f589de,0xec2941da,
0x26e46695,0xb7566419,0xf654efc5,0xd08d58b7,0x48925401,0xc1bacb7f,0xe5ff550f,0xb6083049,
0x5bb5d0e8,0x87d72e5a,0xab6a6ee1,0x223a66ce,0xc62bf3cd,0x9e0885f9,0x68cb3e47,0x086c010f,
0xa21de820,0xd18b69de,0xf3f65777,0xfa02c3f6,0x407edac3,0xcbb3d550,0x1793084d,0xb0d70eba,
0x0ab378d5,0xd951fb0c,0xded7da56,0x4124bbe4,0x94ca0b56,0x0f5755d1,0xe0e1e56e,0x6184b5be,
0x580a249f,0x94f74bc0,0xe327888e,0x9f7b5561,0xc3dc0280,0x05687715,0x646c6bd7,0x44904db3,
0x66b4f0a3,0xc0f1648a,0x697ed5af,0x49e92ff6,0x309e374f,0x2cb6356a,0x85808573,0x4991f840,
0x76f0ae02,0x083be84d,0x28421c9a,0x44489406,0x736e4cb8,0xc1092910,0x8bc95fc6,0x7d869cf4,
0x134f616f,0x2e77118d,0xb31b2be1,0xaa90b472,0x3ca5d717,0x7d161bba,0x9cad9010,0xaf462ba2,
0x9fe459d2,0x45d34559,0xd9f2da13,0xdbc65487,0xf3e4f94e,0x176d486f,0x097c13ea,0x631da5c7,
0x445f7382,0x175683f4,0xcdc66a97,0x70be0288,0xb3cdcf72,0x6e5dd2f3,0x20936079,0x459b80a5,
0xbe60e2db,0xa9c23101,0xeba5315c,0x224e42f2,0x1c5c1572,0xf6721b2c,0x1ad2fff3,0x8c25404e,
0x324ed72f,0x4067b7fd,0x0523138e,0x5ca3bc78,0xdc0fd66e,0x75922283,0x784d6b17,0x58ebb16e,
0x44094f85,0x3f481d87,0xfcfeae7b,0x77b5ff76,0x8c2302bf,0xaaf47556,0x5f46b02a,0x2b092801,
0x3d38f5f7,0x0ca81f36,0x52af4a8a,0x66d5e7c0,0xdf3b0874,0x95055110,0x1b5ad7a8,0xf61ed5ad,
0x6cf6e479,0x20758184,0xd0cefa65,0x88f7be58,0x4a046826,0x0ff6f8f3,0xa09c7f70,0x5346aba0,
0x5ce96c28,0xe176eda3,0x6bac307f,0x376829d2,0x85360fa9,0x17e3fe2a,0x24b79767,0xf5a96b20,
0xd6cd2595,0x68ff1ebf,0x7555442c,0xf19f06be,0xf9e0659a,0xeeb9491d,0x34010718,0xbb30cab8,
0xe822fe15,0x88570983,0x750e6249,0xda627e55,0x5e76ffa8,0xb1534546,0x6d47de08,0xefe9e7d4
},
{
0xf6fa8f9d,0x2cac6ce1,0x4ca34867,0xe2337f7c,0x95db08e7,0x016843b4,0xeced5cbc,0x325553ac,
0xbf9f0960,0xdfa1e2ed,0x83f0579d,0x63ed86b9,0x1ab6a6b8,0xde5ebe39,0xf38ff732,0x8989b138,
0x33f14961,0xc01937bd,0xf506c6da,0xe4625e7e,0xa308ea99,0x4e23e33c,0x79cbd7cc,0x48a14367,
0xa3149619,0xfec94bd5,0xa114174a,0xeaa01866,0xa084db2d,0x09a8486f,0xa888614a,0x2900af98,
0x01665991,0xe1992863,0xc8f30c60,0x2e78ef3c,0xd0d51932,0xcf0fec14,0xf7ca07d2,0xd0a82072,
0xfd41197e,0x9305a6b0,0xe86be3da,0x74bed3cd,0x372da53c,0x4c7f4448,0xdab5d440,0x6dba0ec3,
0x083919a7,0x9fbaeed9,0x49dbcfb0,0x4e670c53,0x5c3d9c01,0x64bdb941,0x2c0e636a,0xba7dd9cd,
0xea6f7388,0xe70bc762,0x35f29adb,0x5c4cdd8d,0xf0d48d8c,0xb88153e2,0x08a19866,0x1ae2eac8,
0x284caf89,0xaa928223,0x9334be53,0x3b3a21bf,0x16434be3,0x9aea3906,0xefe8c36e,0xf890cdd9,
0x80226dae,0xc340a4a3,0xdf7e9c09,0xa694a807,0x5b7c5ecc,0x221db3a6,0x9a69a02f,0x68818a54,
0xceb2296f,0x53c0843a,0xfe893655,0x25bfe68a,0xb4628abc,0xcf222ebf,0x25ac6f48,0xa9a99387,
0x53bddb65,0xe76ffbe7,0xe967fd78,0x0ba93563,0x8e342bc1,0xe8a11be9,0x4980740d,0xc8087dfc,
0x8de4bf99,0xa11101a0,0x7fd37975,0xda5a26c0,0xe81f994f,0x9528cd89,0xfd339fed,0xb87834bf,
0x5f04456d,0x22258698,0xc9c4c83b,0x2dc156be,0x4f628daa,0x57f55ec5,0xe2220abe,0xd2916ebf,
0x4ec75b95,0x24f2c3c0,0x42d15d99,0xcd0d7fa0,0x7b6e27ff,0xa8dc8af0,0x7345c106,0xf41e232f,
0x35162386,0xe6ea8926,0x3333b094,0x157ec6f2,0x372b74af,0x692573e4,0xe9a9d848,0xf3160289,
0x3a62ef1d,0xa787e238,0xf3a5f676,0x74364853,0x20951063,0x4576698d,0xb6fad407,0x592af950,
0x36f73523,0x4cfb6e87,0x7da4cec0,0x6c152daa,0xcb0396a8,0xc50dfe5d,0xfcd707ab,0x0921c42f,
0x89dff0bb,0x5fe2be78,0x448f4f33,0x754613c9,0x2b05d08d,0x48b9d585,0xdc049441,0xc8098f9b,
0x7dede786,0xc39a3373,0x42410005,0x6a091751,0x0ef3c8a6,0x890072d6,0x28207682,0xa9a9f7be,
0xbf32679d,0xd45b5b75,0xb353fd00,0xcbb0e358,0x830f220a,0x1f8fb214,0xd372cf08,0xcc3c4a13,
0x8cf63166,0x061c87be,0x88c98f88,0x6062e397,0x47cf8e7a,0xb6c85283,0x3cc2acfb,0x3fc06976,
0x4e8f0252,0x64d8314d,0xda3870e3,0x1e665459,0xc10908f0,0x513021a5,0x6c5b68b7,0x822f8aa0,
0x3007cd3e,0x74719eef,0xdc872681,0x073340d4,0x7e432fd9,0x0c5ec241,0x8809286c,0xf592d891,
0x08a930f6,0x957ef305,0xb7fbffbd,0xc266e96f,0x6fe4ac98,0xb173ecc0,0xbc60b42a,0x953498da,
0xfba1ae12,0x2d4bd736,0x0f25faab,0xa4f3fceb,0xe2969123,0x257f0c3d,0x9348af49,0x361400bc,
0xe8816f4a,0x3814f200,0xa3f94043,0x9c7a54c2,0xbc704f57,0xda41e7f9,0xc25ad33a,0x54f4a084,
0xb17f5505,0x59357cbe,0xedbd15c8,0x7f97c5ab,0xba5ac7b5,0xb6f6deaf,0x3a479c3a,0x5302da25,
0x653d7e6a,0x54268d49,0x51a477ea,0x5017d55b,0xd7d25d88,0x44136c76,0x0404a8c8,0xb8e5a121,
0xb81a928a,0x60ed5869,0x97c55b96,0xeaec991b,0x29935913,0x01fdb7f1,0x088e8dfa,0x9ab6f6f5,
0x3b4cbf9f,0x4a5de3ab,0xe6051d35,0xa0e1d855,0xd36b4cf1,0xf544edeb,0xb0e93524,0xbebb8fbd,
0xa2d762cf,0x49c92f54,0x38b5f331,0x7128a454,0x48392905,0xa65b1db8,0x851c97bd,0xd675cf2f
},
{
0x85e04019,0x332bf567,0x662dbfff,0xcfc65693,0x2a8d7f6f,0xab9bc912,0xde6008a1,0x2028da1f,
0x0227bce7,0x4d642916,0x18fac300,0x50f18b82,0x2cb2cb11,0xb232e75c,0x4b3695f2,0xb28707de,
0xa05fbcf6,0xcd4181e9,0xe150210c,0xe24ef1bd,0xb168c381,0xfde4e789,0x5c79b0d8,0x1e8bfd43,
0x4d495001,0x38be4341,0x913cee1d,0x92a79c3f,0x089766be,0xbaeeadf4,0x1286becf,0xb6eacb19,
0x2660c200,0x7565bde4,0x64241f7a,0x8248dca9,0xc3b3ad66,0x28136086,0x0bd8dfa8,0x356d1cf2,
0x107789be,0xb3b2e9ce,0x0502aa8f,0x0bc0351e,0x166bf52a,0xeb12ff82,0xe3486911,0xd34d7516,
0x4e7b3aff,0x5f43671b,0x9cf6e037,0x4981ac83,0x334266ce,0x8c9341b7,0xd0d854c0,0xcb3a6c88,
0x47bc2829,0x4725ba37,0xa66ad22b,0x7ad61f1e,0x0c5cbafa,0x4437f107,0xb6e79962,0x42d2d816,
0x0a961288,0xe1a5c06e,0x13749e67,0x72fc081a,0xb1d139f7,0xf9583745,0xcf19df58,0xbec3f756,
0xc06eba30,0x07211b24,0x45c28829,0xc95e317f,0xbc8ec511,0x38bc46e9,0xc6e6fa14,0xbae8584a,
0xad4ebc46,0x468f508b,0x7829435f,0xf124183b,0x821dba9f,0xaff60ff4,0xea2c4e6d,0x16e39264,
0x92544a8b,0x009b4fc3,0xaba68ced,0x9ac96f78,0x06a5b79a,0xb2856e6e,0x1aec3ca9,0xbe838688,
0x0e0804e9,0x55f1be56,0xe7e5363b,0xb3a1f25d,0xf7debb85,0x61fe033c,0x16746233,0x3c034c28,
0xda6d0c74,0x79aac56c,0x3ce4e1ad,0x51f0c802,0x98f8f35a,0x1626a49f,0xeed82b29,0x1d382fe3,
0x0c4fb99a,0xbb325778,0x3ec6d97b,0x6e77a6a9,0xcb658b5c,0xd45230c7,0x2bd1408b,0x60c03eb7,
0xb9068d78,0xa33754f4,0xf430c87d,0xc8a71302,0xb96d8c32,0xebd4e7be,0xbe8b9d2d,0x7979fb06,
0xe7225308,0x8b75cf77,0x11ef8da4,0xe083c858,0x8d6b786f,0x5a6317a6,0xfa5cf7a0,0x5dda0033,
0xf28ebfb0,0xf5b9c310,0xa0eac280,0x08b9767a,0xa3d9d2b0,0x79d34217,0x021a718d,0x9ac6336a,
0x2711fd60,0x438050e3,0x069908a8,0x3d7fedc4,0x826d2bef,0x4eeb8476,0x488dcf25,0x36c9d566,
0x28e74e41,0xc2610aca,0x3d49a9cf,0xbae3b9df,0xb65f8de6,0x92aeaf64,0x3ac7d5e6,0x9ea80509,
0xf22b017d,0xa4173f70,0xdd1e16c3,0x15e0d7f9,0x50b1b887,0x2b9f4fd5,0x625aba82,0x6a017962,
0x2ec01b9c,0x15488aa9,0xd716e740,0x40055a2c,0x93d29a22,0xe32dbf9a,0x058745b9,0x3453dc1e,
0xd699296e,0x496cff6f,0x1c9f4986,0xdfe2ed07,0xb87242d1,0x19de7eae,0x053e561a,0x15ad6f8c,
0x66626c1c,0x7154c24c,0xea082b2a,0x93eb2939,0x17dcb0f0,0x58d4f2ae,0x9ea294fb,0x52cf564c,
0x9883fe66,0x2ec40581,0x763953c3,0x01d6692e,0xd3a0c108,0xa1e7160e,0xe4f2dfa6,0x693ed285,
0x74904698,0x4c2b0edd,0x4f757656,0x5d393378,0xa132234f,0x3d321c5d,0xc3f5e194,0x4b269301,
0xc79f022f,0x3c997e7e,0x5e4f9504,0x3ffafbbd,0x76f7ad0e,0x296693f4,0x3d1fce6f,0xc61e45be,
0xd3b5ab34,0xf72bf9b7,0x1b0434c0,0x4e72b567,0x5592a33d,0xb5229301,0xcfd2a87f,0x60aeb767,
0x1814386b,0x30bcc33d,0x38a0c07d,0xfd1606f2,0xc363519b,0x589dd390,0x5479f8e6,0x1cb8d647,
0x97fd61a9,0xea7759f4,0x2d57539d,0x569a58cf,0xe84e63ad,0x462e1b78,0x6580f87e,0xf3817914,
0x91da55f4,0x40a230f3,0xd1988f35,0xb6e318d2,0x3ffa50bc,0x3d40f021,0xc3c0bdae,0x4958c24c,
0x518f36b2,0x84b1d370,0x0fedce83,0x878ddada,0xf2a279c7,0x94e01be8,0x90716f4b,0x954b8aa3
},
{
0xe216300d,0xbbddfffc,0xa7ebdabd,0x35648095,0x7789f8b7,0xe6c1121b,0x0e241600,0x052ce8b5,
0x11a9cfb0,0xe5952f11,0xece7990a,0x9386d174,0x2a42931c,0x76e38111,0xb12def3a,0x37ddddfc,
0xde9adeb1,0x0a0cc32c,0xbe197029,0x84a00940,0xbb243a0f,0xb4d137cf,0xb44e79f0,0x049eedfd,
0x0b15a15d,0x480d3168,0x8bbbde5a,0x669ded42,0xc7ece831,0x3f8f95e7,0x72df191b,0x7580330d,
0x94074251,0x5c7dcdfa,0xabbe6d63,0xaa402164,0xb301d40a,0x02e7d1ca,0x53571dae,0x7a3182a2,
0x12a8ddec,0xfdaa335d,0x176f43e8,0x71fb46d4,0x38129022,0xce949ad4,0xb84769ad,0x965bd862,
0x82f3d055,0x66fb9767,0x15b80b4e,0x1d5b47a0,0x4cfde06f,0xc28ec4b8,0x57e8726e,0x647a78fc,
0x99865d44,0x608bd593,0x6c200e03,0x39dc5ff6,0x5d0b00a3,0xae63aff2,0x7e8bd632,0x70108c0c,
0xbbd35049,0x2998df04,0x980cf42a,0x9b6df491,0x9e7edd53,0x06918548,0x58cb7e07,0x3b74ef2e,
0x522fffb1,0xd24708cc,0x1c7e27cd,0xa4eb215b,0x3cf1d2e2,0x19b47a38,0x424f7618,0x35856039,
0x9d17dee7,0x27eb35e6,0xc9aff67b,0x36baf5b8,0x09c467cd,0xc18910b1,0xe11dbf7b,0x06cd1af8,
0x7170c608,0x2d5e3354,0xd4de495a,0x64c6d006,0xbcc0c62c,0x3dd00db3,0x708f8f34,0x77d51b42,
0x264f620f,0x24b8d2bf,0x15c1b79e,0x46a52564,0xf8d7e54e,0x3e378160,0x7895cda5,0x859c15a5,
0xe6459788,0xc37bc75f,0xdb07ba0c,0x0676a3ab,0x7f229b1e,0x31842e7b,0x24259fd7,0xf8bef472,
0x835ffcb8,0x6df4c1f2,0x96f5b195,0xfd0af0fc,0xb0fe134c,0xe2506d3d,0x4f9b12ea,0xf215f225,
0xa223736f,0x9fb4c428,0x25d04979,0x34c713f8,0xc4618187,0xea7a6e98,0x7cd16efc,0x1436876c,
0xf1544107,0xbedeee14,0x56e9af27,0xa04aa441,0x3cf7c899,0x92ecbae6,0xdd67016d,0x151682eb,
0xa842eedf,0xfdba60b4,0xf1907b75,0x20e3030f,0x24d8c29e,0xe139673b,0xefa63fb8,0x71873054,
0xb6f2cf3b,0x9f326442,0xcb15a4cc,0xb01a4504,0xf1e47d8d,0x844a1be5,0xbae7dfdc,0x42cbda70,
0xcd7dae0a,0x57e85b7a,0xd53f5af6,0x20cf4d8c,0xcea4d428,0x79d130a4,0x3486ebfb,0x33d3cddc,
0x77853b53,0x37effcb5,0xc5068778,0xe580b3e6,0x4e68b8f4,0xc5c8b37e,0x0d809ea2,0x398feb7c,
0x132a4f94,0x43b7950e,0x2fee7d1c,0x223613bd,0xdd06caa2,0x37df932b,0xc4248289,0xacf3ebc3,
0x5715f6b7,0xef3478dd,0xf267616f,0xc148cbe4,0x9052815e,0x5e410fab,0xb48a2465,0x2eda7fa4,
0xe87b40e4,0xe98ea084,0x5889e9e1,0xefd390fc,0xdd07d35b,0xdb485694,0x38d7e5b2,0x57720101,
0x730edebc,0x5b643113,0x94917e4f,0x503c2fba,0x646f1282,0x7523d24a,0xe0779695,0xf9c17a8f,
0x7a5b2121,0xd187b896,0x29263a4d,0xba510cdf,0x81f47c9f,0xad1163ed,0xea7b5965,0x1a00726e,
0x11403092,0x00da6d77,0x4a0cdd61,0xad1f4603,0x605bdfb0,0x9eedc364,0x22ebe6a8,0xcee7d28a,
0xa0e736a0,0x5564a6b9,0x10853209,0xc7eb8f37,0x2de705ca,0x8951570f,0xdf09822b,0xbd691a6c,
0xaa12e4f2,0x87451c0f,0xe0f6a27a,0x3ada4819,0x4cf1764f,0x0d771c2b,0x67cdb156,0x350d8384,
0x5938fa0f,0x42399ef3,0x36997b07,0x0e84093d,0x4aa93e61,0x8360d87b,0x1fa98b0c,0x1149382c,
0xe97625a5,0x0614d1b7,0x0e25244b,0x0c768347,0x589e8d82,0x0d2059d1,0xa466bb1e,0xf8da0a82,
0x04f19130,0xba6e4ec0,0x99265164,0x1ee7230d,0x50b2ad80,0xeaee6801,0x8db2a283,0xea8bf59e
}
};
#define CS1 CAST_S[0]
#define CS2 CAST_S[1]
#define CS3 CAST_S[2]
#define CS4 CAST_S[3]
#define CS5 CAST_S[4]
#define CS6 CAST_S[5]
#define CS7 CAST_S[6]
#define CS8 CAST_S[7]
#define CAST_ROL(x,n) ((uint32_t)(((x)<<((n)&31))|((x)>>((32-(n))&31))))
#define CAST_XW(a,b,c,d) (((uint32_t)(a)<<24)|((uint32_t)(b)<<16)|((uint32_t)(c)<<8)|(d))
static void cast_setw(unsigned char *a,uint32_t w){ a[0]=(unsigned char)(w>>24);a[1]=(unsigned char)(w>>16);a[2]=(unsigned char)(w>>8);a[3]=(unsigned char)w; }
typedef struct { uint32_t Km[16]; uint32_t Kr[16]; } cast_ctx;
static void cast_key(const unsigned char key[16], cast_ctx *c){
  unsigned char x[16],z[16]; uint32_t K[32]; int m;
  memcpy(x,key,16);
  for(m=0;m<2;m++){ uint32_t *Kp=K+16*m;
    cast_setw(z+0,  CAST_XW(x[0],x[1],x[2],x[3]) ^ CS5[x[13]]^CS6[x[15]]^CS7[x[12]]^CS8[x[14]]^CS7[x[8]]);
    cast_setw(z+4,  CAST_XW(x[8],x[9],x[10],x[11]) ^ CS5[z[0]]^CS6[z[2]]^CS7[z[1]]^CS8[z[3]]^CS8[x[10]]);
    cast_setw(z+8,  CAST_XW(x[12],x[13],x[14],x[15]) ^ CS5[z[7]]^CS6[z[6]]^CS7[z[5]]^CS8[z[4]]^CS5[x[9]]);
    cast_setw(z+12, CAST_XW(x[4],x[5],x[6],x[7]) ^ CS5[z[10]]^CS6[z[9]]^CS7[z[11]]^CS8[z[8]]^CS6[x[11]]);
    Kp[0]=CS5[z[8]]^CS6[z[9]]^CS7[z[7]]^CS8[z[6]]^CS5[z[2]];
    Kp[1]=CS5[z[10]]^CS6[z[11]]^CS7[z[5]]^CS8[z[4]]^CS6[z[6]];
    Kp[2]=CS5[z[12]]^CS6[z[13]]^CS7[z[3]]^CS8[z[2]]^CS7[z[9]];
    Kp[3]=CS5[z[14]]^CS6[z[15]]^CS7[z[1]]^CS8[z[0]]^CS8[z[12]];
    cast_setw(x+0,  CAST_XW(z[8],z[9],z[10],z[11]) ^ CS5[z[5]]^CS6[z[7]]^CS7[z[4]]^CS8[z[6]]^CS7[z[0]]);
    cast_setw(x+4,  CAST_XW(z[0],z[1],z[2],z[3]) ^ CS5[x[0]]^CS6[x[2]]^CS7[x[1]]^CS8[x[3]]^CS8[z[2]]);
    cast_setw(x+8,  CAST_XW(z[4],z[5],z[6],z[7]) ^ CS5[x[7]]^CS6[x[6]]^CS7[x[5]]^CS8[x[4]]^CS5[z[1]]);
    cast_setw(x+12, CAST_XW(z[12],z[13],z[14],z[15]) ^ CS5[x[10]]^CS6[x[9]]^CS7[x[11]]^CS8[x[8]]^CS6[z[3]]);
    Kp[4]=CS5[x[3]]^CS6[x[2]]^CS7[x[12]]^CS8[x[13]]^CS5[x[8]];
    Kp[5]=CS5[x[1]]^CS6[x[0]]^CS7[x[14]]^CS8[x[15]]^CS6[x[13]];
    Kp[6]=CS5[x[7]]^CS6[x[6]]^CS7[x[8]]^CS8[x[9]]^CS7[x[3]];
    Kp[7]=CS5[x[5]]^CS6[x[4]]^CS7[x[10]]^CS8[x[11]]^CS8[x[7]];
    cast_setw(z+0,  CAST_XW(x[0],x[1],x[2],x[3]) ^ CS5[x[13]]^CS6[x[15]]^CS7[x[12]]^CS8[x[14]]^CS7[x[8]]);
    cast_setw(z+4,  CAST_XW(x[8],x[9],x[10],x[11]) ^ CS5[z[0]]^CS6[z[2]]^CS7[z[1]]^CS8[z[3]]^CS8[x[10]]);
    cast_setw(z+8,  CAST_XW(x[12],x[13],x[14],x[15]) ^ CS5[z[7]]^CS6[z[6]]^CS7[z[5]]^CS8[z[4]]^CS5[x[9]]);
    cast_setw(z+12, CAST_XW(x[4],x[5],x[6],x[7]) ^ CS5[z[10]]^CS6[z[9]]^CS7[z[11]]^CS8[z[8]]^CS6[x[11]]);
    Kp[8]=CS5[z[3]]^CS6[z[2]]^CS7[z[12]]^CS8[z[13]]^CS5[z[9]];
    Kp[9]=CS5[z[1]]^CS6[z[0]]^CS7[z[14]]^CS8[z[15]]^CS6[z[12]];
    Kp[10]=CS5[z[7]]^CS6[z[6]]^CS7[z[8]]^CS8[z[9]]^CS7[z[2]];
    Kp[11]=CS5[z[5]]^CS6[z[4]]^CS7[z[10]]^CS8[z[11]]^CS8[z[6]];
    cast_setw(x+0,  CAST_XW(z[8],z[9],z[10],z[11]) ^ CS5[z[5]]^CS6[z[7]]^CS7[z[4]]^CS8[z[6]]^CS7[z[0]]);
    cast_setw(x+4,  CAST_XW(z[0],z[1],z[2],z[3]) ^ CS5[x[0]]^CS6[x[2]]^CS7[x[1]]^CS8[x[3]]^CS8[z[2]]);
    cast_setw(x+8,  CAST_XW(z[4],z[5],z[6],z[7]) ^ CS5[x[7]]^CS6[x[6]]^CS7[x[5]]^CS8[x[4]]^CS5[z[1]]);
    cast_setw(x+12, CAST_XW(z[12],z[13],z[14],z[15]) ^ CS5[x[10]]^CS6[x[9]]^CS7[x[11]]^CS8[x[8]]^CS6[z[3]]);
    Kp[12]=CS5[x[8]]^CS6[x[9]]^CS7[x[7]]^CS8[x[6]]^CS5[x[3]];
    Kp[13]=CS5[x[10]]^CS6[x[11]]^CS7[x[5]]^CS8[x[4]]^CS6[x[7]];
    Kp[14]=CS5[x[12]]^CS6[x[13]]^CS7[x[3]]^CS8[x[2]]^CS7[x[8]];
    Kp[15]=CS5[x[14]]^CS6[x[15]]^CS7[x[1]]^CS8[x[0]]^CS8[x[13]];
  }
  for(int i=0;i<16;i++){ c->Km[i]=K[i]; c->Kr[i]=K[16+i]&0x1f; }
}
#if defined(__aarch64__) && defined(__APPLE__)
extern void cast_core_asm(const uint32_t *Km, const uint32_t *Kr, const unsigned char *typ,
                          const uint32_t *S, uint32_t io[2]);
#endif
static void cast_block(const cast_ctx *c, const unsigned char in[8], unsigned char out[8], int encrypt){
#if defined(__aarch64__) && defined(__APPLE__)
  uint32_t Km[16],Kr[16],io[2]; unsigned char typ[16]; int r;
  for(r=0;r<16;r++){ int i=encrypt?r:15-r; Km[r]=c->Km[i]; Kr[r]=c->Kr[i]; typ[r]=(unsigned char)(i%3); }
  io[0]=CAST_XW(in[0],in[1],in[2],in[3]); io[1]=CAST_XW(in[4],in[5],in[6],in[7]);
  cast_core_asm(Km,Kr,typ,&CAST_S[0][0],io);
  cast_setw(out+0,io[0]); cast_setw(out+4,io[1]);
#else
  uint32_t L=CAST_XW(in[0],in[1],in[2],in[3]),R=CAST_XW(in[4],in[5],in[6],in[7]),t; int r;
  for(r=0;r<16;r++){ int i=encrypt?r:15-r; uint32_t I,F,Km=c->Km[i],Kr=c->Kr[i]; int ty=i%3;
    if(ty==0){ I=CAST_ROL(Km+R,Kr); F=((CS1[I>>24]^CS2[(I>>16)&0xff])-CS3[(I>>8)&0xff])+CS4[I&0xff]; }
    else if(ty==1){ I=CAST_ROL(Km^R,Kr); F=((CS1[I>>24]-CS2[(I>>16)&0xff])+CS3[(I>>8)&0xff])^CS4[I&0xff]; }
    else { I=CAST_ROL(Km-R,Kr); F=((CS1[I>>24]+CS2[(I>>16)&0xff])^CS3[(I>>8)&0xff])-CS4[I&0xff]; }
    t=L^F; L=R; R=t; }
  cast_setw(out+0,R); cast_setw(out+4,L);
#endif
}
void rktcrypto_cast5_ecb(const unsigned char key[16], const unsigned char *in, unsigned char *out,
                         intptr_t nblk, int encrypt){
  cast_ctx c; intptr_t n; cast_key(key,&c);
  for(n=0;n<nblk;n++) cast_block(&c,in+8*n,out+8*n,encrypt);
}
void rktcrypto_cast5_cbc(const unsigned char key[16], const unsigned char iv[8], const unsigned char *in,
                         unsigned char *out, intptr_t nblk, int encrypt){
  cast_ctx c; intptr_t n; unsigned char prev[8]; int q; cast_key(key,&c); memcpy(prev,iv,8);
  if(encrypt){ for(n=0;n<nblk;n++){ unsigned char t[8]; for(q=0;q<8;q++)t[q]=(unsigned char)(in[8*n+q]^prev[q]);
      cast_block(&c,t,out+8*n,1); memcpy(prev,out+8*n,8); } }
  else { for(n=0;n<nblk;n++){ unsigned char cc[8],p[8]; memcpy(cc,in+8*n,8); cast_block(&c,cc,p,0);
      for(q=0;q<8;q++)out[8*n+q]=(unsigned char)(p[q]^prev[q]); memcpy(prev,cc,8); } }
}


/* =============================== SEED =============================== */
/* SEED (RFC 4269 / TTAS.KO-12.0004), 128-bit block, 128-bit key. Korean legacy
   cipher; interop only. SS tables are the standard RFC 4269 tables. 16-round
   Feistel core (F built on the G function) is the rktasm kernel. */
static const uint32_t SEED_SS[4][256]={
{
0x2989a1a8,0x05858184,0x16c6d2d4,0x13c3d3d0,0x14445054,0x1d0d111c,0x2c8ca0ac,0x25052124,
0x1d4d515c,0x03434340,0x18081018,0x1e0e121c,0x11415150,0x3cccf0fc,0x0acac2c8,0x23436360,
0x28082028,0x04444044,0x20002020,0x1d8d919c,0x20c0e0e0,0x22c2e2e0,0x08c8c0c8,0x17071314,
0x2585a1a4,0x0f8f838c,0x03030300,0x3b4b7378,0x3b8bb3b8,0x13031310,0x12c2d2d0,0x2ecee2ec,
0x30407070,0x0c8c808c,0x3f0f333c,0x2888a0a8,0x32023230,0x1dcdd1dc,0x36c6f2f4,0x34447074,
0x2ccce0ec,0x15859194,0x0b0b0308,0x17475354,0x1c4c505c,0x1b4b5358,0x3d8db1bc,0x01010100,
0x24042024,0x1c0c101c,0x33437370,0x18889098,0x10001010,0x0cccc0cc,0x32c2f2f0,0x19c9d1d8,
0x2c0c202c,0x27c7e3e4,0x32427270,0x03838380,0x1b8b9398,0x11c1d1d0,0x06868284,0x09c9c1c8,
0x20406060,0x10405050,0x2383a3a0,0x2bcbe3e8,0x0d0d010c,0x3686b2b4,0x1e8e929c,0x0f4f434c,
0x3787b3b4,0x1a4a5258,0x06c6c2c4,0x38487078,0x2686a2a4,0x12021210,0x2f8fa3ac,0x15c5d1d4,
0x21416160,0x03c3c3c0,0x3484b0b4,0x01414140,0x12425250,0x3d4d717c,0x0d8d818c,0x08080008,
0x1f0f131c,0x19899198,0x00000000,0x19091118,0x04040004,0x13435350,0x37c7f3f4,0x21c1e1e0,
0x3dcdf1fc,0x36467274,0x2f0f232c,0x27072324,0x3080b0b0,0x0b8b8388,0x0e0e020c,0x2b8ba3a8,
0x2282a2a0,0x2e4e626c,0x13839390,0x0d4d414c,0x29496168,0x3c4c707c,0x09090108,0x0a0a0208,
0x3f8fb3bc,0x2fcfe3ec,0x33c3f3f0,0x05c5c1c4,0x07878384,0x14041014,0x3ecef2fc,0x24446064,
0x1eced2dc,0x2e0e222c,0x0b4b4348,0x1a0a1218,0x06060204,0x21012120,0x2b4b6368,0x26466264,
0x02020200,0x35c5f1f4,0x12829290,0x0a8a8288,0x0c0c000c,0x3383b3b0,0x3e4e727c,0x10c0d0d0,
0x3a4a7278,0x07474344,0x16869294,0x25c5e1e4,0x26062224,0x00808080,0x2d8da1ac,0x1fcfd3dc,
0x2181a1a0,0x30003030,0x37073334,0x2e8ea2ac,0x36063234,0x15051114,0x22022220,0x38083038,
0x34c4f0f4,0x2787a3a4,0x05454144,0x0c4c404c,0x01818180,0x29c9e1e8,0x04848084,0x17879394,
0x35053134,0x0bcbc3c8,0x0ecec2cc,0x3c0c303c,0x31417170,0x11011110,0x07c7c3c4,0x09898188,
0x35457174,0x3bcbf3f8,0x1acad2d8,0x38c8f0f8,0x14849094,0x19495158,0x02828280,0x04c4c0c4,
0x3fcff3fc,0x09494148,0x39093138,0x27476364,0x00c0c0c0,0x0fcfc3cc,0x17c7d3d4,0x3888b0b8,
0x0f0f030c,0x0e8e828c,0x02424240,0x23032320,0x11819190,0x2c4c606c,0x1bcbd3d8,0x2484a0a4,
0x34043034,0x31c1f1f0,0x08484048,0x02c2c2c0,0x2f4f636c,0x3d0d313c,0x2d0d212c,0x00404040,
0x3e8eb2bc,0x3e0e323c,0x3c8cb0bc,0x01c1c1c0,0x2a8aa2a8,0x3a8ab2b8,0x0e4e424c,0x15455154,
0x3b0b3338,0x1cccd0dc,0x28486068,0x3f4f737c,0x1c8c909c,0x18c8d0d8,0x0a4a4248,0x16465254,
0x37477374,0x2080a0a0,0x2dcde1ec,0x06464244,0x3585b1b4,0x2b0b2328,0x25456164,0x3acaf2f8,
0x23c3e3e0,0x3989b1b8,0x3181b1b0,0x1f8f939c,0x1e4e525c,0x39c9f1f8,0x26c6e2e4,0x3282b2b0,
0x31013130,0x2acae2e8,0x2d4d616c,0x1f4f535c,0x24c4e0e4,0x30c0f0f0,0x0dcdc1cc,0x08888088,
0x16061214,0x3a0a3238,0x18485058,0x14c4d0d4,0x22426260,0x29092128,0x07070304,0x33033330,
0x28c8e0e8,0x1b0b1318,0x05050104,0x39497178,0x10809090,0x2a4a6268,0x2a0a2228,0x1a8a9298
},
{
0x38380830,0xe828c8e0,0x2c2d0d21,0xa42686a2,0xcc0fcfc3,0xdc1eced2,0xb03383b3,0xb83888b0,
0xac2f8fa3,0x60204060,0x54154551,0xc407c7c3,0x44044440,0x6c2f4f63,0x682b4b63,0x581b4b53,
0xc003c3c3,0x60224262,0x30330333,0xb43585b1,0x28290921,0xa02080a0,0xe022c2e2,0xa42787a3,
0xd013c3d3,0x90118191,0x10110111,0x04060602,0x1c1c0c10,0xbc3c8cb0,0x34360632,0x480b4b43,
0xec2fcfe3,0x88088880,0x6c2c4c60,0xa82888a0,0x14170713,0xc404c4c0,0x14160612,0xf434c4f0,
0xc002c2c2,0x44054541,0xe021c1e1,0xd416c6d2,0x3c3f0f33,0x3c3d0d31,0x8c0e8e82,0x98188890,
0x28280820,0x4c0e4e42,0xf436c6f2,0x3c3e0e32,0xa42585a1,0xf839c9f1,0x0c0d0d01,0xdc1fcfd3,
0xd818c8d0,0x282b0b23,0x64264662,0x783a4a72,0x24270723,0x2c2f0f23,0xf031c1f1,0x70324272,
0x40024242,0xd414c4d0,0x40014141,0xc000c0c0,0x70334373,0x64274763,0xac2c8ca0,0x880b8b83,
0xf437c7f3,0xac2d8da1,0x80008080,0x1c1f0f13,0xc80acac2,0x2c2c0c20,0xa82a8aa2,0x34340430,
0xd012c2d2,0x080b0b03,0xec2ecee2,0xe829c9e1,0x5c1d4d51,0x94148490,0x18180810,0xf838c8f0,
0x54174753,0xac2e8ea2,0x08080800,0xc405c5c1,0x10130313,0xcc0dcdc1,0x84068682,0xb83989b1,
0xfc3fcff3,0x7c3d4d71,0xc001c1c1,0x30310131,0xf435c5f1,0x880a8a82,0x682a4a62,0xb03181b1,
0xd011c1d1,0x20200020,0xd417c7d3,0x00020202,0x20220222,0x04040400,0x68284860,0x70314171,
0x04070703,0xd81bcbd3,0x9c1d8d91,0x98198991,0x60214161,0xbc3e8eb2,0xe426c6e2,0x58194951,
0xdc1dcdd1,0x50114151,0x90108090,0xdc1cccd0,0x981a8a92,0xa02383a3,0xa82b8ba3,0xd010c0d0,
0x80018181,0x0c0f0f03,0x44074743,0x181a0a12,0xe023c3e3,0xec2ccce0,0x8c0d8d81,0xbc3f8fb3,
0x94168692,0x783b4b73,0x5c1c4c50,0xa02282a2,0xa02181a1,0x60234363,0x20230323,0x4c0d4d41,
0xc808c8c0,0x9c1e8e92,0x9c1c8c90,0x383a0a32,0x0c0c0c00,0x2c2e0e22,0xb83a8ab2,0x6c2e4e62,
0x9c1f8f93,0x581a4a52,0xf032c2f2,0x90128292,0xf033c3f3,0x48094941,0x78384870,0xcc0cccc0,
0x14150511,0xf83bcbf3,0x70304070,0x74354571,0x7c3f4f73,0x34350531,0x10100010,0x00030303,
0x64244460,0x6c2d4d61,0xc406c6c2,0x74344470,0xd415c5d1,0xb43484b0,0xe82acae2,0x08090901,
0x74364672,0x18190911,0xfc3ecef2,0x40004040,0x10120212,0xe020c0e0,0xbc3d8db1,0x04050501,
0xf83acaf2,0x00010101,0xf030c0f0,0x282a0a22,0x5c1e4e52,0xa82989a1,0x54164652,0x40034343,
0x84058581,0x14140410,0x88098981,0x981b8b93,0xb03080b0,0xe425c5e1,0x48084840,0x78394971,
0x94178793,0xfc3cccf0,0x1c1e0e12,0x80028282,0x20210121,0x8c0c8c80,0x181b0b13,0x5c1f4f53,
0x74374773,0x54144450,0xb03282b2,0x1c1d0d11,0x24250521,0x4c0f4f43,0x00000000,0x44064642,
0xec2dcde1,0x58184850,0x50124252,0xe82bcbe3,0x7c3e4e72,0xd81acad2,0xc809c9c1,0xfc3dcdf1,
0x30300030,0x94158591,0x64254561,0x3c3c0c30,0xb43686b2,0xe424c4e0,0xb83b8bb3,0x7c3c4c70,
0x0c0e0e02,0x50104050,0x38390931,0x24260622,0x30320232,0x84048480,0x68294961,0x90138393,
0x34370733,0xe427c7e3,0x24240420,0xa42484a0,0xc80bcbc3,0x50134353,0x080a0a02,0x84078783,
0xd819c9d1,0x4c0c4c40,0x80038383,0x8c0f8f83,0xcc0ecec2,0x383b0b33,0x480a4a42,0xb43787b3
},
{
0xa1a82989,0x81840585,0xd2d416c6,0xd3d013c3,0x50541444,0x111c1d0d,0xa0ac2c8c,0x21242505,
0x515c1d4d,0x43400343,0x10181808,0x121c1e0e,0x51501141,0xf0fc3ccc,0xc2c80aca,0x63602343,
0x20282808,0x40440444,0x20202000,0x919c1d8d,0xe0e020c0,0xe2e022c2,0xc0c808c8,0x13141707,
0xa1a42585,0x838c0f8f,0x03000303,0x73783b4b,0xb3b83b8b,0x13101303,0xd2d012c2,0xe2ec2ece,
0x70703040,0x808c0c8c,0x333c3f0f,0xa0a82888,0x32303202,0xd1dc1dcd,0xf2f436c6,0x70743444,
0xe0ec2ccc,0x91941585,0x03080b0b,0x53541747,0x505c1c4c,0x53581b4b,0xb1bc3d8d,0x01000101,
0x20242404,0x101c1c0c,0x73703343,0x90981888,0x10101000,0xc0cc0ccc,0xf2f032c2,0xd1d819c9,
0x202c2c0c,0xe3e427c7,0x72703242,0x83800383,0x93981b8b,0xd1d011c1,0x82840686,0xc1c809c9,
0x60602040,0x50501040,0xa3a02383,0xe3e82bcb,0x010c0d0d,0xb2b43686,0x929c1e8e,0x434c0f4f,
0xb3b43787,0x52581a4a,0xc2c406c6,0x70783848,0xa2a42686,0x12101202,0xa3ac2f8f,0xd1d415c5,
0x61602141,0xc3c003c3,0xb0b43484,0x41400141,0x52501242,0x717c3d4d,0x818c0d8d,0x00080808,
0x131c1f0f,0x91981989,0x00000000,0x11181909,0x00040404,0x53501343,0xf3f437c7,0xe1e021c1,
0xf1fc3dcd,0x72743646,0x232c2f0f,0x23242707,0xb0b03080,0x83880b8b,0x020c0e0e,0xa3a82b8b,
0xa2a02282,0x626c2e4e,0x93901383,0x414c0d4d,0x61682949,0x707c3c4c,0x01080909,0x02080a0a,
0xb3bc3f8f,0xe3ec2fcf,0xf3f033c3,0xc1c405c5,0x83840787,0x10141404,0xf2fc3ece,0x60642444,
0xd2dc1ece,0x222c2e0e,0x43480b4b,0x12181a0a,0x02040606,0x21202101,0x63682b4b,0x62642646,
0x02000202,0xf1f435c5,0x92901282,0x82880a8a,0x000c0c0c,0xb3b03383,0x727c3e4e,0xd0d010c0,
0x72783a4a,0x43440747,0x92941686,0xe1e425c5,0x22242606,0x80800080,0xa1ac2d8d,0xd3dc1fcf,
0xa1a02181,0x30303000,0x33343707,0xa2ac2e8e,0x32343606,0x11141505,0x22202202,0x30383808,
0xf0f434c4,0xa3a42787,0x41440545,0x404c0c4c,0x81800181,0xe1e829c9,0x80840484,0x93941787,
0x31343505,0xc3c80bcb,0xc2cc0ece,0x303c3c0c,0x71703141,0x11101101,0xc3c407c7,0x81880989,
0x71743545,0xf3f83bcb,0xd2d81aca,0xf0f838c8,0x90941484,0x51581949,0x82800282,0xc0c404c4,
0xf3fc3fcf,0x41480949,0x31383909,0x63642747,0xc0c000c0,0xc3cc0fcf,0xd3d417c7,0xb0b83888,
0x030c0f0f,0x828c0e8e,0x42400242,0x23202303,0x91901181,0x606c2c4c,0xd3d81bcb,0xa0a42484,
0x30343404,0xf1f031c1,0x40480848,0xc2c002c2,0x636c2f4f,0x313c3d0d,0x212c2d0d,0x40400040,
0xb2bc3e8e,0x323c3e0e,0xb0bc3c8c,0xc1c001c1,0xa2a82a8a,0xb2b83a8a,0x424c0e4e,0x51541545,
0x33383b0b,0xd0dc1ccc,0x60682848,0x737c3f4f,0x909c1c8c,0xd0d818c8,0x42480a4a,0x52541646,
0x73743747,0xa0a02080,0xe1ec2dcd,0x42440646,0xb1b43585,0x23282b0b,0x61642545,0xf2f83aca,
0xe3e023c3,0xb1b83989,0xb1b03181,0x939c1f8f,0x525c1e4e,0xf1f839c9,0xe2e426c6,0xb2b03282,
0x31303101,0xe2e82aca,0x616c2d4d,0x535c1f4f,0xe0e424c4,0xf0f030c0,0xc1cc0dcd,0x80880888,
0x12141606,0x32383a0a,0x50581848,0xd0d414c4,0x62602242,0x21282909,0x03040707,0x33303303,
0xe0e828c8,0x13181b0b,0x01040505,0x71783949,0x90901080,0x62682a4a,0x22282a0a,0x92981a8a
},
{
0x08303838,0xc8e0e828,0x0d212c2d,0x86a2a426,0xcfc3cc0f,0xced2dc1e,0x83b3b033,0x88b0b838,
0x8fa3ac2f,0x40606020,0x45515415,0xc7c3c407,0x44404404,0x4f636c2f,0x4b63682b,0x4b53581b,
0xc3c3c003,0x42626022,0x03333033,0x85b1b435,0x09212829,0x80a0a020,0xc2e2e022,0x87a3a427,
0xc3d3d013,0x81919011,0x01111011,0x06020406,0x0c101c1c,0x8cb0bc3c,0x06323436,0x4b43480b,
0xcfe3ec2f,0x88808808,0x4c606c2c,0x88a0a828,0x07131417,0xc4c0c404,0x06121416,0xc4f0f434,
0xc2c2c002,0x45414405,0xc1e1e021,0xc6d2d416,0x0f333c3f,0x0d313c3d,0x8e828c0e,0x88909818,
0x08202828,0x4e424c0e,0xc6f2f436,0x0e323c3e,0x85a1a425,0xc9f1f839,0x0d010c0d,0xcfd3dc1f,
0xc8d0d818,0x0b23282b,0x46626426,0x4a72783a,0x07232427,0x0f232c2f,0xc1f1f031,0x42727032,
0x42424002,0xc4d0d414,0x41414001,0xc0c0c000,0x43737033,0x47636427,0x8ca0ac2c,0x8b83880b,
0xc7f3f437,0x8da1ac2d,0x80808000,0x0f131c1f,0xcac2c80a,0x0c202c2c,0x8aa2a82a,0x04303434,
0xc2d2d012,0x0b03080b,0xcee2ec2e,0xc9e1e829,0x4d515c1d,0x84909414,0x08101818,0xc8f0f838,
0x47535417,0x8ea2ac2e,0x08000808,0xc5c1c405,0x03131013,0xcdc1cc0d,0x86828406,0x89b1b839,
0xcff3fc3f,0x4d717c3d,0xc1c1c001,0x01313031,0xc5f1f435,0x8a82880a,0x4a62682a,0x81b1b031,
0xc1d1d011,0x00202020,0xc7d3d417,0x02020002,0x02222022,0x04000404,0x48606828,0x41717031,
0x07030407,0xcbd3d81b,0x8d919c1d,0x89919819,0x41616021,0x8eb2bc3e,0xc6e2e426,0x49515819,
0xcdd1dc1d,0x41515011,0x80909010,0xccd0dc1c,0x8a92981a,0x83a3a023,0x8ba3a82b,0xc0d0d010,
0x81818001,0x0f030c0f,0x47434407,0x0a12181a,0xc3e3e023,0xcce0ec2c,0x8d818c0d,0x8fb3bc3f,
0x86929416,0x4b73783b,0x4c505c1c,0x82a2a022,0x81a1a021,0x43636023,0x03232023,0x4d414c0d,
0xc8c0c808,0x8e929c1e,0x8c909c1c,0x0a32383a,0x0c000c0c,0x0e222c2e,0x8ab2b83a,0x4e626c2e,
0x8f939c1f,0x4a52581a,0xc2f2f032,0x82929012,0xc3f3f033,0x49414809,0x48707838,0xccc0cc0c,
0x05111415,0xcbf3f83b,0x40707030,0x45717435,0x4f737c3f,0x05313435,0x00101010,0x03030003,
0x44606424,0x4d616c2d,0xc6c2c406,0x44707434,0xc5d1d415,0x84b0b434,0xcae2e82a,0x09010809,
0x46727436,0x09111819,0xcef2fc3e,0x40404000,0x02121012,0xc0e0e020,0x8db1bc3d,0x05010405,
0xcaf2f83a,0x01010001,0xc0f0f030,0x0a22282a,0x4e525c1e,0x89a1a829,0x46525416,0x43434003,
0x85818405,0x04101414,0x89818809,0x8b93981b,0x80b0b030,0xc5e1e425,0x48404808,0x49717839,
0x87939417,0xccf0fc3c,0x0e121c1e,0x82828002,0x01212021,0x8c808c0c,0x0b13181b,0x4f535c1f,
0x47737437,0x44505414,0x82b2b032,0x0d111c1d,0x05212425,0x4f434c0f,0x00000000,0x46424406,
0xcde1ec2d,0x48505818,0x42525012,0xcbe3e82b,0x4e727c3e,0xcad2d81a,0xc9c1c809,0xcdf1fc3d,
0x00303030,0x85919415,0x45616425,0x0c303c3c,0x86b2b436,0xc4e0e424,0x8bb3b83b,0x4c707c3c,
0x0e020c0e,0x40505010,0x09313839,0x06222426,0x02323032,0x84808404,0x49616829,0x83939013,
0x07333437,0xc7e3e427,0x04202424,0x84a0a424,0xcbc3c80b,0x43535013,0x0a02080a,0x87838407,
0xc9d1d819,0x4c404c0c,0x83838003,0x8f838c0f,0xcec2cc0e,0x0b33383b,0x4a42480a,0x87b3b437
}
};
#define SEED_G(x) (SEED_SS[0][(x)&0xff]^SEED_SS[1][((x)>>8)&0xff]^SEED_SS[2][((x)>>16)&0xff]^SEED_SS[3][((x)>>24)&0xff])
#define SEED_RL(x,n) ((uint32_t)(((x)<<(n))|((x)>>(32-(n)))))
typedef struct { uint32_t rk[32]; } seed_ctx;
static void seed_key(const unsigned char key[16], seed_ctx *c){
  uint32_t A=(key[0]<<24)|(key[1]<<16)|(key[2]<<8)|key[3], B=(key[4]<<24)|(key[5]<<16)|(key[6]<<8)|key[7];
  uint32_t C=(key[8]<<24)|(key[9]<<16)|(key[10]<<8)|key[11], D=(key[12]<<24)|(key[13]<<16)|(key[14]<<8)|key[15];
  for(int i=0;i<16;i++){ uint32_t KC=SEED_RL(0x9e3779b9,i), t0=A+C-KC, t1=B-D+KC;
    c->rk[2*i]=SEED_G(t0); c->rk[2*i+1]=SEED_G(t1);
    if((i&1)==0){ uint32_t t=A; A=(A>>8)|(B<<24); B=(B>>8)|(t<<24); }
    else { uint32_t t=C; C=(C<<8)|(D>>24); D=(D<<8)|(t>>24); } }
}
#if defined(__aarch64__) && defined(__APPLE__)
extern void seed_core_asm(const uint32_t *rk, const uint32_t *SS, uint32_t io[4]);
#endif
static void seed_block(const seed_ctx *c, const unsigned char in[16], unsigned char out[16], int encrypt){
#if defined(__aarch64__) && defined(__APPLE__)
  uint32_t rk[32],io[4]; int r,j;
  for(r=0;r<16;r++){ int i=encrypt?r:15-r; rk[2*r]=c->rk[2*i]; rk[2*r+1]=c->rk[2*i+1]; }
  for(j=0;j<4;j++) io[j]=((uint32_t)in[4*j]<<24)|((uint32_t)in[4*j+1]<<16)|((uint32_t)in[4*j+2]<<8)|in[4*j+3];
  seed_core_asm(rk,&SEED_SS[0][0],io);
  for(j=0;j<4;j++){ out[4*j]=(unsigned char)(io[j]>>24);out[4*j+1]=(unsigned char)(io[j]>>16);out[4*j+2]=(unsigned char)(io[j]>>8);out[4*j+3]=(unsigned char)io[j]; }
#else
  uint32_t L0=((uint32_t)in[0]<<24)|(in[1]<<16)|(in[2]<<8)|in[3], L1=((uint32_t)in[4]<<24)|(in[5]<<16)|(in[6]<<8)|in[7];
  uint32_t R0=((uint32_t)in[8]<<24)|(in[9]<<16)|(in[10]<<8)|in[11], R1=((uint32_t)in[12]<<24)|(in[13]<<16)|(in[14]<<8)|in[15];
  int r; for(r=0;r<16;r++){ int i=encrypt?r:15-r;
    uint32_t T0=R0^c->rk[2*i], T1=R1^c->rk[2*i+1];
    T1^=T0; T1=SEED_G(T1); T0+=T1; T0=SEED_G(T0); T1+=T0; T1=SEED_G(T1); T0+=T1;
    uint32_t n0=L0^T0,n1=L1^T1; L0=R0;L1=R1;R0=n0;R1=n1; }
  uint32_t y[4]={R0,R1,L0,L1}; int j;
  for(j=0;j<4;j++){ out[4*j]=(unsigned char)(y[j]>>24);out[4*j+1]=(unsigned char)(y[j]>>16);out[4*j+2]=(unsigned char)(y[j]>>8);out[4*j+3]=(unsigned char)y[j]; }
#endif
}
void rktcrypto_seed_ecb(const unsigned char key[16], const unsigned char *in, unsigned char *out,
                        intptr_t nblk, int encrypt){
  seed_ctx c; intptr_t n; seed_key(key,&c);
  for(n=0;n<nblk;n++) seed_block(&c,in+16*n,out+16*n,encrypt);
}
void rktcrypto_seed_cbc(const unsigned char key[16], const unsigned char iv[16], const unsigned char *in,
                        unsigned char *out, intptr_t nblk, int encrypt){
  seed_ctx c; intptr_t n; unsigned char prev[16]; int q; seed_key(key,&c); memcpy(prev,iv,16);
  if(encrypt){ for(n=0;n<nblk;n++){ unsigned char t[16]; for(q=0;q<16;q++)t[q]=(unsigned char)(in[16*n+q]^prev[q]);
      seed_block(&c,t,out+16*n,1); memcpy(prev,out+16*n,16); } }
  else { for(n=0;n<nblk;n++){ unsigned char cc[16],p[16]; memcpy(cc,in+16*n,16); seed_block(&c,cc,p,0);
      for(q=0;q<16;q++)out[16*n+q]=(unsigned char)(p[q]^prev[q]); memcpy(prev,cc,16); } }
}


/* =============================== RC4 =============================== */

void rktcrypto_rc4(const unsigned char *key, intptr_t keylen,
                   const unsigned char *in, unsigned char *out, intptr_t len){
  unsigned char s[256]; int i,j=0; unsigned char a=0,b=0; intptr_t n;
  for(i=0;i<256;i++) s[i]=(unsigned char)i;
  for(i=0;i<256;i++){ j=(j+s[i]+key[i%keylen])&0xFF; { unsigned char t=s[i]; s[i]=s[j]; s[j]=t; } }
  /* uint8_t indices wrap for free -- no &0xFF on the hot keystream path.
     8x unrolled to amortize loop overhead over the serial state machine
     (matches OpenSSL's rc4_enc.c 8-way LOOP unroll). */
  n=0;
#define RC4_STEP(o) do{ unsigned char sa,sb; a=(unsigned char)(a+1); sa=s[a]; \
    b=(unsigned char)(b+sa); sb=s[b]; s[a]=sb; s[b]=sa; \
    out[n+(o)]=in[n+(o)]^s[(unsigned char)(sa+sb)]; }while(0)
  for(; n+8<=len; n+=8){ RC4_STEP(0);RC4_STEP(1);RC4_STEP(2);RC4_STEP(3);
    RC4_STEP(4);RC4_STEP(5);RC4_STEP(6);RC4_STEP(7); }
  for(; n<len; n++) RC4_STEP(0);
#undef RC4_STEP
}

/* ============================ Camellia (RFC 3713) ============================
   128-bit block, 128/192/256-bit keys. Not broken, but legacy relative to AES;
   included for TLS/CMS interoperability. */

static const unsigned char CAM_SBOX1[256]={
};

typedef unsigned __int128 cam_u128;
static unsigned char cam_rol8(unsigned char x,int n){ return (unsigned char)((x<<n)|(x>>(8-n))); }
static const uint64_t CAM_SIG[6]={0xA09E667F3BCC908BULL,0xB67AE8584CAA73B2ULL,0xC6EF372FE94F82BEULL,
                                  0x54FF53A5F1D36F1CULL,0x10E527FADE682D1DULL,0xB05688C2B3E6C1FDULL};
static uint64_t cam_load64(const unsigned char *p){ uint64_t v; memcpy(&v,p,8); return __builtin_bswap64(v); }
static void cam_store64(unsigned char *p,uint64_t v){ v=__builtin_bswap64(v); memcpy(p,&v,8); }

/* SP tables fuse each byte's S-box variant with the linear P-layer, so cam_F is
   8 XORed 64-bit lookups. Generated once by evaluating the exact z-formulas with
   a single active y (guaranteed bit-identical to the byte-wise version). */
static uint64_t CAM_SP[8][256];
static int cam_sp_inited=0;
static void cam_sp_init(void){
  int j,b,i;
  for(j=0;j<8;j++) for(b=0;b<256;b++){
    unsigned char y[8]={0,0,0,0,0,0,0,0}, z[8]; uint64_t r=0;
    switch(j){
      case 0: y[0]=CAM_SBOX1[b]; break;
      case 1: y[1]=cam_rol8(CAM_SBOX1[b],1); break;
      case 2: y[2]=cam_rol8(CAM_SBOX1[b],7); break;
      case 3: y[3]=CAM_SBOX1[cam_rol8((unsigned char)b,1)]; break;
      case 4: y[4]=cam_rol8(CAM_SBOX1[b],1); break;
      case 5: y[5]=cam_rol8(CAM_SBOX1[b],7); break;
      case 6: y[6]=CAM_SBOX1[cam_rol8((unsigned char)b,1)]; break;
      case 7: y[7]=CAM_SBOX1[b]; break;
    }
    z[0]=y[0]^y[2]^y[3]^y[5]^y[6]^y[7]; z[1]=y[0]^y[1]^y[3]^y[4]^y[6]^y[7];
    z[2]=y[0]^y[1]^y[2]^y[4]^y[5]^y[7]; z[3]=y[1]^y[2]^y[3]^y[4]^y[5]^y[6];
    z[4]=y[0]^y[1]^y[5]^y[6]^y[7]; z[5]=y[1]^y[2]^y[4]^y[6]^y[7];
    z[6]=y[2]^y[3]^y[4]^y[5]^y[7]; z[7]=y[0]^y[3]^y[4]^y[5]^y[6];
    for(i=0;i<8;i++) r=(r<<8)|z[i];
    CAM_SP[j][b]=r;
  }
  cam_sp_inited=1;
}
static uint64_t cam_F(uint64_t X,uint64_t k){
  uint64_t u=X^k;
  return CAM_SP[0][(u>>56)&0xff]^CAM_SP[1][(u>>48)&0xff]^CAM_SP[2][(u>>40)&0xff]^CAM_SP[3][(u>>32)&0xff]
        ^CAM_SP[4][(u>>24)&0xff]^CAM_SP[5][(u>>16)&0xff]^CAM_SP[6][(u>>8)&0xff]^CAM_SP[7][u&0xff];
}
static uint64_t cam_rol32(uint64_t x,int n){ uint32_t v=(uint32_t)x; return (uint64_t)((v<<n)|(v>>(32-n))); }
static uint64_t cam_FL(uint64_t X,uint64_t ke){ uint32_t x1=(uint32_t)(X>>32),x2=(uint32_t)X,k1=(uint32_t)(ke>>32),k2=(uint32_t)ke;
  x2^=(uint32_t)cam_rol32(x1&k1,1); x1^=(x2|k2); return ((uint64_t)x1<<32)|x2; }
static uint64_t cam_FLINV(uint64_t Y,uint64_t ke){ uint32_t y1=(uint32_t)(Y>>32),y2=(uint32_t)Y,k1=(uint32_t)(ke>>32),k2=(uint32_t)ke;
  y1^=(y2|k2); y2^=(uint32_t)cam_rol32(y1&k1,1); return ((uint64_t)y1<<32)|y2; }
static cam_u128 cam_rol128(cam_u128 x,int n){ return (x<<n)|(x>>(128-n)); }
static uint64_t CAM_HI(cam_u128 x){ return (uint64_t)(x>>64); }
static uint64_t CAM_LO(cam_u128 x){ return (uint64_t)x; }

typedef struct { uint64_t kw[4],k[24],ke[6]; int nr; } cam_key;
static void cam_schedule(const unsigned char *key,int keylen,cam_key *ck){
  cam_u128 KL,KR=0,KA,KB=0; uint64_t D1,D2;
  if(!cam_sp_inited) cam_sp_init();
  KL=((cam_u128)cam_load64(key)<<64)|cam_load64(key+8);
  if(keylen==24){ uint64_t hi=cam_load64(key+16); KR=((cam_u128)hi<<64)|(uint64_t)(~hi); }
  else if(keylen==32){ KR=((cam_u128)cam_load64(key+16)<<64)|cam_load64(key+24); }
  D1=CAM_HI(KL^KR); D2=CAM_LO(KL^KR);
  D2^=cam_F(D1,CAM_SIG[0]); D1^=cam_F(D2,CAM_SIG[1]); D1^=CAM_HI(KL); D2^=CAM_LO(KL);
  D2^=cam_F(D1,CAM_SIG[2]); D1^=cam_F(D2,CAM_SIG[3]); KA=((cam_u128)D1<<64)|D2;
  if(keylen>16){ D1=CAM_HI(KA^KR); D2=CAM_LO(KA^KR); D2^=cam_F(D1,CAM_SIG[4]); D1^=cam_F(D2,CAM_SIG[5]); KB=((cam_u128)D1<<64)|D2; }
  if(keylen==16){ ck->nr=18;
    ck->kw[0]=CAM_HI(KL); ck->kw[1]=CAM_LO(KL);
    ck->k[0]=CAM_HI(KA); ck->k[1]=CAM_LO(KA);
    ck->k[2]=CAM_HI(cam_rol128(KL,15)); ck->k[3]=CAM_LO(cam_rol128(KL,15));
    ck->k[4]=CAM_HI(cam_rol128(KA,15)); ck->k[5]=CAM_LO(cam_rol128(KA,15));
    ck->ke[0]=CAM_HI(cam_rol128(KA,30)); ck->ke[1]=CAM_LO(cam_rol128(KA,30));
    ck->k[6]=CAM_HI(cam_rol128(KL,45)); ck->k[7]=CAM_LO(cam_rol128(KL,45));
    ck->k[8]=CAM_HI(cam_rol128(KA,45)); ck->k[9]=CAM_LO(cam_rol128(KL,60));
    ck->k[10]=CAM_HI(cam_rol128(KA,60)); ck->k[11]=CAM_LO(cam_rol128(KA,60));
    ck->ke[2]=CAM_HI(cam_rol128(KL,77)); ck->ke[3]=CAM_LO(cam_rol128(KL,77));
    ck->k[12]=CAM_HI(cam_rol128(KL,94)); ck->k[13]=CAM_LO(cam_rol128(KL,94));
    ck->k[14]=CAM_HI(cam_rol128(KA,94)); ck->k[15]=CAM_LO(cam_rol128(KA,94));
    ck->k[16]=CAM_HI(cam_rol128(KL,111)); ck->k[17]=CAM_LO(cam_rol128(KL,111));
    ck->kw[2]=CAM_HI(cam_rol128(KA,111)); ck->kw[3]=CAM_LO(cam_rol128(KA,111));
  } else { ck->nr=24;
    ck->kw[0]=CAM_HI(KL); ck->kw[1]=CAM_LO(KL);
    ck->k[0]=CAM_HI(KB); ck->k[1]=CAM_LO(KB);
    ck->k[2]=CAM_HI(cam_rol128(KR,15)); ck->k[3]=CAM_LO(cam_rol128(KR,15));
    ck->k[4]=CAM_HI(cam_rol128(KA,15)); ck->k[5]=CAM_LO(cam_rol128(KA,15));
    ck->ke[0]=CAM_HI(cam_rol128(KR,30)); ck->ke[1]=CAM_LO(cam_rol128(KR,30));
    ck->k[6]=CAM_HI(cam_rol128(KB,30)); ck->k[7]=CAM_LO(cam_rol128(KB,30));
    ck->k[8]=CAM_HI(cam_rol128(KL,45)); ck->k[9]=CAM_LO(cam_rol128(KL,45));
    ck->k[10]=CAM_HI(cam_rol128(KA,45)); ck->k[11]=CAM_LO(cam_rol128(KA,45));
    ck->ke[2]=CAM_HI(cam_rol128(KL,60)); ck->ke[3]=CAM_LO(cam_rol128(KL,60));
    ck->k[12]=CAM_HI(cam_rol128(KR,60)); ck->k[13]=CAM_LO(cam_rol128(KR,60));
    ck->k[14]=CAM_HI(cam_rol128(KB,60)); ck->k[15]=CAM_LO(cam_rol128(KB,60));
    ck->k[16]=CAM_HI(cam_rol128(KL,77)); ck->k[17]=CAM_LO(cam_rol128(KL,77));
    ck->ke[4]=CAM_HI(cam_rol128(KA,77)); ck->ke[5]=CAM_LO(cam_rol128(KA,77));
    ck->k[18]=CAM_HI(cam_rol128(KR,94)); ck->k[19]=CAM_LO(cam_rol128(KR,94));
    ck->k[20]=CAM_HI(cam_rol128(KA,94)); ck->k[21]=CAM_LO(cam_rol128(KA,94));
    ck->k[22]=CAM_HI(cam_rol128(KL,111)); ck->k[23]=CAM_LO(cam_rol128(KL,111));
    ck->kw[2]=CAM_HI(cam_rol128(KB,111)); ck->kw[3]=CAM_LO(cam_rol128(KB,111));
  }
}
/* Core on register words: takes the two big-endian block words, returns the two
   output words (already in output order). Lets CBC keep the chaining value in
   registers -- no per-block byte-wise fold or memcpy on the critical path. */
static void cam_crypt_words(const cam_key *ck,uint64_t *w0,uint64_t *w1){
  uint64_t D1=*w0,D2=*w1; int stage,fl=0,ns=ck->nr/6;
  const uint64_t *k=ck->k;
  D1^=ck->kw[0]; D2^=ck->kw[1];
  for(stage=0;stage<ns;stage++){
    /* six feistel rounds unrolled: static D1/D2 alternation (no i%2 branch),
       immediate-offset subkey loads instead of a running index. */
    D2^=cam_F(D1,k[0]); D1^=cam_F(D2,k[1]); D2^=cam_F(D1,k[2]);
    D1^=cam_F(D2,k[3]); D2^=cam_F(D1,k[4]); D1^=cam_F(D2,k[5]); k+=6;
    if(stage<ns-1){ D1=cam_FL(D1,ck->ke[fl*2]); D2=cam_FLINV(D2,ck->ke[fl*2+1]); fl++; }
  }
  D2^=ck->kw[2]; D1^=ck->kw[3];
  *w0=D2; *w1=D1;
}
static void cam_crypt_block(const cam_key *ck,const unsigned char in[16],unsigned char out[16]){
  uint64_t w0=cam_load64(in),w1=cam_load64(in+8);
  cam_crypt_words(ck,&w0,&w1);
  cam_store64(out,w0); cam_store64(out+8,w1);
}
static void cam_reverse_key(const cam_key *ck,cam_key *d){
  int i,ne=ck->nr/6-1; *d=*ck;
  d->kw[0]=ck->kw[2]; d->kw[1]=ck->kw[3]; d->kw[2]=ck->kw[0]; d->kw[3]=ck->kw[1];
  for(i=0;i<ck->nr;i++) d->k[i]=ck->k[ck->nr-1-i];
  for(i=0;i<ne;i++){ d->ke[2*i]=ck->ke[2*(ne-1-i)+1]; d->ke[2*i+1]=ck->ke[2*(ne-1-i)]; }
}

void rktcrypto_camellia_ecb(const unsigned char *key,intptr_t keylen,const unsigned char *in,
                            unsigned char *out,intptr_t nblk,int encrypt){
  cam_key ck,d; intptr_t i; cam_schedule(key,(int)keylen,&ck);
  if(!encrypt){ cam_reverse_key(&ck,&d); ck=d; }
  for(i=0;i<nblk;i++) cam_crypt_block(&ck,in+16*i,out+16*i);
}
void rktcrypto_camellia_cbc(const unsigned char *key,intptr_t keylen,const unsigned char iv[16],
                            const unsigned char *in,unsigned char *out,intptr_t nblk,int encrypt){
  cam_key ck,d; unsigned char prev[16],tmp[16]; intptr_t i; int j;
  cam_schedule(key,(int)keylen,&ck); memcpy(prev,iv,16);
  if(encrypt){
    uint64_t p0=cam_load64(prev),p1=cam_load64(prev+8);   /* chaining value stays in registers */
    for(i=0;i<nblk;i++){ uint64_t w0=cam_load64(in+16*i)^p0, w1=cam_load64(in+16*i+8)^p1;
      cam_crypt_words(&ck,&w0,&w1); cam_store64(out+16*i,w0); cam_store64(out+16*i+8,w1); p0=w0; p1=w1; }
    (void)tmp;(void)j;
  } else {
    cam_reverse_key(&ck,&d); ck=d;
    for(i=0;i<nblk;i++){ unsigned char c[16]; memcpy(c,in+16*i,16); cam_crypt_block(&ck,c,tmp);
      for(j=0;j<16;j++) out[16*i+j]=tmp[j]^prev[j]; memcpy(prev,c,16); }
  }
}
