/* ML-DSA (Dilithium) parameterized implementation template, FIPS 204.

   Included once per parameter set from a thin wrapper .c that defines,
   before the include: MLD_K, MLD_L, MLD_ETA, MLD_TAU, MLD_BETA,
   MLD_GAMMA1, MLD_GAMMA2, MLD_OMEGA, MLD_CTILDE, and MLD_NAME(x) ->
   rktcrypto_mldsa<lvl>_##x. Each wrapper is a separate translation unit,
   so the static helpers never collide. Level-specific bit-packing and the
   decompose/hint/eta-sampling routines are selected below with #if on the
   parameter values. From scratch, no external code. */
...
#define DIL_K MLD_K
#define DIL_L MLD_L
#define DIL_ETA MLD_ETA
#define DIL_TAU MLD_TAU
#define DIL_BETA MLD_BETA
#define DIL_GAMMA1 MLD_GAMMA1
#define DIL_GAMMA2 MLD_GAMMA2
#define DIL_OMEGA MLD_OMEGA
#define DIL_CTILDEBYTES MLD_CTILDE
...
#define DIL_SECRETKEYBYTES (2*DIL_SEEDBYTES + DIL_TRBYTES \
  + DIL_L*DIL_POLYETA_PACKEDBYTES + DIL_K*DIL_POLYETA_PACKEDBYTES + DIL_K*DIL_POLYT0_PACKEDBYTES)
#define DIL_SIGBYTES (DIL_CTILDEBYTES + DIL_L*DIL_POLYZ_PACKEDBYTES + DIL_POLYVECH_PACKEDBYTES)

#define DIL_MONT (-4186625)
...
static int32_t montgomery_reduce(int64_t a){
  int32_t t; t=(int32_t)((uint64_t)a*(uint32_t)DIL_QINV); t=(int32_t)((a-(int64_t)t*DIL_Q)>>32); return t;
}
static int32_t reduce32(int32_t a){ int32_t t; t=(a+(1<<22))>>23; t=a-t*DIL_Q; return t; }
...
static void ntt(int32_t a[DIL_N]){
  ...
  for(len=128;len>0;len>>=1) for(start=0;start<DIL_N;start=j+len){
    zeta=zetas[++k];
    for(j=start;j<start+len;j++){ t=montgomery_reduce((int64_t)zeta*a[j+len]); a[j+len]=a[j]-t; a[j]=a[j]+t; }
  }
}
static void invntt_tomont(int32_t a[DIL_N]){
  unsigned int start,len,j,k=256; int32_t t,zeta; const int32_t f=41978;
  for(len=1;len<DIL_N;len<<=1) for(start=0;start<DIL_N;start=j+len){
    zeta=-zetas[--k];
    for(j=start;j<start+len;j++){ t=a[j]; a[j]=t+a[j+len]; a[j+len]=t-a[j+len]; a[j+len]=montgomery_reduce((int64_t)zeta*a[j+len]); }
  }
...
static int32_t power2round(int32_t *a0,int32_t a){
  int32_t a1; a1=(a+(1<<(DIL_D-1))-1)>>DIL_D; *a0=a-(a1<<DIL_D); return a1;
}
/* ---- decompose / use_hint: gamma2-dependent ---- */
#if DIL_GAMMA2 == ((DIL_Q - 1) / 88)
static int32_t decompose(int32_t *a0,int32_t a){
  int32_t a1;
  a1=(a+127)>>7; a1=(a1*11275+(1<<23))>>24; a1^=((43-a1)>>31)&a1;
  *a0=a-a1*2*DIL_GAMMA2; *a0-=(((DIL_Q-1)/2-*a0)>>31)&DIL_Q; return a1;
}
static int32_t use_hint(int32_t a,unsigned int hint){
  int32_t a0,a1; a1=decompose(&a0,a);
  if(hint==0) return a1;
...
static int32_t decompose(int32_t *a0,int32_t a){
  int32_t a1;
  a1=(a+127)>>7; a1=(a1*1025+(1<<21))>>22; a1&=15;
  *a0=a-a1*2*DIL_GAMMA2; *a0-=(((DIL_Q-1)/2-*a0)>>31)&DIL_Q; return a1;
}
static int32_t use_hint(int32_t a,unsigned int hint){
  int32_t a0,a1; a1=decompose(&a0,a);
...
static void poly_power2round(poly *a1,poly *a0,const poly *a){ int i; for(i=0;i<DIL_N;i++) a1->coeffs[i]=power2round(&a0->coeffs[i],a->coeffs[i]); }
static void poly_decompose(poly *a1,poly *a0,const poly *a){ int i; for(i=0;i<DIL_N;i++) a1->coeffs[i]=decompose(&a0->coeffs[i],a->coeffs[i]); }
static unsigned int poly_make_hint(poly *h,const poly *a0,const poly *a1){
  int i; unsigned int s=0; for(i=0;i<DIL_N;i++){ h->coeffs[i]=make_hint(a0->coeffs[i],a1->coeffs[i]); s+=h->coeffs[i]; } return s;
}
static void poly_use_hint(poly *b,const poly *a,const poly *h){ int i; for(i=0;i<DIL_N;i++) b->coeffs[i]=use_hint(a->coeffs[i],h->coeffs[i]); }
static int poly_chknorm(const poly *a,int32_t B){
  int i; int32_t t; if(B>(DIL_Q-1)/8) return 1;
  for(i=0;i<DIL_N;i++){ t=a->coeffs[i]>>31; t=a->coeffs[i]-(t&2*a->coeffs[i]); if(t>=B) return 1; }
  return 0;
}
...
static unsigned int rej_uniform(int32_t *a,unsigned int len,const unsigned char *buf,unsigned int buflen){
  ...
  while(ctr<len && pos+3<=buflen){
    t=buf[pos++]; t|=(uint32_t)buf[pos++]<<8; t|=(uint32_t)buf[pos++]<<16; t&=0x7FFFFF;
    if(t<DIL_Q) a[ctr++]=t;
  }
  return ctr;
}
static void poly_uniform(poly *a,const unsigned char rho[DIL_SEEDBYTES],uint16_t nonce){
  unsigned char seed[DIL_SEEDBYTES+2]; unsigned char buf[168*6];
  memcpy(seed,rho,DIL_SEEDBYTES); seed[DIL_SEEDBYTES]=(unsigned char)(nonce&0xFF); seed[DIL_SEEDBYTES+1]=(unsigned char)(nonce>>8);
  shake128(buf,sizeof(buf),seed,DIL_SEEDBYTES+2); rej_uniform(a->coeffs,DIL_N,buf,sizeof(buf));
}
/* ---- eta sampling: eta-dependent ---- */
#if DIL_ETA == 2
static unsigned int rej_eta(int32_t *a,unsigned int len,const unsigned char *buf,unsigned int buflen){
  ...
  while(ctr<len && pos<buflen){
    ...
    if(t0<15){ t0=t0-(205*t0>>10)*5; a[ctr++]=2-(int32_t)t0; }
    if(t1<15 && ctr<len){ t1=t1-(205*t1>>10)*5; a[ctr++]=2-(int32_t)t1; }
  }
  return ctr;
}
#else
...
static void poly_uniform_eta(poly *a,const unsigned char seed[DIL_CRHBYTES],uint16_t nonce){
  unsigned char in[DIL_CRHBYTES+2]; unsigned char buf[136*3];
  memcpy(in,seed,DIL_CRHBYTES); in[DIL_CRHBYTES]=(unsigned char)(nonce&0xFF); in[DIL_CRHBYTES+1]=(unsigned char)(nonce>>8);
  shake256(buf,sizeof(buf),in,DIL_CRHBYTES+2); rej_eta(a->coeffs,DIL_N,buf,sizeof(buf));
}
...
static void poly_uniform_gamma1(poly *a,const unsigned char seed[DIL_CRHBYTES],uint16_t nonce){
  ...
  memcpy(in,seed,DIL_CRHBYTES); in[DIL_CRHBYTES]=(unsigned char)(nonce&0xFF); in[DIL_CRHBYTES+1]=(unsigned char)(nonce>>8);
  shake256(buf,sizeof(buf),in,DIL_CRHBYTES+2); polyz_unpack(a,buf);
}
static void poly_challenge(poly *c,const unsigned char seed[DIL_CTILDEBYTES]){
  unsigned int i,b,pos; uint64_t signs; unsigned char buf[136*5];
  shake256(buf,sizeof(buf),seed,DIL_CTILDEBYTES);
  signs=0; for(i=0;i<8;i++) signs|=(uint64_t)buf[i]<<(8*i); pos=8;
  for(i=0;i<DIL_N;i++) c->coeffs[i]=0;
  for(i=DIL_N-DIL_TAU;i<DIL_N;i++){
    do { b=buf[pos++]; } while(b>i);
    c->coeffs[i]=c->coeffs[b]; c->coeffs[b]=1-2*(int32_t)(signs&1); signs>>=1;
  }
}

/* ---- eta packing: eta-dependent ---- */
#if DIL_ETA == 2
static void polyeta_pack(unsigned char *r,const poly *a){
  int i; unsigned char t[8];
  for(i=0;i<DIL_N/8;i++){
    int j; for(j=0;j<8;j++) t[j]=(unsigned char)(DIL_ETA-a->coeffs[8*i+j]);
    r[3*i+0]=(unsigned char)(t[0]|(t[1]<<3)|(t[2]<<6));
    r[3*i+1]=(unsigned char)((t[2]>>2)|(t[3]<<1)|(t[4]<<4)|(t[5]<<7));
    r[3*i+2]=(unsigned char)((t[5]>>1)|(t[6]<<2)|(t[7]<<5));
  }
}
static void polyeta_unpack(poly *r,const unsigned char *a){
  ...
  for(i=0;i<DIL_N/8;i++){
    ...
    { int j; for(j=0;j<8;j++) r->coeffs[8*i+j]=DIL_ETA-r->coeffs[8*i+j]; }
  }
}
#else
static void polyeta_pack(unsigned char *r,const poly *a){
  int i; unsigned char t0,t1;
  for(i=0;i<DIL_N/2;i++){ t0=(unsigned char)(DIL_ETA-a->coeffs[2*i+0]); t1=(unsigned char)(DIL_ETA-a->coeffs[2*i+1]); r[i]=(unsigned char)(t0|(t1<<4)); }
}
static void polyeta_unpack(poly *r,const unsigned char *a){
  int i;
  for(i=0;i<DIL_N/2;i++){ r->coeffs[2*i+0]=a[i]&0x0F; r->coeffs[2*i+1]=a[i]>>4; r->coeffs[2*i+0]=DIL_ETA-r->coeffs[2*i+0]; r->coeffs[2*i+1]=DIL_ETA-r->coeffs[2*i+1]; }
}
#endif
static void polyt1_pack(unsigned char *r,const poly *a){
  int i; for(i=0;i<DIL_N/4;i++){
...
static void polyt0_pack(unsigned char *r,const poly *a){
  int i; uint32_t t[8];
  for(i=0;i<DIL_N/8;i++){
    int j; for(j=0;j<8;j++) t[j]=(1<<(DIL_D-1))-a->coeffs[8*i+j];
...
static void polyt0_unpack(poly *r,const unsigned char *a){
  int i; for(i=0;i<DIL_N/8;i++){
    r->coeffs[8*i+0]=a[13*i+0]; r->coeffs[8*i+0]|=(uint32_t)a[13*i+1]<<8; r->coeffs[8*i+0]&=0x1FFF;
    r->coeffs[8*i+1]=a[13*i+1]>>5; r->coeffs[8*i+1]|=(uint32_t)a[13*i+2]<<3; r->coeffs[8*i+1]|=(uint32_t)a[13*i+3]<<11; r->coeffs[8*i+1]&=0x1FFF;
    r->coeffs[8*i+2]=a[13*i+3]>>2; r->coeffs[8*i+2]|=(uint32_t)a[13*i+4]<<6; r->coeffs[8*i+2]&=0x1FFF;
    r->coeffs[8*i+3]=a[13*i+4]>>7; r->coeffs[8*i+3]|=(uint32_t)a[13*i+5]<<1; r->coeffs[8*i+3]|=(uint32_t)a[13*i+6]<<9; r->coeffs[8*i+3]&=0x1FFF;
    r->coeffs[8*i+4]=a[13*i+6]>>4; r->coeffs[8*i+4]|=(uint32_t)a[13*i+7]<<4; r->coeffs[8*i+4]|=(uint32_t)a[13*i+8]<<12; r->coeffs[8*i+4]&=0x1FFF;
    r->coeffs[8*i+5]=a[13*i+8]>>1; r->coeffs[8*i+5]|=(uint32_t)a[13*i+9]<<7; r->coeffs[8*i+5]&=0x1FFF;
    r->coeffs[8*i+6]=a[13*i+9]>>6; r->coeffs[8*i+6]|=(uint32_t)a[13*i+10]<<2; r->coeffs[8*i+6]|=(uint32_t)a[13*i+11]<<10; r->coeffs[8*i+6]&=0x1FFF;
    r->coeffs[8*i+7]=a[13*i+11]>>3; r->coeffs[8*i+7]|=(uint32_t)a[13*i+12]<<5; r->coeffs[8*i+7]&=0x1FFF;
    { int j; for(j=0;j<8;j++) r->coeffs[8*i+j]=(1<<(DIL_D-1))-r->coeffs[8*i+j]; }
  }
}
/* ---- z packing: gamma1-dependent ---- */
#if DIL_GAMMA1 == (1 << 17)
static void polyz_pack(unsigned char *r,const poly *a){
  int i; uint32_t t[4];
  for(i=0;i<DIL_N/4;i++){
    int j; for(j=0;j<4;j++) t[j]=DIL_GAMMA1-a->coeffs[4*i+j];
    r[9*i+0]=(unsigned char)(t[0]);
    r[9*i+1]=(unsigned char)(t[0]>>8);
    r[9*i+2]=(unsigned char)((t[0]>>16)|(t[1]<<2));
    r[9*i+3]=(unsigned char)(t[1]>>6);
    r[9*i+4]=(unsigned char)((t[1]>>14)|(t[2]<<4));
    r[9*i+5]=(unsigned char)(t[2]>>4);
    r[9*i+6]=(unsigned char)((t[2]>>12)|(t[3]<<6));
...
static void polyz_unpack(poly *r,const unsigned char *a){
  int i; for(i=0;i<DIL_N/4;i++){
    r->coeffs[4*i+0]=a[9*i+0]; r->coeffs[4*i+0]|=(uint32_t)a[9*i+1]<<8; r->coeffs[4*i+0]|=(uint32_t)a[9*i+2]<<16; r->coeffs[4*i+0]&=0x3FFFF;
    r->coeffs[4*i+1]=a[9*i+2]>>2; r->coeffs[4*i+1]|=(uint32_t)a[9*i+3]<<6; r->coeffs[4*i+1]|=(uint32_t)a[9*i+4]<<14; r->coeffs[4*i+1]&=0x3FFFF;
    r->coeffs[4*i+2]=a[9*i+4]>>4; r->coeffs[4*i+2]|=(uint32_t)a[9*i+5]<<4; r->coeffs[4*i+2]|=(uint32_t)a[9*i+6]<<12; r->coeffs[4*i+2]&=0x3FFFF;
    r->coeffs[4*i+3]=a[9*i+6]>>6; r->coeffs[4*i+3]|=(uint32_t)a[9*i+7]<<2; r->coeffs[4*i+3]|=(uint32_t)a[9*i+8]<<10; r->coeffs[4*i+3]&=0x3FFFF;
    { int j; for(j=0;j<4;j++) r->coeffs[4*i+j]=DIL_GAMMA1-r->coeffs[4*i+j]; }
  }
}
#else
static void polyz_pack(unsigned char *r,const poly *a){
  int i; uint32_t t[2];
  for(i=0;i<DIL_N/2;i++){
    t[0]=DIL_GAMMA1-a->coeffs[2*i+0]; t[1]=DIL_GAMMA1-a->coeffs[2*i+1];
...
static void polyz_unpack(poly *r,const unsigned char *a){
  int i; for(i=0;i<DIL_N/2;i++){
    r->coeffs[2*i+0]=a[5*i+0]; r->coeffs[2*i+0]|=(uint32_t)a[5*i+1]<<8; r->coeffs[2*i+0]|=(uint32_t)a[5*i+2]<<16; r->coeffs[2*i+0]&=0xFFFFF;
    r->coeffs[2*i+1]=a[5*i+2]>>4; r->coeffs[2*i+1]|=(uint32_t)a[5*i+3]<<4; r->coeffs[2*i+1]|=(uint32_t)a[5*i+4]<<12; r->coeffs[2*i+1]&=0xFFFFF;
    r->coeffs[2*i+0]=DIL_GAMMA1-r->coeffs[2*i+0]; r->coeffs[2*i+1]=DIL_GAMMA1-r->coeffs[2*i+1];
  }
}
#endif
/* ---- w1 packing: gamma2-dependent ---- */
#if DIL_GAMMA2 == ((DIL_Q - 1) / 88)
static void polyw1_pack(unsigned char *r,const poly *a){
  int i; for(i=0;i<DIL_N/4;i++){
    r[3*i+0]=(unsigned char)(a->coeffs[4*i+0]|(a->coeffs[4*i+1]<<6));
    r[3*i+1]=(unsigned char)((a->coeffs[4*i+1]>>2)|(a->coeffs[4*i+2]<<4));
    r[3*i+2]=(unsigned char)((a->coeffs[4*i+2]>>4)|(a->coeffs[4*i+3]<<2));
  }
}
#else
static void polyw1_pack(unsigned char *r,const poly *a){
  int i; for(i=0;i<DIL_N/2;i++) r[i]=(unsigned char)(a->coeffs[2*i+0]|(a->coeffs[2*i+1]<<4));
}
#endif

static void polyvec_matrix_expand(polyvecl mat[DIL_K],const unsigned char rho[DIL_SEEDBYTES]){
  unsigned int i,j; for(i=0;i<DIL_K;i++) for(j=0;j<DIL_L;j++) poly_uniform(&mat[i].vec[j],rho,(uint16_t)((i<<8)+j));
}
...
static void polyvecl_pointwise_acc_montgomery(poly *w,const polyvecl *u,const polyvecl *v){
  int i; poly t; poly_pointwise_montgomery(w,&u->vec[0],&v->vec[0]);
...
static void pack_sk(unsigned char *sk,const unsigned char rho[DIL_SEEDBYTES],
  ...
  for(i=0;i<DIL_L;i++) polyeta_pack(sk+i*DIL_POLYETA_PACKEDBYTES,&s1->vec[i]); sk+=DIL_L*DIL_POLYETA_PACKEDBYTES;
  for(i=0;i<DIL_K;i++) polyeta_pack(sk+i*DIL_POLYETA_PACKEDBYTES,&s2->vec[i]); sk+=DIL_K*DIL_POLYETA_PACKEDBYTES;
...
static void unpack_sk(unsigned char rho[DIL_SEEDBYTES],unsigned char tr[DIL_TRBYTES],
                      unsigned char key[DIL_SEEDBYTES],polyveck *t0,polyvecl *s1,polyveck *s2,const unsigned char *sk){
  int i;
  ...
  for(i=0;i<DIL_L;i++) polyeta_unpack(&s1->vec[i],sk+i*DIL_POLYETA_PACKEDBYTES); sk+=DIL_L*DIL_POLYETA_PACKEDBYTES;
  for(i=0;i<DIL_K;i++) polyeta_unpack(&s2->vec[i],sk+i*DIL_POLYETA_PACKEDBYTES); sk+=DIL_K*DIL_POLYETA_PACKEDBYTES;
...
static void pack_sig(unsigned char *sig,const unsigned char c[DIL_CTILDEBYTES],const polyvecl *z,const polyveck *h){
  ...
  for(i=0;i<DIL_L;i++) polyz_pack(sig+i*DIL_POLYZ_PACKEDBYTES,&z->vec[i]); sig+=DIL_L*DIL_POLYZ_PACKEDBYTES;
  for(i=0;i<DIL_OMEGA+DIL_K;i++) sig[i]=0;
  k=0;
  for(i=0;i<DIL_K;i++){ for(j=0;j<DIL_N;j++) if(h->vec[i].coeffs[j]!=0) sig[k++]=(unsigned char)j; sig[DIL_OMEGA+i]=(unsigned char)k; }
}
static int unpack_sig(unsigned char c[DIL_CTILDEBYTES],polyvecl *z,polyveck *h,const unsigned char *sig){
  ...
  for(i=0;i<DIL_L;i++) polyz_unpack(&z->vec[i],sig+i*DIL_POLYZ_PACKEDBYTES); sig+=DIL_L*DIL_POLYZ_PACKEDBYTES;
  k=0;
  for(i=0;i<DIL_K;i++){
    for(j=0;j<DIL_N;j++) h->vec[i].coeffs[j]=0;
    if(sig[DIL_OMEGA+i]<k || sig[DIL_OMEGA+i]>DIL_OMEGA) return 1;
    for(j=k;j<sig[DIL_OMEGA+i];j++){ if(j>(int)k && sig[j]<=sig[j-1]) return 1; h->vec[i].coeffs[sig[j]]=1; }
    k=sig[DIL_OMEGA+i];
  }
...
int MLD_NAME(keypair_derand)(unsigned char *pk,unsigned char *sk,const unsigned char *seed){
  unsigned char seedbuf[2*DIL_SEEDBYTES+DIL_CRHBYTES]; unsigned char inbuf[DIL_SEEDBYTES+2];
  const unsigned char *rho,*rhoprime,*key; unsigned char tr[DIL_TRBYTES];
  polyvecl mat[DIL_K],s1,s1hat; polyveck s2,t1,t0,t;
  memcpy(inbuf,seed,DIL_SEEDBYTES); inbuf[DIL_SEEDBYTES]=DIL_K; inbuf[DIL_SEEDBYTES+1]=DIL_L;
  ...
  polyveck_add(&t,&t,&s2); polyveck_caddq(&t);
...
static int mld_sign_internal(unsigned char *sig,const unsigned char *m,size_t mlen,
                             const unsigned char *ctx,size_t ctxlen,const unsigned char *sk,const unsigned char *rnd){
  unsigned char rho[DIL_SEEDBYTES],tr[DIL_TRBYTES],key[DIL_SEEDBYTES];
  unsigned char mu[DIL_CRHBYTES],rhoprime[DIL_CRHBYTES],ctilde[DIL_CTILDEBYTES];
  unsigned char w1packed[DIL_K*DIL_POLYW1_PACKEDBYTES];
  unsigned char keymu[DIL_SEEDBYTES+DIL_RNDBYTES+DIL_CRHBYTES]; unsigned char pfx[2];
  polyvecl mat[DIL_K],s1,y,z; polyveck t0,s2,w1,w0,h; poly cp; uint16_t nonce=0; rktcrypto_keccak_ctx_t kc;
  if(ctxlen>255) return 0;
  unpack_sk(rho,tr,key,&t0,&s1,&s2,sk);
  pfx[0]=0; pfx[1]=(unsigned char)ctxlen;
  rktcrypto_keccak_core_init(&kc,136,0x1f);
  rktcrypto_keccak_core_update(&kc,tr,DIL_TRBYTES); rktcrypto_keccak_core_update(&kc,pfx,2);
  if(ctxlen) rktcrypto_keccak_core_update(&kc,ctx,(intptr_t)ctxlen);
  rktcrypto_keccak_core_update(&kc,m,(intptr_t)mlen); rktcrypto_keccak_core_final(&kc,mu,DIL_CRHBYTES);
  memcpy(keymu,key,DIL_SEEDBYTES); memcpy(keymu+DIL_SEEDBYTES,rnd,DIL_RNDBYTES); memcpy(keymu+DIL_SEEDBYTES+DIL_RNDBYTES,mu,DIL_CRHBYTES);
  shake256(rhoprime,DIL_CRHBYTES,keymu,sizeof(keymu));
  polyvec_matrix_expand(mat,rho); polyvecl_ntt(&s1); polyveck_ntt(&s2); polyveck_ntt(&t0);
  for(;;){
    int i;
    for(i=0;i<DIL_L;i++) poly_uniform_gamma1(&y.vec[i],rhoprime,(uint16_t)(DIL_L*nonce+i)); nonce++;
    z=y; polyvecl_ntt(&z);
    polyvec_matrix_pointwise_montgomery(&w1,mat,&z);
    polyveck_reduce(&w1); polyveck_invntt_tomont(&w1); polyveck_caddq(&w1);
    polyveck_decompose(&w1,&w0,&w1); polyveck_pack_w1(w1packed,&w1);
    rktcrypto_keccak_core_init(&kc,136,0x1f);
    rktcrypto_keccak_core_update(&kc,mu,DIL_CRHBYTES); rktcrypto_keccak_core_update(&kc,w1packed,sizeof(w1packed));
    rktcrypto_keccak_core_final(&kc,ctilde,DIL_CTILDEBYTES);
    poly_challenge(&cp,ctilde); poly_ntt(&cp);
    polyvecl_pointwise_poly_montgomery(&z,&cp,&s1); polyvecl_invntt_tomont(&z);
    polyvecl_add(&z,&z,&y); polyvecl_reduce(&z);
    if(polyvecl_chknorm(&z,DIL_GAMMA1-DIL_BETA)) continue;
    polyveck_pointwise_poly_montgomery(&h,&cp,&s2); polyveck_invntt_tomont(&h);
    polyveck_sub(&w0,&w0,&h); polyveck_reduce(&w0);
    if(polyveck_chknorm(&w0,DIL_GAMMA2-DIL_BETA)) continue;
    polyveck_pointwise_poly_montgomery(&h,&cp,&t0); polyveck_invntt_tomont(&h); polyveck_reduce(&h);
    if(polyveck_chknorm(&h,DIL_GAMMA2)) continue;
    polyveck_add(&w0,&w0,&h);
    if(polyveck_make_hint(&h,&w0,&w1)>DIL_OMEGA) continue;
    pack_sig(sig,ctilde,&z,&h); return 1;
  }
}
int MLD_NAME(sign)(unsigned char *sig,const unsigned char *m,intptr_t mlen,const unsigned char *sk){
  unsigned char rnd[DIL_RNDBYTES]; if(!rktcrypto_random_bytes(rnd,0,DIL_RNDBYTES)) return 0;
  return mld_sign_internal(sig,m,(size_t)mlen,NULL,0,sk,rnd);
}
int MLD_NAME(sign_derand)(unsigned char *sig,const unsigned char *m,intptr_t mlen,const unsigned char *sk){
  unsigned char rnd[DIL_RNDBYTES]; memset(rnd,0,sizeof(rnd));
  return mld_sign_internal(sig,m,(size_t)mlen,NULL,0,sk,rnd);
}
int MLD_NAME(verify)(const unsigned char *sig,const unsigned char *m,intptr_t mlen,const unsigned char *pk){
  unsigned char rho[DIL_SEEDBYTES],tr[DIL_TRBYTES],mu[DIL_CRHBYTES];
  unsigned char c[DIL_CTILDEBYTES],c2[DIL_CTILDEBYTES]; unsigned char w1packed[DIL_K*DIL_POLYW1_PACKEDBYTES]; unsigned char pfx[2];
  polyvecl mat[DIL_K],z; polyveck t1,w1,h; poly cp; rktcrypto_keccak_ctx_t kc; int i; size_t ctxlen=0;
  unpack_pk(rho,&t1,pk);
  ...
  rktcrypto_keccak_core_update(&kc,tr,DIL_TRBYTES); rktcrypto_keccak_core_update(&kc,pfx,2);
  rktcrypto_keccak_core_update(&kc,m,(intptr_t)mlen); rktcrypto_keccak_core_final(&kc,mu,DIL_CRHBYTES);
  poly_challenge(&cp,c); polyvec_matrix_expand(mat,rho); polyvecl_ntt(&z);
  polyvec_matrix_pointwise_montgomery(&w1,mat,&z);
  poly_ntt(&cp); polyveck_shiftl(&t1); polyveck_ntt(&t1);
  polyveck_pointwise_poly_montgomery(&t1,&cp,&t1);
  polyveck_sub(&w1,&w1,&t1); polyveck_reduce(&w1); polyveck_invntt_tomont(&w1); polyveck_caddq(&w1);
  polyveck_use_hint(&w1,&w1,&h); polyveck_pack_w1(w1packed,&w1);
  rktcrypto_keccak_core_init(&kc,136,0x1f);
  rktcrypto_keccak_core_update(&kc,mu,DIL_CRHBYTES); rktcrypto_keccak_core_update(&kc,w1packed,sizeof(w1packed));
...
int MLD_NAME(keypair)(unsigned char *pk,unsigned char *sk){
  unsigned char seed[DIL_SEEDBYTES]; if(!rktcrypto_random_bytes(seed,0,DIL_SEEDBYTES)) return 0;
  return MLD_NAME(keypair_derand)(pk,sk,seed);
}
