...
{
  /* Both the asm kernel and the NEON-intrinsic path below use FEAT_SHA1
     instructions (sha1c/h/m/p/su0/su1). On a generic aarch64 core that lacks
     SHA1 fall back to the portable scalar block. On Apple the flag is always
     set, so this branch is never taken and behavior is bit-identical. */
  if (!rktcrypto_cpu_has(RKTCRYPTO_CPU_ARM_SHA1)) {
