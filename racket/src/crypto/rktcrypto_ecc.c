...
typedef struct {
  ...
  int a_m3;                   /* 1 = curve coefficient a == -3 (NIST); 0 = general a (brainpool) */
  u64 p[MAXL], n[MAXL], b[MAXL], gx[MAXL], gy[MAXL];
  u64 rr_p[MAXL], rr_n[MAXL], amont[MAXL], b3mont[MAXL], bmont[MAXL], n0_p, n0_n;
  void *comb;                 /* per-curve fixed-base comb table (jpt[topnib][16]) */
} curve;
...
/* Generic-width CIOS Montgomery multiply in rktasm (rktcrypto_mont_cios_asm.S):
   the fp_mul kernel for the generic-prime curves (brainpool nl=4/6/8). */
...
static void montmul(u64 *r,const u64 *a,const u64 *b,const u64 *m,u64 n0,int nl){
  u64 t[MAXL+2]; int k;
  mont_cios_asm(r,a,b,m,n0,(u64)nl,t);
...
static void __attribute__((unused)) reduce_p384(u64 *r,const u64 *prod){  /* asm reduce_p384_asm routed on aarch64+apple; kept as oracle + portable fallback */
...
#if defined(__aarch64__)
/* Multiply-free Solinas reduction, ~2x the C reduce_p384 (asm/reduce_p384.asm,
   in rktcrypto_ecc_asm.S). Bit-exact with reduce_p384 (asm/tests/test_reduce_p384.c
   differential); the field-mul reduce is P-384's dominant cost, so this lifts all
   P-384 point arithmetic. */
extern void reduce_p384_asm(u64 *r,const u64 *prod);
#endif
static void fp_reduce(u64 *r,const u64 *prod,const curve *cv){
  if(cv->fast==521) reduce_p521(r,prod);
#if defined(__aarch64__)
  else reduce_p384_asm(r,prod);
#else
...
static void jac_double(jpt *r,const jpt *p,const curve *cv){   /* dbl-2001-b, a=-3 */
  ...
  fp_mul(S,p->X,YY,cv); fp_mul(Y4,YY,YY,cv);
  fp_add(S,S,S,cv); fp_add(S,S,S,cv);                        /* S = 4XY^2 */
  if(cv->a_m3){
    fp_sub(xm,p->X,ZZ,cv); fp_add(xp,p->X,ZZ,cv); fp_mul(prod,xm,xp,cv);
    fp_add(M,prod,prod,cv); fp_add(M,M,prod,cv);             /* M = 3(X-ZZ)(X+ZZ) = 3X^2 - 3ZZ^2 */
  } else {
    u64 XX[MAXL],ZZ2[MAXL],aZ[MAXL];
    fp_mul(XX,p->X,p->X,cv); fp_mul(ZZ2,ZZ,ZZ,cv); fp_mul(aZ,cv->amont,ZZ2,cv);
    fp_add(M,XX,XX,cv); fp_add(M,M,XX,cv); fp_add(M,M,aZ,cv); /* M = 3X^2 + a*Z^4 */
  }
...
static jpt COMB_BP256[64][16], COMB_BP384[96][16], COMB_BP512[128][16];
static void scalar_mul_base(jpt *R,const u64 *k,const curve *cv){
  jpt (*comb)[16]=(jpt(*)[16])cv->comb;
...
static curve C384, C521, C_BP256, C_BP384, C_BP512; static int inited=0;
/* brainpoolP256r1 (RFC 5639): general-a curve (a != -3), generic Montgomery. */
static const unsigned char BP256_P[32]={0xa9,0xfb,0x57,0xdb,0xa1,0xee,0xa9,0xbc,0x3e,0x66,0x0a,0x90,0x9d,0x83,0x8d,0x72,0x6e,0x3b,0xf6,0x23,0xd5,0x26,0x20,0x28,0x20,0x13,0x48,0x1d,0x1f,0x6e,0x53,0x77};
static const unsigned char BP512_B[64]={0x3d,0xf9,0x16,0x10,0xa8,0x34,0x41,0xca,0xea,0x98,0x63,0xbc,0x2d,0xed,0x5d,0x5a,0xa8,0x25,0x3a,0xa1,0x0a,0x2e,0xf1,0xc9,0x8b,0x9a,0xc8,0xb5,0x7f,0x11,0x17,0xa7,0x2b,0xf2,0xc7,0xb9,0xe7,0xc1,0xac,0x4d,0x77,0xfc,0x94,0xca,0xdc,0x08,0x3e,0x67,0x98,0x40,0x50,0xb7,0x5e,0xba,0xe5,0xdd,0x28,0x09,0xbd,0x63,0x80,0x16,0xf7,0x23};
static const unsigned char BP512_N[64]={0xaa,0xdd,0x9d,0xb8,0xdb,0xe9,0xc4,0x8b,0x3f,0xd4,0xe6,0xae,0x33,0xc9,0xfc,0x07,0xcb,0x30,0x8d,0xb3,0xb3,0xc9,0xd2,0x0e,0xd6,0x63,0x9c,0xca,0x70,0x33,0x08,0x70,0x55,0x3e,0x5c,0x41,0x4c,0xa9,0x26,0x19,0x41,0x86,0x61,0x19,0x7f,0xac,0x10,0x47,0x1d,0xb1,0xd3,0x81,0x08,0x5d,0xda,0xdd,0xb5,0x87,0x96,0x82,0x9c,0xa9,0x00,0x69};
static const unsigned char BP512_GX[64]={0x81,0xae,0xe4,0xbd,0xd8,0x2e,0xd9,0x64,0x5a,0x21,0x32,0x2e,0x9c,0x4c,0x6a,0x93,0x85,0xed,0x9f,0x70,0xb5,0xd9,0x16,0xc1,0xb4,0x3b,0x62,0xee,0xf4,0xd0,0x09,0x8e,0xff,0x3b,0x1f,0x78,0xe2,0xd0,0xd4,0x8d,0x50,0xd1,0x68,0x7b,0x93,0xb9,0x7d,0x5f,0x7c,0x6d,0x50,0x47,0x40,0x6a,0x5e,0x68,0x8b,0x35,0x22,0x09,0xbc,0xb9,0xf8,0x22};
static const unsigned char BP512_GY[64]={0x7d,0xde,0x38,0x5d,0x56,0x63,0x32,0xec,0xc0,0xea,0xbf,0xa9,0xcf,0x78,0x22,0xfd,0xf2,0x09,0xf7,0x00,0x24,0xa5,0x7b,0x1a,0xa0,0x00,0xc5,0x5b,0x88,0x1f,0x81,0x11,0xb2,0xdc,0xde,0x49,0x4a,0x5f,0x48,0x5e,0x5b,0xca,0x4b,0xd8,0x8a,0x27,0x63,0xae,0xd1,0xca,0x2b,0x2f,0xa8,0xf0,0x54,0x06,0x78,0xcd,0x1e,0x0f,0x3a,0xd8,0x08,0x92};
...
static void init_curve(curve *cv,int nbytes,int pbits,int nbits,int halg,int fast,
  ...
  const unsigned char*a /* NULL => a = -3 (NIST); else the curve's a (brainpool) */, void *comb){
  cv->comb=comb;
  ...
  { u64 am[MAXL],three[MAXL],b3[MAXL];
    if(a){ bytes_to_limbs(am,a,nbytes,cv->nl); cv->a_m3=0; }
    else { bn_zero(three); three[0]=3; bn_sub(am,cv->p,three,cv->nl); cv->a_m3=1; }  /* a = -3 mod p */
...
static void ecc_init(void){
  if(inited) return;
  init_curve(&C384,48,384,384,RKTCRYPTO_SHA384,384,P384_P,P384_N,P384_B,P384_GX,P384_GY,0,COMB384);
  init_curve(&C521,66,521,521,RKTCRYPTO_SHA512,521,P521_P,P521_N,P521_B,P521_GX,P521_GY,0,COMB521);
  init_curve(&C_BP256,32,256,256,RKTCRYPTO_SHA256,0,BP256_P,BP256_N,BP256_B,BP256_GX,BP256_GY,BP256_A,COMB_BP256);
  init_curve(&C_BP384,48,384,384,RKTCRYPTO_SHA384,0,BP384_P,BP384_N,BP384_B,BP384_GX,BP384_GY,BP384_A,COMB_BP384);
  init_curve(&C_BP512,64,512,512,RKTCRYPTO_SHA512,0,BP512_P,BP512_N,BP512_B,BP512_GX,BP512_GY,BP512_A,COMB_BP512);
  inited=1;
}
...
int rktcrypto_brainpoolP256_pubkey(unsigned char *out,const unsigned char *priv){ ecc_init(); return ecc_pubkey(&C_BP256,out,priv); }
int rktcrypto_brainpoolP256_ecdh(unsigned char *out,const unsigned char *scalar,const unsigned char *point){ ecc_init(); return ecc_ecdh(&C_BP256,out,scalar,point); }
int rktcrypto_brainpoolP256_ecdsa_sign(unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *priv){ ecc_init(); return ecc_sign(&C_BP256,sig,msg,mlen,priv); }
int rktcrypto_brainpoolP256_ecdsa_verify(const unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *pub){ ecc_init(); return ecc_verify(&C_BP256,sig,msg,mlen,pub); }
int rktcrypto_brainpoolP384_pubkey(unsigned char *out,const unsigned char *priv){ ecc_init(); return ecc_pubkey(&C_BP384,out,priv); }
int rktcrypto_brainpoolP384_ecdh(unsigned char *out,const unsigned char *scalar,const unsigned char *point){ ecc_init(); return ecc_ecdh(&C_BP384,out,scalar,point); }
int rktcrypto_brainpoolP384_ecdsa_sign(unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *priv){ ecc_init(); return ecc_sign(&C_BP384,sig,msg,mlen,priv); }
int rktcrypto_brainpoolP384_ecdsa_verify(const unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *pub){ ecc_init(); return ecc_verify(&C_BP384,sig,msg,mlen,pub); }
int rktcrypto_brainpoolP512_pubkey(unsigned char *out,const unsigned char *priv){ ecc_init(); return ecc_pubkey(&C_BP512,out,priv); }
int rktcrypto_brainpoolP512_ecdh(unsigned char *out,const unsigned char *scalar,const unsigned char *point){ ecc_init(); return ecc_ecdh(&C_BP512,out,scalar,point); }
int rktcrypto_brainpoolP512_ecdsa_sign(unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *priv){ ecc_init(); return ecc_sign(&C_BP512,sig,msg,mlen,priv); }
int rktcrypto_brainpoolP512_ecdsa_verify(const unsigned char *sig,const unsigned char *msg,intptr_t mlen,const unsigned char *pub){ ecc_init(); return ecc_verify(&C_BP512,sig,msg,mlen,pub); }
