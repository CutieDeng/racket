...
int rktcrypto_x509_verify_selfsigned(const unsigned char *der_buf, intptr_t derlen){
  ...
  if(is_rsa){
    ...
    { unsigned char eb[16]; int j; for(j=0;j<16;j++) eb[j]=(j<8)?0:(unsigned char)(ev>>(8*(15-j))); bn_from_be(&k.e,eb,16); }
...
/* Extracts the EC public point (0x04||X||Y) from a cert's SPKI into pub[].
   Returns the field byte size (32/48/66) or 0. */
static int cert_ec_pubkey(const unsigned char *cert_der, intptr_t len, unsigned char *pub){
  der top,cert,t,spki; int tag; const unsigned char *cp,*ce; intptr_t klen;
  top.p=cert_der; top.end=cert_der+len;
  if(!der_into(&top,0x30,&cert)) return 0;
  if(!der_into(&cert,0x30,&t)) return 0;                 /* TBSCertificate */
  if(t.p<t.end && (unsigned char)*t.p==0xA0){ if(!der_skip(&t)) return 0; }  /* version [0] */
  if(!der_skip(&t)||!der_skip(&t)||!der_skip(&t)||!der_skip(&t)||!der_skip(&t)) return 0;
  if(!der_into(&t,0x30,&spki)) return 0;                 /* SubjectPublicKeyInfo */
  { der a; if(!der_into(&spki,0x30,&a)) return 0; }      /* algorithm */
  if(!der_tlv(&spki,&tag,&cp,&ce)||tag!=0x03) return 0;  /* subjectPublicKey BIT STRING */
  klen=(ce-cp)-1;                                         /* drop the unused-bits octet */
  if(klen==65){ memcpy(pub,cp+1,65); return 32; }
  if(klen==97){ memcpy(pub,cp+1,97); return 48; }
  if(klen==133){ memcpy(pub,cp+1,133); return 66; }
  return 0;
}
...
int rktcrypto_cms_verify(const unsigned char *der_buf, intptr_t derlen){
  ...
  const unsigned char *sig; intptr_t siglen;
  rsa_key k; unsigned char hash[32],cd[32]; int rc; int sig_is_ec=0;
  ...
  /* signatureAlgorithm { OID } — classify RSA (default) vs ECDSA-with-SHA256 */
  { der sa2; const unsigned char *aoid; intptr_t aoidlen;
    static const unsigned char ECDSA_SHA256[]={0x2a,0x86,0x48,0xce,0x3d,0x04,0x03,0x02};
    if(!der_into(&si,0x30,&sa2)) return 0;
    if(!der_tlv(&sa2,&tag,&aoid,&ce)||tag!=0x06) return 0; aoidlen=ce-aoid;
    if(aoidlen==(intptr_t)sizeof ECDSA_SHA256 && memcmp(aoid,ECDSA_SHA256,sizeof ECDSA_SHA256)==0) sig_is_ec=1;
  }
  ...
  if(signedattrs){
    ...
    der sa; int found=0; sa.p=signedattrs; sa.end=signedattrs+sa_len;
  ...
  if(sig_is_ec){
    unsigned char pub[200],rawsig[132]; int fb; der ss,sq; const unsigned char *rp,*sp2; intptr_t rlen,slen2;
    fb=cert_ec_pubkey(cert_der,cert_len,pub); if(fb!=32) return 0;    /* P-256/SHA-256 */
    ss.p=sig; ss.end=sig+siglen; if(!der_into(&ss,0x30,&sq)) return 0;
    if(!der_tlv(&sq,&tag,&cp,&ce)||tag!=0x02) return 0; rp=cp; rlen=ce-cp; trim_int(&rp,&rlen);
    if(!der_tlv(&sq,&tag,&cp,&ce)||tag!=0x02) return 0; sp2=cp; slen2=ce-cp; trim_int(&sp2,&slen2);
    memset(rawsig,0,2*fb); memcpy(rawsig+fb-rlen,rp,rlen); memcpy(rawsig+2*fb-slen2,sp2,slen2);
    return rktcrypto_p256_ecdsa_verify_h(rawsig,hash,32,pub);
  }
