...
/* Operand-scanning 2-carry-chain asm kernel (rktcrypto_bn_op.S): 113 ns at k=16
   -- beats a FIPS product-scanning kernel at k=16 (129 ns) and OpenSSL (~116 ns).
   The RSA-CRT (mod p/q) hot path. (k=32 can't keep the accumulator
   register-resident, so it uses the FIPS kernel below. The k=16 FIPS variant is
   therefore unused and no longer generated -- see asm/gen_fips.py / regen.sh.) */
