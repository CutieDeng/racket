...
#if defined(__aarch64__) && !defined(RKTCRYPTO_P256_NO_ASM)
...
/* Hand-scheduled register-resident mixed-add (rktcrypto_p256_hand.S), ~0.88x the
   C body below; bit-exact (asm/tests/test_mixed_add.c). The variable-base scalar
   mul's inner add. */
extern void mixed_add_hw(jac *r,const jac *p,const jac *q);
#endif
static void mixed_add(jac *r,const jac *p,const jac *q){
#if defined(__aarch64__)
  mixed_add_hw(r,p,q); return;
}
static void __attribute__((unused)) mixed_add_portable(jac *r,const jac *p,const jac *q){
#endif
