...
`example/028-deflate-dynamic-canonical-codes.asm` consumes a length table and
...
`example/029-deflate-dynamic-reverse-codes.asm` consumes the length table and
...
`example/030-deflate-dynamic-balanced-litonly.asm` wires the native helper
...
`example/031-deflate-dynamic-code-length-rle.asm` ports the code-length RLE
...
`example/032-deflate-dynamic-blfreq.asm` consumes those RLE events and fills
...
`example/033-deflate-dynamic-bllen-balanced.asm` consumes `bl_freq[19]` and
...
`example/034-deflate-dynamic-blcodes-count.asm` consumes `bl_len[19]` and
...
`example/035-deflate-dynamic-compact-header.asm` consumes `bl_len[19]`,
...
`example/036-deflate-dynamic-compact-balanced-litonly.asm` wires the compact
...
`example/037-deflate-dynamic-code-counts.asm` computes the dynamic header's
...
`example/038-deflate-dynamic-compact-trimmed-litonly.asm` integrates 037 into
...
`example/039-deflate-dynamic-litlen-huffman.asm` adds the first native
...
`example/040-deflate-dynamic-compact-huffman-litonly.asm` wires 039 into the
...
`example/041-deflate-dynamic-bllen-huffman.asm` ports the same native tree
...
`example/042-deflate-dynamic-compact-dual-huffman-litonly.asm` wires both 039
...
`example/043-deflate-dynamic-lz77-freq.asm` starts that LZ77 bridge without
...
`example/044-deflate-dynamic-distlen-huffman.asm` turns `dist_freq[30]` into
...
`example/045-deflate-dynamic-lz77-huffman.asm` wires those pieces into the
...
`example/046-deflate-auto-dynamic-probe.asm` is the first measured selector
...
`example/047-deflate-auto-cost-probe.asm` is the next selector probe. It still
...
`example/048-deflate-auto-size-probe.asm` is the first estimated-size selector.
...
`example/049-deflate-auto-prepared-size-probe.asm` keeps 045 and 048 intact and
...
`example/050-deflate-auto-cheap-prepared-size-probe.asm` adds a cheap front
...
`example/051-deflate-blocked-fixed.asm` is the first block-framing probe for
...
`example/052-deflate-blocked-auto.asm` keeps the 051 framing and adds per-block
...
`example/053-deflate-blocked-dynamic-auto.asm` adds prepared dynamic emission
