...
```bash
racket research/deflate-optimization/build-native-library.rkt --out-dir /tmp/asmp-deflate-lib
```

The build consumes `asmp-deflate-library.rktd`, which is the current single
...
`research/deflate-optimization/c-examples/raw-roundtrip.c`. It calls the
...
```bash
clang research/deflate-optimization/c-examples/raw-roundtrip.c \
  /tmp/asmp-deflate-lib/libasmp_deflate.a \
...
```bash
racket research/deflate-optimization/render-library-header.rkt
```

To check that the snapshot is current:

```bash
racket research/deflate-optimization/render-library-header.rkt --check
```
