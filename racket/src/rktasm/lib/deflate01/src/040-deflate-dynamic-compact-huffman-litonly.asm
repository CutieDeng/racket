// ============================================================
// 040-deflate-dynamic-compact-huffman-litonly.asm
// ============================================================
//
// Literal-only dynamic-Huffman pipeline with:
//   - frequency-driven LL lengths from 039;
//   - compact code-length RLE header from 031/032/033;
//   - trimmed HLIT/HDIST counts from 037.
//
// The bit-length tree is still the balanced 033 scaffold.  This version keeps
// 038 available as the balanced LL baseline and moves only the LL tree to the
// repaired frequency-driven builder.
...
.function dch_flush8 (inout: x.out, x.bitbuf, w.bit_count)
entry:
flush_loop:
  cmp w.bit_count, #8
  b.lt flush_done
...
.function dch_write_raw (in: x.bits, w.nbits, inout: x.out, x.bitbuf, w.bit_count)
entry:
  cbz w.nbits, write_done
  lsl x.tmp, x.bits, x.bit_count
  orr x.bitbuf, x.bitbuf, x.tmp
  add w.bit_count, w.bit_count, w.nbits
  .inline dch_flush8 (x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
write_done:
  .return
.end

.function deflate_dynamic_compact_huffman_trimmed_litonly_aarch64_asm (
...
rle_start:
  ...
  adrp x.order, :pg_hi21:deflate_dynamic_compact_huffman_bl_order
  add x.order, x.order, :lo12:deflate_dynamic_compact_huffman_bl_order
...
write_header_start:
  ...
  mov x.bits, #5
  mov w.nbits, #3
  .inline dch_write_raw (x.bits=x.bits, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
.function asmp_deflate_raw_dynamic_compact_huffman_trimmed_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function asmp_deflate_raw_dynamic_compact_huffman_trimmed_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #11376
...
.function asmp_deflate_raw_dynamic_compact_huffman_trimmed_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #16
...
.function asmp_deflate_raw_dynamic_compact_huffman_trimmed export profile=c-aapcs64 (
...
public_src_ok:
  ...
  .call deflate_dynamic_compact_huffman_trimmed_litonly_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.scratch=x.scratch,
...
.section .rodata
deflate_dynamic_compact_huffman_bl_order:
