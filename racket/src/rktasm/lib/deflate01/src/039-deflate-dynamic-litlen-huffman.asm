// ============================================================
// 039-deflate-dynamic-litlen-huffman.asm - frequency LL lengths
// ============================================================
//
// Builds literal/length Huffman code lengths from ll_freq[286].
//
// This is the first native frequency-driven tree builder.  It deliberately
// uses a simple O(n^2) merge loop so the data flow is easy to inspect:
//   - root candidates are nodes whose parent is 0xffff;
//   - inactive leaf slots have parent 0xfffe;
//   - ties are ordered by the smallest original symbol under the node.
//
// Raw Huffman depths over 15 bits are repaired with the same bit-length-count
// overflow strategy used by the Racket oracle: clamp overlong leaves at 15,
// move counts from the deepest available shorter level, then reassign lengths
// to active symbols sorted by frequency.

// Scratch layout, 16-byte aligned:
//   +0      work_freq[572]  uint64_t  4576 bytes
//   +4576   parent[572]     uint16_t  1144 bytes
//   +5720   tie[572]        uint16_t  1144 bytes
//   total = 6864 bytes

.function deflate_dynamic_litlen_huffman_aarch64_asm export profile=c-aapcs64 visibility=hidden no-header (
...
.function asmp_deflate_dynamic_litlen_huffman export profile=c-aapcs64 (
...
entry:
  ...
  mov x.required_scratch, #6864
  cmp x.scratch_len, x.required_scratch
  b.lo public_scratch_too_small

  .call deflate_dynamic_litlen_huffman_aarch64_asm (
