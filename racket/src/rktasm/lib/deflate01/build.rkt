...
(define (write-pkg-config prefix pc-path)
  (call-with-output-file pc-path
    (lambda (out)
      ...
      (display "Name: asmp-deflate\n" out)
      (display "Description: AArch64 raw-deflate compressor generated by asmp\n" out)
      (display "Version: 0.1.0\n" out)
      (display "Cflags: -I${includedir}\n" out)
      (display "Libs: -L${libdir} -lasmp_deflate -lz\n" out))
...
(define (install-native-library library-path header-path prefix)
  ...
  (define installed-library (build-path lib-dir "libasmp_deflate.a"))
  (copy-file header-path installed-header #t)
  (copy-file library-path installed-library #t)
  (define pc-path (build-path pc-dir "asmp-deflate.pc"))
