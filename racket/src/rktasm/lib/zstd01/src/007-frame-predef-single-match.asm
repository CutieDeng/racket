// ============================================================
// 007-frame-predef-single-match.asm - predefined-FSE short single match
// ============================================================
//
// This step keeps the previous single-match encoder as the complete fallback,
// then adds a narrow predefined-FSE sequence path:
//
//   input = prefix bytes followed by a repeated suffix byte, length <= 49
//   literals = 1..15 prefix bytes
//   sequence = LL code prefix_len, OF code 0, ML code match_len - 3
//
// The LL/OF/ML table modes are all Predefined_Mode. For this MVP there are no
// LL/OF/ML extra bits, so the sequence bitstream is only the three initial FSE
// states plus the zstd end mark.

.function zstd07_bound_inner (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function zstd07_write_frame_header (
  inout: x.out,
  in: x.src_len
)
entry:
...
.function zstd07_write_empty_block (
  inout: x.out
)
entry:
  mov x.header, #1
  .inline zstd07_write_u24 (x.out=x.out, x.value=x.header)
  ret
.end

.function zstd07_lit_header_size (
  in: x.block_len,
  out: x.header_size
)
entry:
...
.function zstd07_write_litonly_block (
  inout: x.out, x.in,
  in: x.block_len, x.last
)
entry:
  .inline zstd07_lit_header_size (x.block_len=x.block_len, x.header_size=x.lit_header_size)
...
litonly_header_done:
  ...
  .inline zstd07_copy_bytes (x.out=x.out, x.in=x.in, x.count=x.count)
...
litonly_header_two_or_three:
  ...
  .inline zstd07_write_u16 (x.out=x.out, x.value=x.header)
  b litonly_header_done
...
.function zstd07_compress_litonly_frame (
...
entry:
  ...
  .inline zstd07_write_frame_header (x.out=x.out, x.src_len=x.src_len)

  cbnz x.remaining, block_loop
  .inline zstd07_write_empty_block (x.out=x.out)
  b frame_done
...
have_block_len:
  ...
  cset x.last, eq
  .inline zstd07_write_litonly_block (x.out=x.out, x.in=x.in, x.block_len=x.block_len, x.last=x.last)
...
.function zstd07_find_single_match (
  in: x.src, x.src_len,
...
.function zstd07_match_length_code (
  in: x.match_len,
...
.function zstd07_write_ml_bitstream (
  inout: x.out,
...
.function zstd07_find_predef_single_match (
  in: x.src, x.src_len,
  out: w.ok, x.prefix_len
)
entry:
  .inline zstd07_find_single_match (x.src=x.src, x.src_len=x.src_len, w.ok=w.ok, x.prefix_len=x.prefix_len)
  cbz w.ok, done
...
.function zstd07_ll_predef_state (
  in: x.ll_code,
  out: x.ll_state
)
entry:
...
.function zstd07_ml_predef_state (
  in: x.ml_code,
  out: x.ml_state
)
entry:
...
.function zstd07_write_predef_bitstream (
  inout: x.out,
...
.function zstd07_write_predef_singlematch_block (
  inout: x.out,
...
entry:
  sub x.match_len, x.src_len, x.prefix_len
  sub x.ml_code, x.match_len, #3
  .inline zstd07_ll_predef_state (x.ll_code=x.prefix_len, x.ll_state=x.ll_state)
  .inline zstd07_ml_predef_state (x.ml_code=x.ml_code, x.ml_state=x.ml_state)
  ...
  .inline zstd07_write_predef_bitstream (x.out=x.out, x.ll_state=x.ll_state, x.ml_state=x.ml_state)
  ret
.end

.function zstd07_write_singlematch_block (
  inout: x.out,
  in: x.src, x.src_len, x.prefix_len
)
entry:
  sub x.match_len, x.src_len, x.prefix_len
  .inline zstd07_match_length_code (x.match_len=x.match_len, x.ml_code=x.ml_code, x.ml_extra=x.ml_extra, x.ml_extra_bits=x.ml_extra_bits)
  ...
  .inline zstd07_write_ml_bitstream (x.out=x.out, x.ml_extra=x.ml_extra, x.ml_extra_bits=x.ml_extra_bits)
  ret
.end

.function zstd07_compress_predef_singlematch_frame (
  in: x.dst, x.src, x.src_len,
  out: x.written
)
entry:
  ...
  .inline zstd07_find_predef_single_match (x.src=x.src, x.src_len=x.src_len, w.ok=w.ok, x.prefix_len=x.prefix_len)
  cbz w.ok, fallback_singlematch

  mov x.out, x.dst
  .inline zstd07_write_frame_header (x.out=x.out, x.src_len=x.src_len)
  .inline zstd07_write_predef_singlematch_block (x.out=x.out, x.src=x.src, x.src_len=x.src_len, x.prefix_len=x.prefix_len)
  sub x.written, x.out, x.dst
  b done

fallback_singlematch:
  .call zstd07_compress_singlematch_frame (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
...
.function zstd07_compress_singlematch_frame (
  in: x.dst, x.src, x.src_len,
  out: x.written
)
entry:
  ...
  mov x.out, x.dst
  .inline zstd07_write_frame_header (x.out=x.out, x.src_len=x.src_len)
  .inline zstd07_write_singlematch_block (x.out=x.out, x.src=x.src, x.src_len=x.src_len, x.prefix_len=x.prefix_len)
  sub x.written, x.out, x.dst
  b done

fallback_litonly:
  .call zstd07_compress_litonly_frame (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
...
.function asmp_zstd_compress_predef_singlematch_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
  .inline zstd07_bound_inner (x.src_len=x.src_len, x.bound=x.bound)
  ret
.end

.function asmp_zstd_compress_predef_singlematch_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #0
...
.function asmp_zstd_compress_predef_singlematch_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #1
...
.function asmp_zstd_compress_predef_singlematch export profile=c-aapcs64 (
...
src_ok:
  .inline zstd07_bound_inner (x.src_len=x.src_len, x.bound=x.required)
  cmp x.dst_cap, x.required
  b.lo dst_too_small

  .call zstd07_compress_predef_singlematch_frame (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
