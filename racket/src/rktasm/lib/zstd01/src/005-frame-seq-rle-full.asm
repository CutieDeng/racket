...
// This still emits one sequence and falls back to literal-only for non-repeated
// inputs.

.function zstd05_bound_inner (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function zstd05_write_frame_header (
  inout: x.out,
  in: x.src_len
)
entry:
...
.function zstd05_write_empty_block (
  inout: x.out
)
entry:
  mov x.header, #1
  .inline zstd05_write_u24 (x.out=x.out, x.value=x.header)
  ret
.end

.function zstd05_lit_header_size (
  in: x.block_len,
  out: x.header_size
)
entry:
...
.function zstd05_write_litonly_block (
  inout: x.out, x.in,
  in: x.block_len, x.last
)
entry:
  .inline zstd05_lit_header_size (x.block_len=x.block_len, x.header_size=x.lit_header_size)
...
litonly_header_done:
  ...
  .inline zstd05_copy_bytes (x.out=x.out, x.in=x.in, x.count=x.count)
...
litonly_header_two_or_three:
  ...
  .inline zstd05_write_u16 (x.out=x.out, x.value=x.header)
  b litonly_header_done
...
.function zstd05_compress_litonly_frame (
...
entry:
  ...
  .inline zstd05_write_frame_header (x.out=x.out, x.src_len=x.src_len)

  cbnz x.remaining, block_loop
  .inline zstd05_write_empty_block (x.out=x.out)
  b frame_done
...
have_block_len:
  ...
  cset x.last, eq
  .inline zstd05_write_litonly_block (x.out=x.out, x.in=x.in, x.block_len=x.block_len, x.last=x.last)
...
.function zstd05_all_equal_full (
  in: x.src, x.src_len,
  out: w.ok
)
entry:
...
.function zstd05_match_length_code (
  in: x.match_len,
...
.function zstd05_write_ml_bitstream (
  inout: x.out,
...
.function zstd05_write_seqrle_block (
  inout: x.out,
  in: x.src, x.src_len
)
entry:
  sub x.match_len, x.src_len, #1
  .inline zstd05_match_length_code (x.match_len=x.match_len, x.ml_code=x.ml_code, x.ml_extra=x.ml_extra, x.ml_extra_bits=x.ml_extra_bits)

  add x.bitstream_bytes, x.ml_extra_bits, #8
  ubfm x.bitstream_bytes, x.bitstream_bytes, #3, #63
  add x.content_size, x.bitstream_bytes, #7
  add x.header, x.content_size, x.content_size
  add x.header, x.header, x.header
  add x.header, x.header, x.header
  ...
  .inline zstd05_write_ml_bitstream (x.out=x.out, x.ml_extra=x.ml_extra, x.ml_extra_bits=x.ml_extra_bits)
  ret
.end

.function zstd05_compress_seqrle_frame (
  in: x.dst, x.src, x.src_len,
  out: x.written
)
entry:
  ...
  .inline zstd05_all_equal_full (x.src=x.src, x.src_len=x.src_len, w.ok=w.ok)
  cbz w.ok, fallback_litonly

  mov x.out, x.dst
  .inline zstd05_write_frame_header (x.out=x.out, x.src_len=x.src_len)
  .inline zstd05_write_seqrle_block (x.out=x.out, x.src=x.src, x.src_len=x.src_len)
  sub x.written, x.out, x.dst
  b done

fallback_litonly:
  .call zstd05_compress_litonly_frame (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
...
.function asmp_zstd_compress_seqrle_full_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
  .inline zstd05_bound_inner (x.src_len=x.src_len, x.bound=x.bound)
  ret
.end

.function asmp_zstd_compress_seqrle_full_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #0
...
.function asmp_zstd_compress_seqrle_full_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #1
...
.function asmp_zstd_compress_seqrle_full export profile=c-aapcs64 (
...
src_ok:
  .inline zstd05_bound_inner (x.src_len=x.src_len, x.bound=x.required)
  cmp x.dst_cap, x.required
  b.lo dst_too_small

  .call zstd05_compress_seqrle_frame (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
