// ============================================================
// 019-deflate-fixed-chain.asm - fixed-Huffman deflate with hash chains
// ============================================================
//
// This is the next step after 013-deflate-fixed-fast.asm. It keeps the stream
// format deliberately small: raw fixed-Huffman output, a raw stored-block
// encoder, and a simple fixed-vs-stored public selector; no zlib/gzip wrapper,
// no checksum, no dynamic tree, only one-byte lazy matching, and no
// parser-driven compressed block split.
// The algorithmic change is the match finder: a head table plus a prev ring
// forms bounded hash chains, so each position can test several prior matches.
//
// Prototype:
...
// ------------------------------------------------------------
.function deflate_stored_aarch64_asm export profile=c-aapcs64 visibility=hidden no-header (
...
public_src_ok:
  ...
  .call deflate_fixed_chain_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.head=x.scratch,
...
stored_public_src_ok:
  ...
  .call deflate_stored_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
