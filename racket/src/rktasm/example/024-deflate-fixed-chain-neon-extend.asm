// ============================================================
// 024-deflate-fixed-chain-neon-extend.asm - fixed-Huffman deflate with NEON match extension
// ============================================================
//
// This is the next LZ77 step after 023-deflate-fixed-chain-word-extend.asm. It
// keeps the same raw fixed-Huffman/stored/auto public surface and the same
// bounded hash-chain parser, but uses NEON to skip equal 16-byte chunks during
// match extension. Mismatching chunks still fall back to the scalar 8-byte
// locator from 023, so compressed output should stay byte-for-byte identical.
//
// Prototype:
//   uint64_t deflate_fixed_chain_neon_extend_aarch64_asm(uint8_t *dst,
//                                            const uint8_t *src,
//                                            uint64_t len,
//                                            uint32_t *head,
//                                            uint32_t *prev);
...
//   - This version deliberately uses physical v16..v18/q16..q18 inside the
//     NEON extension loop. They are caller-clobbered under AAPCS64 and keep this
//     first NEON experiment independent of vector regalloc quality.
//   - Public fixed/auto wrappers call the logical scalar core with
//     `feature=neon` or `variant=neon-extend`; the frontend resolves that to
//     this file's hand-written source variant before emitting `bl`.
...
neon_extend_loop:
  ...
  cmp x.chunk_left, #16
  b.lo word_extend_loop
  ldr q16, [x.src, x.scan_pos]
  ldr q17, [x.src, x.candidate_scan]
  eor v18.16b, v16.16b, v17.16b
  umaxv b18, v18.16b
  umov w.neon_diff, v18.b@0
  cbnz w.neon_diff, word_extend_loop
  add x.scan_pos, x.scan_pos, #16
  add x.candidate_scan, x.candidate_scan, #16
  add w.match_len, w.match_len, #16
  b neon_extend_loop
...
// ------------------------------------------------------------
// deflate_fixed_chain_neon_extend_aarch64_asm
// ------------------------------------------------------------
.function deflate_fixed_chain_neon_extend_aarch64_asm export profile=c-aapcs64 visibility=hidden no-header variant-of=deflate_fixed_chain_word_extend_aarch64_asm version=neon-extend feature=neon (
...
// ------------------------------------------------------------
// deflate_neon_extend_stored_aarch64_asm
...
// ------------------------------------------------------------
.function deflate_neon_extend_stored_aarch64_asm export profile=c-aapcs64 visibility=hidden no-header (
...
.function asmp_deflate_neon_extend_raw_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function asmp_deflate_neon_extend_raw_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #262144
...
.function asmp_deflate_neon_extend_raw_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #16
...
.function asmp_deflate_raw_fixed_neon_extend export profile=c-aapcs64 (
...
public_src_ok:
  ...
  .call deflate_fixed_chain_word_extend_aarch64_asm feature=neon (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.head=x.scratch,
...
.function asmp_deflate_raw_stored_neon_extend export profile=c-aapcs64 (
...
stored_public_src_ok:
  ...
  .call deflate_neon_extend_stored_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.written=x.written
...
.function asmp_deflate_raw_auto_neon_extend export profile=c-aapcs64 (
  in: x.dst, x.dst_cap, x.dst_len, x.src, x.src_len, x.scratch, x.scratch_len,
  out: w.status
)
entry:
...
auto_public_src_ok:
  ...
  mov x.table_bytes, #131072
  add x.prev, x.scratch, x.table_bytes
  .call deflate_fixed_chain_word_extend_aarch64_asm variant=neon-extend (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.head=x.scratch,
