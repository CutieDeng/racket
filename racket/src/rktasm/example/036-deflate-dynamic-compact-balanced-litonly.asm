// ============================================================
// 036-deflate-dynamic-compact-balanced-litonly.asm
// ============================================================
//
// Native literal-only dynamic-Huffman pipeline with a compact RLE header.
// This preserves 030 as the simple non-RLE header baseline and wires the later
// dynamic helpers into a new full stream wrapper:
//   021: literal/length frequencies
//   022: balanced literal/length lengths
//   028/029: canonical and bit-order literal/length codes
//   031: code-length RLE event table
//   032: bit-length alphabet frequencies
//   033: balanced bit-length alphabet lengths
//   028/029: canonical and bit-order bit-length codes
//
// The bit writer stays live across header and payload. Do not call the 035
// standalone header emitter here: that helper flushes the final partial header
// byte for byte-level tests, while a real deflate stream must continue writing
// payload bits immediately after the header.
...
.extern asmp_deflate_dynamic_bllen_balanced
...
//   +2574   combined_len[287]  uint8_t    287 bytes
//   +2861   rle_events[287]    4 bytes   1148 bytes
//   +4012   bl_freq[19]        uint32_t    76 bytes
//   +4088   bl_len[19]         uint8_t     19 bytes
//   +4108   bl_code[19]        uint16_t    38 bytes
//   +4146   bl_bit_code[19]    uint16_t    38 bytes
//   +4184   canon_scratch                  128 bytes
//   +4312   event_count_slot   uint64_t      8 bytes
//   total = 4320 bytes

// ------------------------------------------------------------
// dcb_flush8
// ------------------------------------------------------------
.function dcb_flush8 (inout: x.out, x.bitbuf, w.bit_count)
entry:
flush_loop:
  cmp w.bit_count, #8
  b.lt flush_done
...
// dcb_write_raw
// ------------------------------------------------------------
.function dcb_write_raw (in: x.bits, w.nbits, inout: x.out, x.bitbuf, w.bit_count)
entry:
  cbz w.nbits, write_done
  lsl x.tmp, x.bits, x.bit_count
  orr x.bitbuf, x.bitbuf, x.tmp
  add w.bit_count, w.bit_count, w.nbits
  .inline dcb_flush8 (x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
write_done:
  .return
.end

// ------------------------------------------------------------
// deflate_dynamic_compact_balanced_litonly_aarch64_asm
// ------------------------------------------------------------
.function deflate_dynamic_compact_balanced_litonly_aarch64_asm (
...
entry:
  ...
  add x.combined_len_ptr, x.scratch, #2574
  add x.event_ptr, x.scratch, #2861
  mov x.off, #4012
...
copy_ll_len_loop:
  ...
  mov x2, x.bl_len_ptr
  mov x3, #19
  bl asmp_deflate_dynamic_bllen_balanced
  mov w.status, w0
  cbnz w.status, return_status
  ...
  adrp x.order, :pg_hi21:deflate_dynamic_compact_balanced_bl_order
  add x.order, x.order, :lo12:deflate_dynamic_compact_balanced_bl_order
...
write_header_start:
  ...
  mov x.bits, #5
  mov w.nbits, #3
  .inline dcb_write_raw (x.bits=x.bits, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)

  // HLIT=29 => 286 literal/length codes. HDIST=0 => one distance code.
  mov x.bits, #29
  mov w.nbits, #5
  .inline dcb_write_raw (x.bits=x.bits, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
.function asmp_deflate_raw_dynamic_compact_balanced_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function asmp_deflate_raw_dynamic_compact_balanced_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #4320
...
.function asmp_deflate_raw_dynamic_compact_balanced_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #16
...
.function asmp_deflate_raw_dynamic_compact_balanced export profile=c-aapcs64 (
...
public_src_ok:
  ...
  .call deflate_dynamic_compact_balanced_litonly_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.scratch=x.scratch,
...
.section .rodata
deflate_dynamic_compact_balanced_bl_order:
