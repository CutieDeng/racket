// ============================================================
// 052-deflate-blocked-auto.asm - blocked fixed/stored deflate
// ============================================================
//
// This extends 051's split-block bitstream into a per-block fixed/stored
// selector. Each block first tries fixed-Huffman LZ77, then rewinds to the
// block entry state and emits a stored block when that is smaller.
//
// Prototype:
//   int asmp_deflate_raw_blocked_auto(uint8_t *dst, uint64_t dst_cap,
//                                      uint64_t *dst_len,
...
//   flags
// ------------------------------------------------------------
.function dba_flush8 (inout: x.out, x.bitbuf, w.bit_count)
entry:
flush_loop:
  cmp w.bit_count, #8
  b.lt flush_done
...
// ------------------------------------------------------------
.function dba_write_raw (in: x.bits, w.nbits, inout: x.out, x.bitbuf, w.bit_count)
entry:
  lsl x.tmp, x.bits, x.bit_count
  orr x.bitbuf, x.bitbuf, x.tmp
  add w.bit_count, w.bit_count, w.nbits
  .inline dba_flush8 (x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
  .return
.end
...
//   inout x.out, x.bitbuf, w.bit_count
// ------------------------------------------------------------
.function dba_write_huff (inout: w.code, x.out, x.bitbuf, w.bit_count, in: w.nbits)
entry:
  rbit w.code, w.code
  mov w.tmp, #32
  sub w.tmp, w.tmp, w.nbits
  lsr w.code, w.code, w.tmp
  .inline dba_write_raw (x.bits=x.code, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
  .return
.end
...
//   inout x.out, x.bitbuf, w.bit_count
// ------------------------------------------------------------
.function dba_write_fixed_lit (in: w.lit, inout: x.out, x.bitbuf, w.bit_count)
entry:
  cmp w.lit, #144
  b.hs lit_high
  ...
  add w.code, w.lit, #48
  mov w.nbits, #8
  .inline dba_write_huff (w.code=w.code, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
  .return
...
//   inout x.out, x.bitbuf, w.bit_count
// ------------------------------------------------------------
.function dba_write_eob (inout: x.out, x.bitbuf, w.bit_count)
entry:
  mov w.code, #0
  mov w.nbits, #7
  .inline dba_write_huff (w.code=w.code, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
//   inout x.out, x.bitbuf, w.bit_count
// ------------------------------------------------------------
.function dba_write_len (in: w.len, inout: x.out, x.bitbuf, w.bit_count)
entry:
  cmp w.len, #258
  b.eq len_258
...
len_extra_bits:
  ...
  .inline dba_write_raw (x.bits=x.extra_value, w.nbits=w.extra_bits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
len_done:
  .return
.end
...
//   inout x.out, x.bitbuf, w.bit_count
// ------------------------------------------------------------
.function dba_write_dist (in: w.dist, inout: x.out, x.bitbuf, w.bit_count)
entry:
  // n = distance - 1
  sub w.n, w.dist, #1
  cmp w.n, #4
...
// ------------------------------------------------------------
.function dba_insert_hash_pos (in: x.src, x.head, x.prev, x.pos)
entry:
...
// ------------------------------------------------------------
// deflate_blocked_auto_fixed_block_aarch64_asm
// ------------------------------------------------------------
.function deflate_blocked_auto_fixed_block_aarch64_asm (
...
clear_head_loop:
  ...
  mov w.nbits, #3
  .inline dba_write_raw (x.bits=x.bits, w.nbits=w.nbits, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
main_loop:
  ...
  .inline dba_search_chain (x.src=x.src, x.len=x.len, x.find_pos=x.pos, x.prev=x.prev, w.word=w.word, w.candidate_plus1=w.candidate_plus1, w.best_len=w.best_len, w.best_dist=w.best_dist)
...
emit_literal:
  ...
  .inline dba_write_fixed_lit (w.lit=w.lit, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
  add x.pos, x.pos, #1
  b main_loop
...
emit_match:
  ...
  .inline dba_search_chain (x.src=x.src, x.len=x.len, x.find_pos=x.lazy_pos, x.prev=x.prev, w.word=w.lazy_word, w.candidate_plus1=w.lazy_candidate_plus1, w.best_len=w.lazy_len, w.best_dist=w.lazy_dist)
  cmp w.lazy_len, w.best_len
  b.hi emit_literal

emit_current_match:
  .inline dba_write_len (w.len=w.best_len, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
  .inline dba_write_dist (w.dist=w.best_dist, x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
reinsert_skipped_loop:
  ...
  .inline dba_insert_hash_pos (x.src=x.src, x.head=x.head, x.prev=x.prev, x.pos=x.insert_pos)
...
finish_block:
  .inline dba_write_eob (x.out=x.out, x.bitbuf=x.bitbuf, w.bit_count=w.bit_count)
...
// ------------------------------------------------------------
// deflate_blocked_auto_stored_block_aarch64_asm
...
// ------------------------------------------------------------
.function deflate_blocked_auto_stored_block_aarch64_asm (
...
.function asmp_deflate_raw_blocked_auto_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function asmp_deflate_raw_blocked_auto_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  mov x.size, #262144
...
.function asmp_deflate_raw_blocked_auto_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #16
...
.function asmp_deflate_raw_blocked_auto export profile=c-aapcs64 (
...
emit_block:
  ...
  .call deflate_blocked_auto_fixed_block_aarch64_asm (
    x.src=x.cursor,
...
stored_estimate_ready:
  ...
  .call deflate_blocked_auto_stored_block_aarch64_asm (
    x.src=x.cursor,
    x.src_len=x.chunk_len,
    w.bfinal=w.bfinal,
    x.out=x.out,
