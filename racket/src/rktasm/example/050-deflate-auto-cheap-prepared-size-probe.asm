// ============================================================
// 050-deflate-auto-cheap-prepared-size-probe.asm
// ============================================================
//
// Experimental fixed/stored-vs-dynamic selector that first applies a cheap
// large-input LZ77 match gate, then prepares dynamic Huffman metadata once,
// estimates the encoded size, and reuses that prepared metadata when the
// dynamic path wins.
//
...
//   - 050 tests a cheap-gated prepared-emission shape without changing either
//     one.
...
.function dcpsp_len_extra_bits (in: w.symbol, out: x.extra_bits)
entry:
  mov x.extra_bits, #0
  cmp w.symbol, #265
  b.lo len_extra_done
...
.function dcpsp_dist_extra_bits (in: w.symbol, out: x.extra_bits)
entry:
  mov x.extra_bits, #0
  cmp w.symbol, #4
...
.function dcpsp_fixed_ll_nbits (in: w.symbol, out: x.nbits)
entry:
  mov x.nbits, #8
  cmp w.symbol, #144
  b.lo fixed_ll_done
...
.function dcpsp_dynamic_ll_bits (
...
dynamic_ll_loop:
  ...
  .inline dcpsp_len_extra_bits (w.symbol=w.index, x.extra_bits=x.extra_bits)
...
.function dcpsp_fixed_ll_bits (in: x.freq_ptr, out: x.bits)
entry:
  .save all
  mov x.bits, #0
  mov w.index, #0
...
fixed_ll_loop:
  ...
  mov x.freq64, #0
  add x.freq64, x.freq64, w.freq, uxtw #0
  .inline dcpsp_fixed_ll_nbits (w.symbol=w.index, x.nbits=x.nbits)
  .inline dcpsp_len_extra_bits (w.symbol=w.index, x.extra_bits=x.extra_bits)
  add x.nbits, x.nbits, x.extra_bits
  mul x.term, x.freq64, x.nbits
  add x.bits, x.bits, x.term
...
.function dcpsp_dynamic_dist_bits (
  in: x.freq_ptr, x.len_ptr,
  out: x.bits, w.status
)
entry:
...
dynamic_dist_loop:
  ...
  mov x.nbits, #0
  add x.nbits, x.nbits, w.len, uxtw #0
  .inline dcpsp_dist_extra_bits (w.symbol=w.index, x.extra_bits=x.extra_bits)
  add x.nbits, x.nbits, x.extra_bits
  mul x.term, x.freq64, x.nbits
  add x.bits, x.bits, x.term
...
.function dcpsp_fixed_dist_bits (in: x.freq_ptr, out: x.bits)
entry:
  .save all
  mov x.bits, #0
  mov w.index, #0
...
.function dcpsp_blcodes_count (in: x.bl_len_ptr, out: x.blcodes, w.status)
entry:
  .save all
  adrp x.order, :pg_hi21:deflate_auto_cheap_prepared_size_probe_bl_order
  add x.order, x.order, :lo12:deflate_auto_cheap_prepared_size_probe_bl_order
...
.function dcpsp_stored_bytes (in: x.src_len, out: x.bytes)
entry:
...
.function dcpsp_cheap_match_gate (
...
.function dcpsp_prepare_size_aarch64_asm (
...
rle_start:
  ...
  .call dcpsp_fixed_ll_bits (
    x.freq_ptr=x.ll_freq_ptr,
    x.bits=x.fixed_bits
  )
  .call dcpsp_fixed_dist_bits (
    x.freq_ptr=x.dist_freq_ptr,
...
.function dcpsp_emit_prepared_aarch64_asm (
...
.function asmp_deflate_raw_auto_dynamic_cheap_prepared_size_probe_bound export profile=c-aapcs64 (
  in: x.src_len,
  out: x.bound
)
entry:
...
.function asmp_deflate_raw_auto_dynamic_cheap_prepared_size_probe_scratch_size export profile=c-aapcs64 (
  out: x.size
)
entry:
  movz x.size, #0x4fa0
...
.function asmp_deflate_raw_auto_dynamic_cheap_prepared_size_probe_scratch_align export profile=c-aapcs64 (
  out: x.align
)
entry:
  mov x.align, #16
...
.function asmp_deflate_raw_auto_dynamic_cheap_prepared_size_probe export profile=c-aapcs64 (
...
public_src_ok:
  ...
  cbz w.allow_prepare, public_call_auto

  .call dcpsp_prepare_size_aarch64_asm (
    x.src=x.src,
    x.src_len=x.src_len,
    x.scratch=x.scratch,
    x.scratch_len=x.scratch_len,
  ...
  .call dcpsp_emit_prepared_aarch64_asm (
    x.dst=x.dst,
    x.src=x.src,
    x.src_len=x.src_len,
    x.scratch=x.scratch,
...
.data
.p2align 3
deflate_auto_cheap_prepared_size_probe_bl_order:
